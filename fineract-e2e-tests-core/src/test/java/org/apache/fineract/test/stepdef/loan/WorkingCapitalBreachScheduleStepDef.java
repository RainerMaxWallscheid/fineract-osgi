...
public class WorkingCapitalBreachScheduleStepDef extends AbstractStepDef {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(WorkingCapitalBreachScheduleStepDef.class);
    ...
    public void verifyBreachScheduleNotFound(final long loanId, final int expectedStatus) {
        final CallFailedRuntimeException exception = fail(() -> fineractClient.workingCapitalLoanBreachSchedule().retrieveBreachSchedule(loanId));
        assertThat(exception.getStatus()).as(ErrorMessageHelper.wrongStatusCodeInBreachScheduleRetrieval(exception.getStatus(), expectedStatus, loanId)).isEqualTo(expectedStatus);
    ...
    private List<WorkingCapitalLoanBreachScheduleData> retrieveBreachSchedule(final Long loanId) {
        final List<WorkingCapitalLoanBreachScheduleData> schedule = ok(() -> fineractClient.workingCapitalLoanBreachSchedule().retrieveBreachSchedule(loanId));
        log.debug("Breach schedule for loan {}: {}", loanId, schedule);
        return schedule;
    }

    private void verifyField(final WorkingCapitalLoanBreachScheduleData actual, final String fieldName, final String expectedValue, final int rowNumber) {
        switch (fieldName) {
            ...
            case "minPaymentAmount" -> verifyNullableBigDecimal(actual.getMinPaymentAmount(), expectedValue, "Min payment amount", rowNumber);
            case "outstandingAmount" -> verifyNullableBigDecimal(actual.getOutstandingAmount(), expectedValue, "Outstanding amount", rowNumber);
    ...
    private void verifyNullableBoolean(final Boolean actualValue, final String expectedValue, final String fieldDescription, final int rowNumber) {
    ...
    private void verifyNullableBigDecimal(final BigDecimal actualValue, final String expectedValue, final String fieldDescription, final int rowNumber) {
        if ("null".equals(expectedValue)) {
            assertThat(actualValue).as("%s for row %d", fieldDescription, rowNumber).isNull();
        } else {
    ...
    private void verifyNullableInteger(final Integer actualValue, final String expectedValue, final String fieldDescription, final int rowNumber) {
        if ("null".equals(expectedValue)) {
            assertThat(actualValue).as("%s for row %d", fieldDescription, rowNumber).isNull();
        } else {
    ...
    @java.lang.SuppressWarnings("all")
        public WorkingCapitalBreachScheduleStepDef(final FineractFeignClient fineractClient) {
