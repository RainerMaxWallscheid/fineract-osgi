...
public class LoanCOBStepDef extends AbstractStepDef {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(LoanCOBStepDef.class);
    ...
    public void listOfLockedLoansEmpty() {
        LoanAccountLockResponseDTO response = ok(() -> fineractClient.loanAccountLock().retrieveLockedAccounts(Map.of("page", 0, "size", 1000)));
    ...
    public void loanIsNotInListOfLockedLoans() {
        ...
        boolean contains = //
        //
        content.stream().map(LoanAccountLock::getLoanId).anyMatch(targetLoanId::equals);//
    ...
    public void loanIsLockedByChunkProcessing() {
        ...
        final LoanAccountLockResponseDTO response = ok(() -> fineractClient.loanAccountLock().retrieveLockedAccounts(Map.of("page", 0, "size", 1000)));
        Assertions.assertNotNull(response.getContent());
        Assertions.assertNotNull(targetLoanId);
        final boolean stillLocked = //
        //
        response.getContent().stream().map(LoanAccountLock::getLoanId).anyMatch(targetLoanId::equals);//
    ...
    public void placeLockOnLoanAccountWithErrorMessage() {
        ...
        executeVoid(() -> fineractClient.loanAccountLock().placeLockOnLoanAccount(loanId, "LOAN_COB_CHUNK_PROCESSING", new LockRequest().error("ERROR")));
    }
    ...
    public void placeInlineLockOnLoanAccountWithErrorMessage() {
        final PostLoansResponse loanResponse = testContext().get(TestContextKey.LOAN_CREATE_RESPONSE);
        final Long loanId = loanResponse.getLoanId();
        executeVoid(() -> fineractClient.loanAccountLock().placeLockOnLoanAccount(loanId, "LOAN_INLINE_COB_PROCESSING", new LockRequest().error("ERROR")));
    }
    ...
    public void placeLockOnLoanAccountWithNullCobBusinessDate() {
        final PostLoansResponse loanResponse = testContext().get(TestContextKey.LOAN_CREATE_RESPONSE);
        final Long loanId = loanResponse.getLoanId();
        executeVoid(() -> fineractClient.loanAccountLock().placeLockOnLoanAccount(loanId, "LOAN_COB_CHUNK_PROCESSING", new LockRequest().nullCobBusinessDate(true)));
    }
    ...
    @java.lang.SuppressWarnings("all")
        public LoanCOBStepDef(final FineractFeignClient fineractClient) {
