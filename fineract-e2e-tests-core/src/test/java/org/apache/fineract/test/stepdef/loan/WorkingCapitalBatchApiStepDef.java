...
public class WorkingCapitalBatchApiStepDef extends AbstractStepDef {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(WorkingCapitalBatchApiStepDef.class);
    ...
    private void batchApiCreateWCLoanInternal(DataTable table, String externalId) throws IOException {
        ...
        final PostWorkingCapitalLoansRequest request = workingCapitalLoanRequestFactory.defaultWorkingCapitalLoansRequest(clientId).productId(productId).submittedOnDate(loanData.get(1)).expectedDisbursementDate(loanData.get(2)).principalAmount(new BigDecimal(loanData.get(3))).totalPaymentVolume(new BigDecimal(loanData.get(4))).periodPaymentRate(new BigDecimal(loanData.get(5))).discount(new BigDecimal(loanData.get(6)));
        if (externalId != null) {
            request.externalId(externalId);
        }
        final BatchRequest batchRequest = buildBatchRequest(1L, null, "v1/working-capital-loans", BATCH_API_METHOD_POST, GSON.toJson(request));
    ...
    private void batchApiModifyWCLoanInternal(String principal, String relativeUrl) throws IOException {
        final PutWorkingCapitalLoansLoanIdRequest request = workingCapitalLoanRequestFactory.defaultModifyWorkingCapitalLoansRequest().principalAmount(new BigDecimal(principal));
    ...
    public void batchApiApproveWCLoanByExternalId(String approvedOnDate, String approvedAmount, String expectedDisbursementDate) throws IOException {
        batchApiApproveWCLoanInternal(approvedOnDate, approvedAmount, expectedDisbursementDate, resolveLoanUrlByExternalId(COMMAND_APPROVE));
    }

    private void batchApiApproveWCLoanInternal(String approvedOnDate, String approvedAmount, String expectedDisbursementDate, String relativeUrl) throws IOException {
        final PostWorkingCapitalLoansLoanIdRequest request = workingCapitalLoanRequestFactory.defaultWorkingCapitalLoanApproveRequest().approvedOnDate(approvedOnDate).expectedDisbursementDate(expectedDisbursementDate).approvedLoanAmount(new BigDecimal(approvedAmount));
        ...
        final PostWorkingCapitalLoansLoanIdResponse approveResponse = new PostWorkingCapitalLoansLoanIdResponse().loanId(loanId).resourceId(loanId);
    ...
    private void batchApiDisburseWCLoanInternal(String actualDisbursementDate, String transactionAmount, String relativeUrl) throws IOException {
        final PostWorkingCapitalLoansLoanIdRequest request = workingCapitalLoanRequestFactory.defaultWorkingCapitalLoanDisburseRequest().actualDisbursementDate(actualDisbursementDate).transactionAmount(new BigDecimal(transactionAmount));
        final BatchRequest batchRequest = buildBatchRequest(1L, null, relativeUrl, BATCH_API_METHOD_POST, GSON.toJson(request));
        final List<BatchResponse> responses = handleBatchRequests(List.of(batchRequest), false);
        testContext().set(TestContextKey.BATCH_API_CALL_RESPONSE, responses);
        final Long resourceId = extractResourceId(responses.get(0).getBody());
        if (resourceId != null) {
            final PostWorkingCapitalLoansLoanIdResponse disburseResponse = new PostWorkingCapitalLoansLoanIdResponse().resourceId(resourceId);
    ...
    public void batchApiRejectWCLoan(String rejectedOnDate) throws IOException {
        final Long loanId = getCreatedWCLoanId();
        final PostWorkingCapitalLoansLoanIdRequest request = workingCapitalLoanRequestFactory.defaultWorkingCapitalLoanRejectRequest().rejectedOnDate(rejectedOnDate);
        final BatchRequest batchRequest = buildBatchRequest(1L, null, resolveLoanUrl(COMMAND_REJECT), BATCH_API_METHOD_POST, GSON.toJson(request));
        final List<BatchResponse> responses = handleBatchRequests(List.of(batchRequest), false);
        testContext().set(TestContextKey.BATCH_API_CALL_RESPONSE, responses);
        final PostWorkingCapitalLoansLoanIdResponse rejectResponse = new PostWorkingCapitalLoansLoanIdResponse().loanId(loanId).resourceId(loanId);
    ...
    public void batchApiGetWCLoanByExternalId() throws IOException {
        final BatchRequest batchRequest = buildBatchRequest(1L, null, resolveLoanUrlForGetByExternalId(), BATCH_API_METHOD_GET, BODY_GET_REQUEST);
        final List<BatchResponse> responses = handleBatchRequests(List.of(batchRequest), false);
        testContext().set(TestContextKey.BATCH_API_CALL_RESPONSE, responses);
    }
    ...
    public void batchApiGetWCLoanDisbursementTransaction() throws IOException {
        final Long transactionId = getDisburseTransactionId();
        final BatchRequest batchRequest = buildBatchRequest(1L, null, resolveTransactionUrl(transactionId), BATCH_API_METHOD_GET, BODY_GET_REQUEST);
        final List<BatchResponse> responses = handleBatchRequests(List.of(batchRequest), false);
        testContext().set(TestContextKey.BATCH_API_CALL_RESPONSE, responses);
    }
    ...
    public void batchApiGetWCLoanDisbursementTransactionByExternalId() throws IOException {
        final Long transactionId = getDisburseTransactionId();
        final BatchRequest batchRequest = buildBatchRequest(1L, null, resolveTransactionUrlByExternalId(transactionId), BATCH_API_METHOD_GET, BODY_GET_REQUEST);
        final List<BatchResponse> responses = handleBatchRequests(List.of(batchRequest), false);
        testContext().set(TestContextKey.BATCH_API_CALL_RESPONSE, responses);
    }
    ...
    private void batchApiWCCallWithStepsInternal(String steps, String enclosingTransaction, Map<String, String> loanData) throws IOException {
    ...
    private void storeLoanIdFromBatchResponse(String[] stepArray, List<BatchResponse> responses) {
        ...
        if (wcLoanIndex >= 0 && wcLoanIndex < responses.size()) {
            ...
            if (loanId != null) {
                testContext().set(TestContextKey.WORKING_CAPITAL_LOAN_ID, loanId);
                testContext().set(TestContextKey.LOAN_CREATE_RESPONSE, new PostWorkingCapitalLoansResponse().loanId(loanId).resourceId(loanId));
            }
        }
    }
    ...
    public void batchApiWCCallWithStepsByExternalIdAndLoanData(String steps, String enclosingTransaction, DataTable loanData) throws IOException {
        batchApiWCCallWithStepsByExternalIdInternal(steps, enclosingTransaction, loanData.asMaps().get(0), null);
    }

    private void batchApiWCCallWithStepsByExternalIdInternal(String steps, String enclosingTransaction, Map<String, String> loanData, String preCreatedClientExternalId) throws IOException {
        final String[] stepArray = steps.split(", ", -1);
        final List<BatchRequest> requestList = new ArrayList<>();
        final String idempotencyKey = UUID.randomUUID().toString();
        ...
        Long previousRequestId = null;
        for (final String step : stepArray) {
            final BatchRequest request = createWCStepRequestByExternalId(requestId, previousRequestId, step.trim(), idempotencyKey, clientExternalId, loanExternalId, loanData);
            if (request != null) {
                requestList.add(request);
                previousRequestId = requestId;
                requestId++;
    ...
    private void batchApiWCCallWithFailedStepInternal(String steps, String enclosingTransaction, Map<String, String> loanData) throws IOException {
        final String[] stepArray = steps.split(", ", -1);
        final List<BatchRequest> requestList = new ArrayList<>();
        final String idempotencyKey = UUID.randomUUID().toString();
        long requestId = 1L;
    ...
    public void verifyWCLoanNoLongerExists() {
        final Long loanId = getCreatedWCLoanId();
        final CallFailedRuntimeException exception = fail(() -> fineractFeignClient.workingCapitalLoans().retrieveWorkingCapitalLoanById(loanId));
    ...
    private List<String> extractLoanFieldValues(GetWorkingCapitalLoansLoanIdResponse loan, List<String> fields) {
        ...
        for (final String field : fields) {
            switch (field) {
                ...
                case "submittedOnDate" -> values.add(loan.getTimeline() == null || loan.getTimeline().getSubmittedOnDate() == null ? null : loan.getTimeline().getSubmittedOnDate().toString());
                case "expectedDisbursementDate" -> values.add(loan.getDisbursementDetails() == null || loan.getDisbursementDetails().isEmpty() ? null : loan.getDisbursementDetails().getFirst().getExpectedDisbursementDate().toString());
                case "status" -> values.add(loan.getStatus() == null ? null : loan.getStatus().getValue());
                case "proposedPrincipal" -> values.add(loan.getProposedPrincipal() == null ? null : new Utils.DoubleFormatter(loan.getProposedPrincipal().doubleValue()).format());
                case "principal" -> values.add(loan.getBalance() == null || loan.getBalance().getPrincipal() == null ? null : new Utils.DoubleFormatter(loan.getBalance().getPrincipal().doubleValue()).format());
                case "approvedPrincipal" -> values.add(loan.getApprovedPrincipal() == null ? "0" : new Utils.DoubleFormatter(loan.getApprovedPrincipal().doubleValue()).format());
                case "totalPaymentVolume" -> values.add(loan.getTotalPaymentVolume() == null ? null : new Utils.DoubleFormatter(loan.getTotalPaymentVolume().doubleValue()).format());
                case "periodPaymentRate" -> values.add(loan.getPaymentRate() == null ? null : new Utils.DoubleFormatter(loan.getPaymentRate().doubleValue()).format());
                case "discount" -> values.add(loan.getDiscountFee() == null ? "null" : new Utils.DoubleFormatter(loan.getDiscountFee().doubleValue()).format());
                case "discountProposed" -> values.add(loan.getProposedDiscountFee() == null ? "null" : new Utils.DoubleFormatter(loan.getProposedDiscountFee().doubleValue()).format());
                case "discountApproved" -> values.add(loan.getApprovedDiscountFee() == null ? "null" : new Utils.DoubleFormatter(loan.getApprovedDiscountFee().doubleValue()).format());
    ...
    public void verifyBatchResponseWCTransactionData(final DataTable table) {
        final List<BatchResponse> responses = testContext().get(TestContextKey.BATCH_API_CALL_RESPONSE);
        assertThat(responses).isNotEmpty();
        final GetWorkingCapitalLoanTransactionIdResponse txn = fromJson(responses.get(responses.size() - 1).getBody(), GetWorkingCapitalLoanTransactionIdResponse.class);
        final List<String> header = table.row(0);
        final List<String> expectedValues = table.row(1);
        ...
        for (final String field : header) {
            switch (field) {
                ...
                case "transactionAmount" -> actualValues.add(txn.getTransactionAmount() == null ? null : new Utils.DoubleFormatter(txn.getTransactionAmount().doubleValue()).format());
                case "principalPortion" -> actualValues.add(txn.getPrincipalPortion() == null ? null : new Utils.DoubleFormatter(txn.getPrincipalPortion().doubleValue()).format());
                case "feeChargesPortion" -> actualValues.add(txn.getFeeChargesPortion() == null ? null : new Utils.DoubleFormatter(txn.getFeeChargesPortion().doubleValue()).format());
                case "penaltyChargesPortion" -> actualValues.add(txn.getPenaltyChargesPortion() == null ? null : new Utils.DoubleFormatter(txn.getPenaltyChargesPortion().doubleValue()).format());
    ...
    public void verifyBatchResponseContainsWCLoanDetailsWithData(final DataTable table) {
        ...
        final List<BatchResponse> responses = testContext().get(TestContextKey.BATCH_API_CALL_RESPONSE);
        final GetWorkingCapitalLoansLoanIdResponse loan = fromJson(responses.get(responses.size() - 1).getBody(), GetWorkingCapitalLoansLoanIdResponse.class);
        final List<String> header = table.row(0);
        final List<String> expectedValues = table.row(1);
        final List<String> actualValues = extractLoanFieldValues(loan, header);
    ...
    public void verifyStepResults200(int step) {
        final List<BatchResponse> responses = testContext().get(TestContextKey.BATCH_API_CALL_RESPONSE);
        final BatchResponse response = responses.stream().filter(r -> r.getRequestId() == step).findFirst().orElseThrow(() -> new IllegalStateException(String.format("Step %d is not found in batch responses", step)));
    ...
    private BatchRequest createWCStepRequest(Long requestId, Long reference, String step, String idempotencyKey, Map<String, String> loanData) {
        return switch (step) {
            ...
            case "approveWCLoan" -> createApproveWCLoanBatchRequest(requestId, reference, "v1/working-capital-loans/$.resourceId?command=approve");
            case "disburseWCLoan" -> createDisburseWCLoanBatchRequest(requestId, reference, "v1/working-capital-loans/$.resourceId?command=disburse");
            case "getWCLoanDetails" -> buildBatchRequest(requestId, reference, "v1/working-capital-loans/$.resourceId", BATCH_API_METHOD_GET, BODY_GET_REQUEST);
            case "getDisbursementTransaction" -> buildBatchRequest(requestId, reference, "v1/working-capital-loans/$.loanId/transactions/$.resourceId", BATCH_API_METHOD_GET, BODY_GET_REQUEST);
            case "addDiscountFee" -> createDiscountFeeBatchRequest(requestId, reference, "v1/working-capital-loans/$.loanId/transactions?command=discountFee");
            default -> null;
        };
    }

    private BatchRequest createWCStepRequestByExternalId(Long requestId, Long reference, String step, String idempotencyKey, String clientExternalId, String loanExternalId, Map<String, String> loanData) {
        return switch (step) {
            ...
            case "approveWCLoan" -> createApproveWCLoanBatchRequest(requestId, reference, "v1/working-capital-loans/external-id/" + loanExternalId + "?command=approve");
            case "disburseWCLoan" -> createDisburseWCLoanBatchRequest(requestId, reference, "v1/working-capital-loans/external-id/" + loanExternalId + "?command=disburse");
            case "getWCLoanDetails" -> buildBatchRequest(requestId, reference, "v1/working-capital-loans/external-id/" + loanExternalId, BATCH_API_METHOD_GET, BODY_GET_REQUEST);
            case "getDisbursementTransaction" -> buildBatchRequest(requestId, reference, "v1/working-capital-loans/$.loanId/transactions/$.resourceId", BATCH_API_METHOD_GET, BODY_GET_REQUEST);
            default -> null;
        };
    }
    ...
    private BatchRequest createWCLoanBatchRequest(Long requestId, Long reference, String loanExternalId, Map<String, String> loanData) {
        ...
        final PostWorkingCapitalLoansRequest wcRequest = workingCapitalLoanRequestFactory.defaultWorkingCapitalLoansRequest(1L).productId(productId).submittedOnDate(submittedOnDate).expectedDisbursementDate(expectedDisbursementDate).principalAmount(new BigDecimal(principalAmount)).totalPaymentVolume(new BigDecimal(totalPaymentVolume)).periodPaymentRate(new BigDecimal(periodPaymentRate)).discount(new BigDecimal(discount));
    ...
    @java.lang.SuppressWarnings("all")
        public WorkingCapitalBatchApiStepDef(final FineractFeignClient fineractFeignClient, final WorkingCapitalLoanProductResolver workingCapitalLoanProductResolver, final WorkingCapitalLoanRequestFactory workingCapitalLoanRequestFactory) {
