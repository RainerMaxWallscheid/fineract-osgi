...
public class LoanReAgingStepDef extends AbstractStepDef {
    ...
    public void createReAgingTransaction(DataTable table) throws IOException {
        ...
        PostLoansLoanIdTransactionsResponse response = ok(() -> fineractClient.loanTransactions().handleCommandsLoanTransaction(loanId, reAgingRequest, Map.of("command", "reAge")));
    ...
    public void createReAgingTransactionByLoanExternalId(DataTable table) {
        ...
        PostLoansLoanIdTransactionsResponse response = ok(() -> fineractClient.loanTransactions().handleCommandsLoanTransactionByLoanExternalId(loanExternalId, reAgingRequest, Map.of("command", "reAge")));
        testContext().set(TestContextKey.LOAN_REAGING_RESPONSE, response);
    }
    ...
    public void undoReAgingTransaction() {
        PostLoansResponse loanResponse = testContext().get(TestContextKey.LOAN_CREATE_RESPONSE);
        long loanId = loanResponse.getLoanId();
        PostLoansLoanIdTransactionsResponse response = ok(() -> fineractClient.loanTransactions().handleCommandsLoanTransaction(loanId, new PostLoansLoanIdTransactionsRequest(), Map.of("command", "undoReAge")));
    ...
    public void adminFailsToCreateReAgingTransactionWithError(final int statusCode, final String expectedError, final DataTable table) {
        ...
        if (developerMessage.contains(expectedError)) {
            ...
            assertThat(developerMessage).containsAnyOf("Loan cannot be re-aged as there are no outstanding balances to be re-aged", "The parameter `startDate` must be greater than or equal to the provided date");
        }
    }
    ...
    public LoanScheduleData reAgingPreviewByLoanExternalId(DataTable table) {
        ...
        String loanExternalId = loanResponse.getResourceExternalId();
        Map<String, Object> queryParams = resolveReAgingQueryParams(table);
        LoanScheduleData result = ok(() -> fineractClient.loanTransactions().previewReAgeLoanScheduleByLoanExternalId(loanExternalId, queryParams));
    ...
    public void reAgePreviewChargedOffLoanFailure(final DataTable table) {
        ...
        String loanExternalId = loanResponse.getResourceExternalId();
        Map<String, Object> queryParams = resolveReAgingQueryParams(table);
        CallFailedRuntimeException exception = fail(() -> fineractClient.loanTransactions().previewReAgeLoanScheduleByLoanExternalId(loanExternalId, queryParams));
    ...
    private List<String> validateRepaymentScheduleTotal(List<String> header, LoanScheduleData repaymentSchedule, List<String> expectedAmounts) {
        List<String> actualValues = new ArrayList<>();
        ...
        for (int i = 0; i < header.size(); i++) {
            ...
            switch (headerName) {
                case "Principal due" -> //
                //
                assertThat(repaymentSchedule.getTotalPrincipalExpected()).as(ErrorMessageHelper.wrongAmountInRepaymentSchedulePrincipal(repaymentSchedule.getTotalPrincipalExpected().doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
                case "Interest" -> //
                //
                assertThat(repaymentSchedule.getTotalInterestCharged()).as(ErrorMessageHelper.wrongAmountInRepaymentScheduleInterest(repaymentSchedule.getTotalInterestCharged().doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
                case "Fees" -> //
                //
                assertThat(repaymentSchedule.getTotalFeeChargesCharged()).as(ErrorMessageHelper.wrongAmountInRepaymentScheduleFees(repaymentSchedule.getTotalFeeChargesCharged().doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
                case "Penalties" -> //
                //
                assertThat(repaymentSchedule.getTotalPenaltyChargesCharged()).as(ErrorMessageHelper.wrongAmountInRepaymentSchedulePenalties(repaymentSchedule.getTotalPenaltyChargesCharged().doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
                case "Due" -> //
                //
                assertThat(repaymentSchedule.getTotalRepaymentExpected()).as(ErrorMessageHelper.wrongAmountInRepaymentScheduleDue(repaymentSchedule.getTotalRepaymentExpected().doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
                case "Paid" -> //
                //
                assertThat(paidActualBd).as(ErrorMessageHelper.wrongAmountInRepaymentSchedulePaid(paidActualBd.doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
                case "In advance" -> //
                //
                assertThat(repaymentSchedule.getTotalPaidInAdvance()).as(ErrorMessageHelper.wrongAmountInRepaymentScheduleInAdvance(repaymentSchedule.getTotalPaidInAdvance().doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
                case "Late" -> //
                //
                assertThat(repaymentSchedule.getTotalPaidLate()).as(ErrorMessageHelper.wrongAmountInRepaymentScheduleLate(repaymentSchedule.getTotalPaidLate().doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
                case "Waived" -> //
                //
                assertThat(repaymentSchedule.getTotalWaived()).as(ErrorMessageHelper.wrongAmountInRepaymentScheduleWaived(repaymentSchedule.getTotalWaived().doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
                case "Outstanding" -> //
                //
                assertThat(repaymentSchedule.getTotalOutstanding()).as(ErrorMessageHelper.wrongAmountInRepaymentScheduleOutstanding(repaymentSchedule.getTotalOutstanding().doubleValue(), Double.valueOf(expectedValue))).isEqualByComparingTo(new BigDecimal(expectedValue));//
            }
        }
        return actualValues;
    }

    PostLoansLoanIdTransactionsRequest setReAgeingRequestProperties(PostLoansLoanIdTransactionsRequest request, List<String> headers, List<String> values) {
    ...
    public void adminCreatesALoanReAgingTransactionByLoanExternalIDWithTheFollowingDataButFailsWithErrorCode(int errorCode, DataTable table) {
        PostLoansResponse loanResponse = testContext().get(TestContextKey.LOAN_CREATE_RESPONSE);
        String loanExternalId = loanResponse.getResourceExternalId();
        PostLoansLoanIdTransactionsRequest reAgingRequest = setReAgeingRequestProperties(//
        loanRequestFactory.defaultReAgingRequest(),  //
        ...
        CallFailedRuntimeException response = fail(() -> fineractClient.loanTransactions().handleCommandsLoanTransactionByLoanExternalId(loanExternalId, reAgingRequest, Map.of("command", "reAge")));
    ...
    public CallFailedRuntimeException getCreateLoanReAgeFailureResponse(DataTable table) {
        ...
        return fail(() -> fineractClient.loanTransactions().handleCommandsLoanTransaction(loanId, reAgingRequest, Map.of("command", "reAge")));
    }
    ...
    @java.lang.SuppressWarnings("all")
        public LoanReAgingStepDef(final EventAssertion eventAssertion, final FineractFeignClient fineractClient, final LoanRequestFactory loanRequestFactory) {
