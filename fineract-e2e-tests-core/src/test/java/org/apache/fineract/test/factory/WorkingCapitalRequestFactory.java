...
public class WorkingCapitalRequestFactory {
    ...
    public PostWorkingCapitalLoanProductsRequest defaultWorkingCapitalLoanProductRequestWithAccrualAccounting() {
        ...
        //
        //
        defaultWorkingCapitalLoanProductRequest().accountingRule(AccountingRuleEnum.ACC_DEF_REV_AM).fundSourceAccountId(accountTypeResolver.resolve(DefaultAccountType.SUSPENSE_CLEARING_ACCOUNT)).loanPortfolioAccountId(accountTypeResolver.resolve(DefaultAccountType.LOANS_RECEIVABLE)).transfersInSuspenseAccountId(accountTypeResolver.resolve(DefaultAccountType.TRANSFER_IN_SUSPENSE_ACCOUNT)).deferredIncomeLiabilityAccountId(accountTypeResolver.resolve(DefaultAccountType.DEFERRED_INTEREST_REVENUE)).incomeFromDiscountFeeAccountId(accountTypeResolver.resolve(DefaultAccountType.INTEREST_INCOME)).incomeFromFeeAccountId(accountTypeResolver.resolve(DefaultAccountType.FEE_INCOME)).incomeFromPenaltyAccountId(accountTypeResolver.resolve(DefaultAccountType.FEE_INCOME)).incomeFromRecoveryAccountId(accountTypeResolver.resolve(DefaultAccountType.RECOVERIES)).writeOffAccountId(accountTypeResolver.resolve(DefaultAccountType.WRITTEN_OFF)).overpaymentLiabilityAccountId(accountTypeResolver.resolve(DefaultAccountType.OVERPAYMENT_ACCOUNT)).goodwillCreditAccountId(accountTypeResolver.resolve(DefaultAccountType.GOODWILL_EXPENSE_ACCOUNT)).incomeFromGoodwillCreditFeesAccountId(accountTypeResolver.resolve(DefaultAccountType.FEE_CHARGE_OFF)).incomeFromGoodwillCreditPenaltyAccountId(accountTypeResolver.resolve(DefaultAccountType.FEE_CHARGE_OFF)).incomeFromChargeOffFeesAccountId(accountTypeResolver.resolve(DefaultAccountType.FEE_CHARGE_OFF)).incomeFromChargeOffPenaltyAccountId(accountTypeResolver.resolve(DefaultAccountType.FEE_CHARGE_OFF)).chargeOffExpenseAccountId(accountTypeResolver.resolve(DefaultAccountType.CREDIT_LOSS_BAD_DEBT)).chargeOffFraudExpenseAccountId(accountTypeResolver.resolve(DefaultAccountType.CREDIT_LOSS_BAD_DEBT_FRAUD)).receivableFeeAccountId(accountTypeResolver.resolve(DefaultAccountType.INTEREST_FEE_RECEIVABLE)).receivablePenaltyAccountId(accountTypeResolver.resolve(DefaultAccountType.INTEREST_FEE_RECEIVABLE));//
    }
    ...
    public PostWorkingCapitalLoanProductsRequest defaultWorkingCapitalLoanProductRequestWithDistinctAccrualAccountingMappings() {
        ...
        //
        //
        defaultWorkingCapitalLoanProductRequestWithAccrualAccounting().incomeFromPenaltyAccountId(accountTypeResolver.resolve(DefaultAccountType.RECOVERIES)).incomeFromGoodwillCreditFeesAccountId(accountTypeResolver.resolve(DefaultAccountType.INTEREST_INCOME_CHARGE_OFF)).incomeFromGoodwillCreditPenaltyAccountId(accountTypeResolver.resolve(DefaultAccountType.FEE_INCOME)).incomeFromChargeOffFeesAccountId(accountTypeResolver.resolve(DefaultAccountType.RECOVERIES)).incomeFromChargeOffPenaltyAccountId(accountTypeResolver.resolve(DefaultAccountType.INTEREST_INCOME_CHARGE_OFF));
    }
    ...
    public PostWorkingCapitalLoanProductsRequest defaultWorkingCapitalLoanProductRequest() {
        ...
        //
        //
        new PostWorkingCapitalLoanProductsRequest().name(name).shortName(shortName).description(WCLP_DESCRIPTION).fundId(FUND_ID).periodPaymentRate(new BigDecimal(1)).repaymentFrequencyType(PostWorkingCapitalLoanProductsRequest.RepaymentFrequencyTypeEnum.DAYS).repaymentEvery(DAYS_IN_MONTH_TYPE_30).startDate(null).closeDate(null).currencyCode(CURRENCY_CODE).digitsAfterDecimal(2).inMultiplesOf(1).principal(new BigDecimal(100)).minPrincipal(new BigDecimal(10)).maxPrincipal(new BigDecimal(100000)).amortizationType(PostWorkingCapitalLoanProductsRequest.AmortizationTypeEnum.EIR).npvDayCount(DAYS_IN_YEAR_TYPE_360).delinquencyBucketId(delinquencyBucketResolver.resolve(DelinquencyBucket.WC_DELINQUENCY_BUCKET)).dateFormat(DATE_FORMAT).locale(LOCALE_EN).accountingRule(AccountingRuleEnum.NONE).paymentAllocation(List.of(//
        createPaymentAllocation(PostPaymentAllocation.TransactionTypeEnum.DEFAULT.getValue(), List.of(DUE_PENALTY, DUE_FEE, DUE_PRINCIPAL, IN_ADVANCE_PENALTY, IN_ADVANCE_FEE, IN_ADVANCE_PRINCIPAL))));//
    }
    ...
    public PostWorkingCapitalLoanProductsRequest defaultWorkingCapitalLoanProductAllowAttributesOverrideRequest() {
        ...
        return //
        //
        defaultWorkingCapitalLoanProductRequest().name(name).shortName(shortName).allowAttributeOverrides(allowAttributeOverrides);
    }
    ...
    public PostWorkingCapitalLoanProductsRequest defaultWorkingCapitalLoanProductBreachRequest() {
        ...
        return //
        //
        defaultWorkingCapitalLoanProductAllowAttributesOverrideRequest().name(name).shortName(shortName).breachId(breachId);
    }
    ...
    public PostWorkingCapitalLoanProductsRequest defaultWorkingCapitalLoanProductBreachNearBreachRequest() {
        ...
        //
        //
        defaultWorkingCapitalLoanProductAllowAttributesOverrideRequest().name(name).shortName(shortName).breachId(breachId).nearBreachId(nearBreachId); //
    }

    public PutWorkingCapitalLoanProductsProductIdRequest defaultWorkingCapitalLoanProductRequestUpdate() {
        String name = Utils.randomStringGenerator(WCLP_NAME_PREFIX, 10);
        String shortName = loanProductsRequestFactory.generateShortNameSafely();
        PostAllowAttributeOverrides allowAttributeOverrides = new PostAllowAttributeOverrides().delinquencyBucketClassification(true).breach(true).discountDefault(false).periodPaymentFrequencyType(false).periodPaymentFrequency(true);
        return //
        //
        //
        //
        ...
        //
        //
        new PutWorkingCapitalLoanProductsProductIdRequest().name(name).shortName(shortName).description(WCLP_DESCRIPTION).fundId(FUND_ID).periodPaymentRate(new BigDecimal(1)).repaymentFrequencyType(PutWorkingCapitalLoanProductsProductIdRequest.RepaymentFrequencyTypeEnum.MONTHS).repaymentEvery(1).startDate(null).closeDate(null).currencyCode(CURRENCY_CODE_USD).digitsAfterDecimal(2).inMultiplesOf(1).principal(new BigDecimal(200)).minPrincipal(new BigDecimal(15)).maxPrincipal(new BigDecimal(300000)).discount(new BigDecimal(50)).amortizationType(PutWorkingCapitalLoanProductsProductIdRequest.AmortizationTypeEnum.EIR).npvDayCount(DAYS365.value).delinquencyBucketId(delinquencyBucketResolver.resolve(DelinquencyBucket.WC_DELINQUENCY_BUCKET)).dateFormat(DATE_FORMAT).locale(LOCALE_EN).allowAttributeOverrides(allowAttributeOverrides).paymentAllocation(List.of(//
    ...
    public DelinquencyBucketRequest defaultWorkingCapitalDelinquencyBucketRequest() {
        ...
        //
        //
        new DelinquencyBucketRequest().name("DB-WCL-" + Utils.randomStringGenerator(8)).bucketType(DelinquencyBucketType.WORKING_CAPITAL.name()).ranges(List.of(1L)).minimumPaymentPeriodAndRule( //
        //
        //
        //
        new MinimumPaymentPeriodAndRule().frequency(1).minimumPaymentType(DelinquencyMinimumPayment.PERCENTAGE.name()).frequencyType(DelinquencyFrequencyType.WEEKS.name()).minimumPayment(new BigDecimal("1.23")));
    }
    ...
    public WorkingCapitalBreachRequest defaultWorkingCapitalBreachRequest() {
        ...
        //
        //
        new WorkingCapitalBreachRequest().name(Utils.randomStringGenerator(DEFAULT_WC_BREACH_NAME_PREFIX, 12)).breachFrequency(DEFAULT_WC_BREACH_FREQUENCY).breachFrequencyType(DEFAULT_WC_BREACH_FREQUENCY_TYPE).breachAmountCalculationType(DEFAULT_WC_BREACH_AMOUNT_CALCULATION_TYPE).breachAmount(DEFAULT_WC_BREACH_AMOUNT);
    }
    ...
    public WorkingCapitalNearBreachRequest defaultWorkingCapitalNearBreachRequest() {
        ...
        //
        //
        new WorkingCapitalNearBreachRequest().nearBreachName("NearBreach-WCL-" + Utils.randomStringGenerator(8)).nearBreachFrequency(DEFAULT_WC_NEAR_BREACH_FREQUENCY).nearBreachFrequencyType(DEFAULT_WC_NEAR_BREACH_FREQUENCY_TYPE).nearBreachThreshold(DEFAULT_WC_NEAR_BREACH_THRESHOLD); //
    }

    public PostWorkingCapitalLoanTransactionsRequest defaultWorkingCapitalLoanRepaymentRequest() {
        return  //
        //
        new PostWorkingCapitalLoanTransactionsRequest().dateFormat(DATE_FORMAT).locale(LOCALE_EN);
    }
    ...
    private Long getWCBreachIdByName(String breachName) {
        try {
            ...
            return breaches.stream().filter(b -> breachName.equals(b.getName())).findFirst().map(WorkingCapitalBreachData::getId).orElseThrow(() -> new RuntimeException("Working Capital Breach not found with name: " + breachName));
        } catch (Exception e) {
    ...
    private Long getWCNearBreachIdByName(String breachName) {
        try {
            List<WorkingCapitalNearBreachData> breaches = fineractClient.workingCapitalNearBreaches().retrieveAllWorkingCapitalNearBreaches(Map.of());
            return breaches.stream().filter(b -> breachName.equals(b.getName())).findFirst().map(WorkingCapitalNearBreachData::getId).orElseThrow(() -> new RuntimeException("Working Capital Breach not found with name: " + breachName));
        } catch (Exception e) {
            throw new RuntimeException("Failed to fetch Working Capital Breacht by name: " + breachName, e);
        }
    }
    ...
    @java.lang.SuppressWarnings("all")
        public WorkingCapitalRequestFactory(final LoanProductsRequestFactory loanProductsRequestFactory, final FineractFeignClient fineractClient, final AccountTypeResolver accountTypeResolver, final DelinquencyBucketResolver delinquencyBucketResolver) {
