...
public class WorkingCapitalBreachConfigStepDef extends AbstractStepDef {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(WorkingCapitalBreachConfigStepDef.class);
    @Autowired
    ...
    public void adminCallsBreachTemplate() {
        final WorkingCapitalBreachTemplateResponse template = ok(() -> fineractFeignClient.workingCapitalBreaches().retrieveWorkingCapitalBreachTemplate());
    ...
    public void adminCreatesWCBreachWithValuesForUpdate() {
        ...
        //
        //
        workingCapitalRequestFactory.defaultWorkingCapitalBreachRequest().name("Breach-WC-" + Utils.randomStringGenerator(12)).breachFrequency(5).breachFrequencyType(WorkingCapitalBreachFrequencyType.YEARS.getCode()).breachAmountCalculationType(WorkingCapitalBreachCalculationType.FLAT.getCode()).breachAmount(new BigDecimal("2.34")); //
        final CommandProcessingResult response = ok(() -> fineractFeignClient.workingCapitalBreaches().createWorkingCapitalBreach(request));
        assertThat(response).isNotNull();
        assertThat(response.getResourceId()).isNotNull();
    ...
    public void adminCreateWCBreachWithDuplicateNameFailure() {
        ...
        WorkingCapitalBreachRequest breachRequestForUpdate = TestContext.INSTANCE.get(TestContextKey.WORKING_CAPITAL_BREACH_CREATE_REQUEST_FOR_UPDATE);
    ...
    public void adminModifiesWCBreachWithValues() {
        ...
        //
        //
        workingCapitalRequestFactory.defaultWorkingCapitalBreachRequest().breachFrequency(4).breachFrequencyType(WorkingCapitalBreachFrequencyType.DAYS.getCode()).breachAmountCalculationType(WorkingCapitalBreachCalculationType.FLAT.getCode()).breachAmount(new BigDecimal("7.89"));
    ...
    public void adminUpdateWCBreachWithDuplicateNameFailure() {
        ...
        String name = breachRequestForUpdate.getName();
        WorkingCapitalBreachRequest breachRequest =  //
        workingCapitalRequestFactory.defaultWorkingCapitalBreachRequest().name(name); //
        String errorMessage = ErrorMessageHelper.workingCapitalBreachDuplicateNameFailure(existingBreachId);
    ...
    public void createWCBreachWithInvalidDataFailed(final String fieldName, final String value, final String errorMessage) {
        final WorkingCapitalBreachRequest request = setWCBreachFieldValue(workingCapitalRequestFactory.defaultWorkingCapitalBreachRequest(), fieldName, value);
    ...
    private void checkCreateWCBreachWithInvalidDataFailure(final WorkingCapitalBreachRequest request, final String errorMessage, final int errorCode) {
        final CallFailedRuntimeException exception = fail(() -> fineractFeignClient.workingCapitalBreaches().createWorkingCapitalBreach(request));
    ...
    private void checkUpdateWCBreachWithInvalidDataFailure(final Long id, final WorkingCapitalBreachRequest request, final String errorMessage, final int errorCode) {
        final CallFailedRuntimeException exception = fail(() -> fineractFeignClient.workingCapitalBreaches().updateWorkingCapitalBreach(id, request));
        assertThat(exception.getStatus()).as(ErrorMessageHelper.incorrectExpectedValueInResponse()).isEqualTo(errorCode);
        assertThat(exception.getDeveloperMessage()).contains(errorMessage);
    }

    private void checkDeleteWCBreachNotFoundFailure(final Long id) {
        final CallFailedRuntimeException exception = fail(() -> fineractFeignClient.workingCapitalBreaches().deleteWorkingCapitalBreach(id));
    ...
    public void deleteWCBreachAssignedToLoanEntityFailure(Long id, String errorCode, String errorMessage) {
        ...
        //
        //
        assertThat(exception.getDeveloperMessage()).contains(errorMessage).doesNotContain("Cannot delete or update a parent row").doesNotContain("m_wc_loan_product");
    }
    ...
    public void adminFailedToDeleteWCBreachAssignedToLoanProduct() {
        ...
        //
        //
        assertThat(exception.getDeveloperMessage()).contains(ErrorMessageHelper.workingCapitalBreachLinkedToLoanProductFailure(id)).doesNotContain("Cannot delete or update a parent row").doesNotContain("m_wc_loan_product");
    }
    ...
    private void checkRetrieveWCBreachNotFoundFailure(final Long id) {
        final CallFailedRuntimeException exception = fail(() -> fineractFeignClient.workingCapitalBreaches().retrieveWorkingCapitalBreach(id));
        assertThat(exception.getStatus()).isEqualTo(404);
        assertThat(exception.getDeveloperMessage()).contains(ErrorMessageHelper.workingCapitalBreachNotFoundFailure(id));
    }

    private WorkingCapitalBreachRequest setWCBreachFieldValue(final WorkingCapitalBreachRequest request, final String fieldName, String fieldValue) {
    ...
    @java.lang.SuppressWarnings("all")
        public WorkingCapitalBreachConfigStepDef(final FineractFeignClient fineractFeignClient) {
