...
public class LoanInterestPauseStepDef extends AbstractStepDef {
    ...
    public void interestPauseCreate(final String startDate, final String endDate) throws IOException {
        ...
        final InterestPauseRequestDto interestPauseRequest = loanRequestFactory.defaultInterestPauseRequest().startDate(startDate).endDate(endDate);
        final CommandProcessingResult interestPauseResponse = ok(() -> fineractClient.loanInterestPause().createLoanInterestPause(loanId, interestPauseRequest));
    ...
    public void createLoanInterestPauseFailure(final String startDate, final String endDate) throws IOException {
        ...
        final Long loanId = loanResponse.getLoanId();
        final InterestPauseRequestDto interestPauseRequest = loanRequestFactory.defaultInterestPauseRequest().startDate(startDate).endDate(endDate);
        CallFailedRuntimeException exception = fail(() -> fineractClient.loanInterestPause().createLoanInterestPause(loanId, interestPauseRequest));
    ...
    public void checkLoanScheduleVariationsAddedBusinessEvent(final String start, final String end) {
        ...
        eventAssertion.assertEvent(LoanScheduleVariationsAddedEvent.class, loanId).extractingData(loanAccountData -> {
            ...
            boolean foundInterestPause = loanAccountData.getLoanTermVariations().stream().anyMatch(variation -> isInterestPauseWithDates(variation, startDateParsed, endDateParsed));
            assertThat(foundInterestPause).as("LoanTermVariations should contain an INTEREST_PAUSE with start date \'%s\' and end date \'%s\'", start, end).isTrue();
            return true;
        });
    }
    ...
    public void checkLoanScheduleVariationsAddedBusinessEvent(final String date) {
        eventAssertion.assertEventNotRaised(LoanScheduleVariationsAddedEvent.class, em -> FORMATTER.format(em.getBusinessDate()).equals(date));
    }
    ...
    public void checkLoanScheduleVariationsDeletedBusinessEvent(final String start, final String end) {
        ...
        eventAssertion.assertEvent(LoanScheduleVariationsDeletedEvent.class, loanId).extractingData(loanAccountData -> {
            ...
            if (loanAccountData.getLoanTermVariations() != null && !loanAccountData.getLoanTermVariations().isEmpty()) {
                foundInterestPause = loanAccountData.getLoanTermVariations().stream().anyMatch(variation -> isInterestPauseWithDates(variation, startDateParsed, endDateParsed));
            }
            assertThat(foundInterestPause).as("LoanTermVariations should NOT contain an INTEREST_PAUSE with start date \'%s\' and end date \'%s\' after deletion", start, end).isFalse();
            return true;
        });
    }
    ...
    public void checkLoanScheduleVariationsAddedBusinessEventForUpdate(final String oldStart, final String oldEnd, final String newStart, final String newEnd) {
        final PostLoansResponse loanResponse = testContext().get(TestContextKey.LOAN_CREATE_RESPONSE);
        Assertions.assertNotNull(loanResponse);
        final Long loanId = loanResponse.getLoanId();
        ...
        eventAssertion.assertEvent(LoanScheduleVariationsAddedEvent.class, loanId).extractingData(loanAccountData -> {
            assertThat(loanAccountData.getLoanTermVariations()).isNotNull();
            assertThat(loanAccountData.getLoanTermVariations()).isNotEmpty();
            boolean foundOldInterestPause = loanAccountData.getLoanTermVariations().stream().anyMatch(variation -> isInterestPauseWithDates(variation, oldStartDateParsed, oldEndDateParsed));
            assertThat(foundOldInterestPause).as("LoanTermVariations should NOT contain old INTEREST_PAUSE with start date \'%s\' and end date \'%s\' after update", oldStart, oldEnd).isFalse();
            boolean foundUpdatedInterestPause = loanAccountData.getLoanTermVariations().stream().anyMatch(variation -> isInterestPauseWithDates(variation, newStartDateParsed, newEndDateParsed));
            assertThat(foundUpdatedInterestPause).as("LoanTermVariations should contain updated INTEREST_PAUSE with new start date \'%s\' and new end date \'%s\'", newStart, newEnd).isTrue();
            return true;
        });
    }
    ...
    private boolean isInterestPauseWithDates(final org.apache.fineract.avro.loan.v1.LoanTermVariationsDataV1 variation, final LocalDate startDate, final LocalDate endDate) {
    ...
    @java.lang.SuppressWarnings("all")
        public LoanInterestPauseStepDef(final EventAssertion eventAssertion, final FineractFeignClient fineractClient, final LoanRequestFactory loanRequestFactory) {
