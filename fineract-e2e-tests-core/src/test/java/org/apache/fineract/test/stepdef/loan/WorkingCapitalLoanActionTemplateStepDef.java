...
public class WorkingCapitalLoanActionTemplateStepDef extends AbstractStepDef {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(WorkingCapitalLoanActionTemplateStepDef.class);
    ...
    public void retrieveWcLoanActionTemplate(final String templateType) {
        ...
        final WorkingCapitalLoanCommandTemplateData response = ok(() -> fineractClient.workingCapitalLoanTransactions().retrieveWorkingCapitalLoanActionTemplate(loanId, templateType));
    ...
    public void retrieveTemplateWithInvalidType(final String templateType) {
        final Long loanId = getCreatedLoanId();
        final CallFailedRuntimeException exception = fail(() -> fineractClient.workingCapitalLoanTransactions().retrieveWorkingCapitalLoanActionTemplate(loanId, templateType));
    ...
    public void retrieveTemplateWithoutType() {
        final Long loanId = getCreatedLoanId();
        final CallFailedRuntimeException exception = fail(() -> fineractClient.workingCapitalLoanTransactions().retrieveWorkingCapitalLoanActionTemplate(loanId, (String) null));
        assertThat(exception.getStatus()).as("HTTP status code").isEqualTo(400);
    ...
    public void retrieveTemplateForNonExistentLoan(final Long loanId) {
        final CallFailedRuntimeException exception = fail(() -> fineractClient.workingCapitalLoanTransactions().retrieveWorkingCapitalLoanActionTemplate(loanId, "approve"));
    ...
    private void verifyTemplateData(final DataTable table) {
        ...
        SoftAssertions.assertSoftly(softly -> expected.forEach((field, value) -> {
            switch (field) {
                case "approvalAmount" -> softly.assertThat(response.getApprovalAmount()).as(field).isNotNull().isEqualByComparingTo(new BigDecimal(value));
                case "approvalDate" -> softly.assertThat(response.getApprovalDate()).as(field).isNotNull().isEqualTo(LocalDate.parse(value));
                case "expectedDisbursementDate" -> softly.assertThat(response.getExpectedDisbursementDate()).as(field).isNotNull().isEqualTo(LocalDate.parse(value));
                case "expectedAmount" -> {
    ...
    @java.lang.SuppressWarnings("all")
        public WorkingCapitalLoanActionTemplateStepDef(final FineractFeignClient fineractClient) {
