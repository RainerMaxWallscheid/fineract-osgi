...
public class WorkingCapitalDelinquencyRescheduleStepDef extends AbstractStepDef {
    ...
    public void createProductWithDelinquencyBucket() {
        ...
        final PostWorkingCapitalLoanProductsRequest request = workingCapitalRequestFactory.defaultWorkingCapitalLoanProductRequest().name("WCLP-DLQ-" + Utils.randomStringGenerator(8)).delinquencyBucketId(bucketId).allowAttributeOverrides(new PostAllowAttributeOverrides().discountDefault(true));
        final PostWorkingCapitalLoanProductsResponse response = ok(() -> fineractFeignClient.workingCapitalLoanProducts().createWorkingCapitalLoanProduct(request));
    ...
    public void createProductWithDelinquencyBucketAndCustomBreachConfig(final DataTable table) {
        ...
        //
        //
        workingCapitalRequestFactory.defaultWorkingCapitalLoanProductAllowAttributesOverrideRequest().name("WCLP-DLQ-BR-" + Utils.randomStringGenerator(8)).delinquencyBucketId(bucketId).breachId(breachId);
        final PostWorkingCapitalLoanProductsResponse response = ok(() -> fineractFeignClient.workingCapitalLoanProducts().createWorkingCapitalLoanProduct(request));
        testContext().set(TestContextKey.WORKING_CAPITAL_LOAN_PRODUCT_CREATE_RESPONSE, response);
        testContext().set(TestContextKey.WORKING_CAPITAL_LOAN_PRODUCT_CREATE_REQUEST, request);
    ...
    public void createWcDelinquencyBucket(final int frequency, final String frequencyType, final int minimumPayment, final String minimumPaymentType) {
        final DelinquencyBucketRequest request = new DelinquencyBucketRequest().name("DB-WCL-" + Utils.randomStringGenerator(12)).bucketType("WORKING_CAPITAL").ranges(List.of(1L)).minimumPaymentPeriodAndRule(new MinimumPaymentPeriodAndRule().frequency(frequency).frequencyType(frequencyType).minimumPayment(new BigDecimal(minimumPayment)).minimumPaymentType(minimumPaymentType));
        final PostDelinquencyBucketResponse result = ok(() -> fineractFeignClient.delinquencyRangeAndBucketsManagement().createBucket(request));
        ...
        log.info("Created WC delinquency bucket id={} with frequency={} {} minimumPayment={} {}", result.getResourceId(), frequency, frequencyType, minimumPayment, minimumPaymentType);
    }
    ...
    public void failToCreateRescheduleAction(final int minimumPayment, final String minimumPaymentType, final int frequency, final String frequencyType) {
        final Long loanId = getLoanId();
        final PostWorkingCapitalLoansDelinquencyActionRequest request = buildRescheduleRequest(new BigDecimal(minimumPayment), minimumPaymentType, frequency, frequencyType);
        log.info("Attempting to create RESCHEDULE action for WC loan {} (expecting failure): minimumPayment={} {}, frequency={} {}", loanId, minimumPayment, minimumPaymentType, frequency, frequencyType);
    ...
    public void failToCreateRescheduleActionWithMessage(final int minimumPayment, final String minimumPaymentType, final int frequency, final String frequencyType, final String expectedMessage) {
        final Long loanId = getLoanId();
        final PostWorkingCapitalLoansDelinquencyActionRequest request = buildRescheduleRequest(new BigDecimal(minimumPayment), minimumPaymentType, frequency, frequencyType);
        log.info("Attempting to create RESCHEDULE action for WC loan {} (expecting HTTP 400 and message \'{}\'): minimumPayment={} {}, frequency={} {}", loanId, expectedMessage, minimumPayment, minimumPaymentType, frequency, frequencyType);
        final CallFailedRuntimeException exception = fail(() -> fineractFeignClient.workingCapitalLoanDelinquencyActions().createDelinquencyAction(loanId, request));
    ...
    public void failToCreateRescheduleActionWithTableAndMessage(final String expectedMessage, final DataTable table) {
        ...
        Optional.ofNullable(params.get("frequency")).ifPresent(v -> request.setFrequency(Integer.parseInt(v)));
        Optional.ofNullable(params.get("frequencyType")).ifPresent(request::setFrequencyType);
        log.info("Attempting to create RESCHEDULE action for WC loan {} (expecting HTTP 400 and message \'{}\'): {}", loanId, expectedMessage, params);
        final CallFailedRuntimeException exception = fail(() -> fineractFeignClient.workingCapitalLoanDelinquencyActions().createDelinquencyAction(loanId, request));
        assertThat(exception.getStatus()).as("HTTP status code").isEqualTo(400);
        assertThat(exception.getDeveloperMessage()).as("Developer message").contains(expectedMessage);
    }
    ...
    public void failToCreateEmptyRescheduleAction(final String expectedMessage) {
        ...
        request.setAction("reschedule");
        request.setLocale("en");
        log.info("Attempting to create empty RESCHEDULE action for WC loan {} (expecting HTTP 400 and message \'{}\')", loanId, expectedMessage);
        final CallFailedRuntimeException exception = fail(() -> fineractFeignClient.workingCapitalLoanDelinquencyActions().createDelinquencyAction(loanId, request));
        assertThat(exception.getStatus()).as("HTTP status code").isEqualTo(400);
        assertThat(exception.getDeveloperMessage()).as("Developer message").contains(expectedMessage);
    }
    ...
    public void verifyPeriods(final DataTable table) {
        final Long loanId = getLoanId();
        final List<WorkingCapitalLoanDelinquencyRangeScheduleData> periods = ok(() -> fineractFeignClient.workingCapitalLoanDelinquencyRangeSchedule().retrieveDelinquencyRangeSchedule(loanId));
    ...
    public void verifyBothPauseAndRescheduleActions() {
        final Long loanId = getLoanId();
        final List<WorkingCapitalLoanDelinquencyActionData> actions = retrieveDelinquencyActions(loanId);
        assertThat(actions.stream().map(a -> a.getAction().name()).toList()).as("Should contain both PAUSE and RESCHEDULE").contains("PAUSE", "RESCHEDULE");
    }
    ...
    public void verifySpecificPeriods(final DataTable table) {
        ...
        for (final Map<String, String> expected : table.asMaps()) {
            ...
            final WorkingCapitalLoanDelinquencyRangeScheduleData actual = periods.stream().filter(p -> p.getPeriodNumber().equals(periodNumber)).findFirst().orElse(null);
    ...
    private void executeRescheduleAction(final PostWorkingCapitalLoansDelinquencyActionRequest request) {
        ...
        final PostWorkingCapitalLoansDelinquencyActionResponse result = ok(() -> fineractFeignClient.workingCapitalLoanDelinquencyActions().createDelinquencyAction(loanId, request));
        assertThat(result).isNotNull();
        assertThat(result.getResourceId()).isNotNull();
    ...
    private void verifyActionField(final WorkingCapitalLoanDelinquencyActionData actual, final String field, final String expected) {
        switch (field) {
            ...
            case "endDate" -> verifyOptionalField(expected, v -> assertThat(actual.getEndDate()).as("endDate").isEqualTo(LocalDate.parse(v, DATE_FORMAT)), () -> assertThat(actual.getEndDate()).as("endDate").isNull());
            case "minimumPayment" -> verifyOptionalField(expected, v -> assertThat(actual.getMinimumPayment()).as("minimumPayment").isEqualByComparingTo(new BigDecimal(v)), () -> assertThat(actual.getMinimumPayment()).as("minimumPayment").isNull());
            case "minimumPaymentType" -> verifyOptionalField(expected, v -> assertThat(actual.getMinimumPaymentType().name()).as("minimumPaymentType").isEqualTo(v), () -> assertThat(actual.getMinimumPaymentType()).as("minimumPaymentType").isNull());
            case "frequency" -> verifyOptionalField(expected, v -> assertThat(actual.getFrequency()).as("frequency").isEqualTo(Integer.parseInt(v)), () -> assertThat(actual.getFrequency()).as("frequency").isNull());
            case "frequencyType" -> verifyOptionalField(expected, v -> assertThat(actual.getFrequencyType().name()).as("frequencyType").isEqualTo(v), () -> assertThat(actual.getFrequencyType()).as("frequencyType").isNull());
            default -> throw new IllegalArgumentException("Unknown action field: " + field);
        }
    }

    private void verifyFullScheduleField(final WorkingCapitalLoanDelinquencyRangeScheduleData actual, final String field, final String expected, final int periodNumber) {
        ...
        switch (field) {
            ...
            case "minPaymentCriteriaMet" -> verifyOptionalField(expected, v -> assertThat(actual.getMinPaymentCriteriaMet()).as(label).isEqualTo(Boolean.parseBoolean(v)), () -> assertThat(actual.getMinPaymentCriteriaMet()).as(label).isNull());
            case "delinquentDays" -> verifyOptionalField(expected, v -> assertThat(actual.getDelinquentDays()).as(label).isEqualTo(Long.parseLong(v)), () -> assertThat(actual.getDelinquentDays()).as(label).isNull());
            case "delinquentAmount" -> verifyOptionalField(expected, v -> assertThat(actual.getDelinquentAmount()).as(label).isEqualByComparingTo(new BigDecimal(v)), () -> assertThat(actual.getDelinquentAmount()).as(label).isNull());
    ...
    private PostWorkingCapitalLoansDelinquencyActionRequest buildRescheduleRequest(final BigDecimal minimumPayment, final String minimumPaymentType, final int frequency, final String frequencyType) {
        final PostWorkingCapitalLoansDelinquencyActionRequest request = new PostWorkingCapitalLoansDelinquencyActionRequest();
        request.setAction("reschedule");
    ...
    @java.lang.SuppressWarnings("all")
        public WorkingCapitalDelinquencyRescheduleStepDef(final FineractFeignClient fineractFeignClient, final WorkingCapitalRequestFactory workingCapitalRequestFactory) {
