...
public class LoanApplicationApprovalTest {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(LoanApplicationApprovalTest.class);
    ...
    public void loanApplicationApprovedAmountGreaterThanProposedAmount() {
        ...
        List<HashMap> error = (List<HashMap>) this.loanTransactionHelper.approveLoan(approveDate, approvalAmount, loanID, CommonConstants.RESPONSE_ERROR);
        assertEquals("error.msg.loan.approval.amount.can\'t.be.greater.than.loan.amount.demanded", error.get(0).get(CommonConstants.RESPONSE_ERROR_MESSAGE_CODE));
    }
    ...
    public void loanApplicationApprovalAndValidationForMultiDisburseLoans() {
        ...
        //
        //
        new LoanProductTestBuilder().withInterestTypeAsDecliningBalance().withTranches(true).withInterestCalculationPeriodTypeAsRepaymentPeriod(true).build(null));
    ...
    private void trancheLoansApprovalValidation(Integer clientID, Integer loanProductID, List<HashMap> createTranches) {
        ...
        List<HashMap<String, Object>> error = this.loanTransactionHelper.approveLoanForTranches(approveDate, expectedDisbursementDate, approvalAmount4, loanID, approveTranche4, CommonConstants.RESPONSE_ERROR);
        assertEquals("validation.msg.loan.principal.sum.of.multi.disburse.amounts.must.be.equal.to.or.lesser.than.approved.principal", error.get(0).get(CommonConstants.RESPONSE_ERROR_MESSAGE_CODE));
        this.loanTransactionHelper = new LoanTransactionHelper(this.requestSpec, this.responseSpecForStatusCode403);
        /* Sum of tranches exceeds the proposed amount */
        error = this.loanTransactionHelper.approveLoanForTranches(approveDate, expectedDisbursementDate, approvalAmount1, loanID, approveTranche1, CommonConstants.RESPONSE_ERROR);
        assertEquals("error.msg.loan.approval.amount.can\'t.be.greater.than.loan.amount.demanded", error.get(0).get(CommonConstants.RESPONSE_ERROR_MESSAGE_CODE));
        /* No. of tranches exceeds the max tranche count at product level */
        error = this.loanTransactionHelper.approveLoanForTranches(approveDate, expectedDisbursementDate, approvalAmount3, loanID, approveTranche3, CommonConstants.RESPONSE_ERROR);
        assertEquals("error.msg.disbursementData.exceeding.max.tranche.count", error.get(0).get(CommonConstants.RESPONSE_ERROR_MESSAGE_CODE));
    ...
    public Integer applyForLoanApplicationWithTranches(final Integer clientID, final Integer loanProductID, String principal, List<HashMap> tranches) {
