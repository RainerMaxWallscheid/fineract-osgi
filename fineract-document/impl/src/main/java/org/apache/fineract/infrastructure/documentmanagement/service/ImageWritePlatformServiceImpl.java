...
public class ImageWritePlatformServiceImpl implements ImageWritePlatformService {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ImageWritePlatformServiceImpl.class);
    ...
    public ImageCreateResponse createImage(final ImageCreateRequest request) {
        try {
            ...
            if (StringUtils.isEmpty(request.getType())) {
                final var type = Optional.of(contentDetectorManager.detect(ContentDetectorContext.builder().fileName(request.getFileName()).build())).filter(contentDetectorContext -> StringUtils.isNotEmpty(contentDetectorContext.getMimeType())).filter(contentDetectorContext -> !APPLICATION_OCTET_STREAM_VALUE.equalsIgnoreCase(contentDetectorContext.getMimeType())).map(ContentDetectorContext::getMimeType).orElse(APPLICATION_OCTET_STREAM_VALUE);
            ...
            final var result = 
            // delete old image
            imageIdAdapters.stream().filter(imageIdAdapter -> imageIdAdapter.accept(imageEntityType)).findFirst().flatMap(imageIdAdapter -> imageIdAdapter.get(request.getEntityId())).flatMap(imageIdResult -> imageRepository.findById(imageIdResult.getId())).map(image -> {
                storeService.delete(image.getLocation());
                return image;
            }).or(() -> Optional.of(new Image())).map(image -> image.setLocation(imagePath).setStorageType(storeService.getType().getValue())).map(imageRepository::save).map(image -> ImageCreateResponse.builder().resourceId(image.getId()).build());
            imageIdAdapters.stream().filter(imageIdAdapter -> imageIdAdapter.accept(imageEntityType)).findFirst().ifPresent(imageIdAdapter -> result.ifPresent(imageCreateResponse -> imageIdAdapter.set(request.getEntityId(), imageCreateResponse.getResourceId())));
    ...
    public ImageDeleteResponse deleteImage(final ImageDeleteRequest request) {
        try {
            return delete(request.getEntityType(), request.getEntityId()).orElseThrow(() -> new DocumentNotFoundException(request.getEntityType(), request.getEntityId(), -1L));
        } catch (final Exception e) {
            throw new DocumentInvalidRequestException(e);
        }
    }
    ...
    private Optional<ImageDeleteResponse> delete(final String entityType, final Long entityId) {
        return imageIdAdapters.stream().filter(imageIdAdapter -> imageIdAdapter.accept(entityType)).findFirst().flatMap(imageIdAdapter -> imageIdAdapter.set(entityId, null)).flatMap(imageIdResult -> imageRepository.findById(imageIdResult.getId())).map(image -> {
    ...
    @java.lang.SuppressWarnings("all")
        public ImageWritePlatformServiceImpl(final List<EntityImageIdAdapter> imageIdAdapters, final ContentStoreService storeService, final ImageRepository imageRepository, final ContentDetectorManager contentDetectorManager) {
