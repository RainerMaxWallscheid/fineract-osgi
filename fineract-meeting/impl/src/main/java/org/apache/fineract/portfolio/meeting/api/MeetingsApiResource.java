...
public class MeetingsApiResource {
    ...
    public MeetingData template(@PathParam("entityType") final String entityType, @PathParam("entityId") final Long entityId, @QueryParam("calendarId") final Long calendarId) {
        CalendarData calendarData = null;
        if (calendarId != null) {
            calendarData = calendarReadPlatformService.retrieveCalendar(calendarId, entityId, CalendarEntityType.valueOf(entityType.toUpperCase()).getValue());
        ...
        if (CalendarEntityType.isGroup(entityType)) {
            return MeetingData.builder().clients(clientReadPlatformService.retrieveActiveClientMembersOfGroup(entityId)).calendarData(calendarData).attendanceTypeOptions(meetingAttendanceDropdownReadService.retrieveAttendanceTypeOptions()).build();
        } else if (CalendarEntityType.isCenter(entityType)) {
            return MeetingData.builder().clients(clientReadPlatformService.retrieveActiveClientMembersOfCenter(entityId)).calendarData(calendarData).attendanceTypeOptions(meetingAttendanceDropdownReadService.retrieveAttendanceTypeOptions()).build();
        } else {
            throw new MeetingNotSupportedResourceException("Meeting attendance is not supported for the resource " + entityType + ". The supported resources are [" + CalendarEntityType.GROUPS.name() + ", " + CalendarEntityType.CENTERS.name() + "]", CalendarEntityType.GROUPS.name(), CalendarEntityType.CENTERS.name());
        }
    }
    ...
    public Collection<MeetingData> retrieveMeetings(@PathParam("entityType") final String entityType, @PathParam("entityId") final Long entityId, @QueryParam("limit") final Integer limit) {
        return meetingReadService.retrieveMeetingsByEntity(entityId, CalendarEntityType.valueOf(entityType.toUpperCase()).getValue(), limit);
    }
    ...
    public MeetingData retrieveMeeting(@PathParam("meetingId") final Long meetingId, @PathParam("entityType") final String entityType, @PathParam("entityId") final Long entityId) {
        var meetingData = meetingReadService.retrieveMeeting(meetingId, entityId, CalendarEntityType.valueOf(entityType.toUpperCase()).getValue());
        var clientsAttendance = meetingAttendanceReadService.retrieveClientAttendanceByMeetingId(meetingId);
        return MeetingData.builder().id(meetingData.getId()).meetingDate(meetingData.getMeetingDate()).clients(meetingData.getClients()).calendarData(meetingData.getCalendarData()).clientsAttendance(clientsAttendance).attendanceTypeOptions(meetingAttendanceDropdownReadService.retrieveAttendanceTypeOptions()).build();
    }

    @POST
    @Consumes({MediaType.APPLICATION_JSON})
    @Operation(summary = "Create a Meeting", operationId = "createMeeting")
    public MeetingCreateResponse createMeeting(@PathParam("entityType") final String entityType, @PathParam("entityId") final Long entityId, final MeetingCreateRequest request) {
    ...
    public MeetingUpdateResponse updateMeeting(@PathParam("entityType") final String entityType, @PathParam("entityId") final Long entityId, @PathParam("meetingId") final Long meetingId, final MeetingUpdateRequest request) {
        var calendarEntityType = CalendarEntityType.getEntityType(entityType);
        if (calendarEntityType == null) {
            throw new CalendarEntityTypeNotSupportedException(entityType);
        }
    ...
    public MeetingDeleteResponse deleteMeeting(@PathParam("entityType") final String entityType, @PathParam("entityId") final Long entityId, @PathParam("meetingId") final Long meetingId) {
    ...
    public MeetingAttendanceUpdateResponse updateMeetingAttendance(@PathParam("entityType") final String entityType, @PathParam("entityId") final Long entityId, @PathParam("meetingId") final Long meetingId, @QueryParam("command") final String commandParam, final MeetingAttendanceUpdateRequest request) {
    ...
    @java.lang.SuppressWarnings("all")
        public MeetingsApiResource(final MeetingReadService meetingReadService, final MeetingAttendanceReadService meetingAttendanceReadService, final MeetingAttendanceDropdownReadService meetingAttendanceDropdownReadService, final ClientReadPlatformService clientReadPlatformService, final CalendarReadPlatformService calendarReadPlatformService, final CommandDispatcher dispatcher) {
        this.meetingReadService = meetingReadService;
        this.meetingAttendanceReadService = meetingAttendanceReadService;
        this.meetingAttendanceDropdownReadService = meetingAttendanceDropdownReadService;
