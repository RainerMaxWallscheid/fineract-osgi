...
/** Spring ↔ OSGi bridge for search ports. */
@Component
public class SearchOsgiServiceRegistrar implements InitializingBean, DisposableBean {

    private static final Logger LOG = LoggerFactory.getLogger(SearchOsgiServiceRegistrar.class);

    private final ObjectProvider<SearchReadService> read;
    private final List<Object> registrations = new ArrayList<>();

    public SearchOsgiServiceRegistrar(final ObjectProvider<SearchReadService> read) {
        this.read = read;
    }
    ...
    public void afterPropertiesSet() {
        try {
            ...
            final Object bundle = getBundle.invoke(null, SearchOsgiServiceRegistrar.class);
            if (bundle == null) {
                return;
            }
            ...
            register(context, SearchReadService.class, read.getIfAvailable());
            LOG.info("Registered {} search OSGi service(s)", registrations.size());
        } catch (final ClassNotFoundException ex) {
            LOG.debug("OSGi framework classes not present; Spring-only search wiring");
        } catch (final ReflectiveOperationException ex) {
            LOG.warn("Failed to register search OSGi services: {}", ex.toString());
        }
    }
    ...
    private <T> void register(final Object context, final Class<T> type, final T service) throws ReflectiveOperationException {
        ...
        props.put("provider", "fineract-search-impl");
