...
public class JobExecutionRepository {
    ...
    public List<String> getStuckJobNames(NamedParameterJdbcTemplate jdbcTemplate) {
        ...
        return jdbcTemplate.queryForList("""
            ...
            """, Map.of("statuses", List.of(STARTED.name()), "completedStatuses", List.of(COMPLETED.name(), FAILED.name(), UNKNOWN.name()), "threshold", threshold), String.class);
    }
    ...
    public Long getStuckJobCountByJobName(String jobName) {
        ...
        return namedParameterJdbcTemplate.queryForObject("""
                ...
                GROUP BY BJI.JOB_INSTANCE_ID
                HAVING COUNT(BJI.JOB_INSTANCE_ID) <= :threshold
            """, Map.of("statuses", List.of(STARTED.name()), "jobName", jobName, "completedStatuses", List.of(COMPLETED.name(), FAILED.name(), UNKNOWN.name()), "threshold", threshold), Long.class);
    }
        ...
        try {
            if (databaseTypeResolver.isPostgreSQL()) {
                return namedParameterJdbcTemplate.queryForObject("""
                    ...
                    """, Map.of("jobCustomParamKeyName", jobCustomParamKeyName, "filterParameterName", parameterKeyName, "filterParameterValue", parameterValue, "statuses", List.of(STARTED.name(), STARTING.name()), "completedStatus", COMPLETED.name(), "jobName", jobName, "dateParameterName", dateParameterName), LocalDate.class);
                    ...
                    WHERE
                      ...
                      AND BJE.JOB_INSTANCE_ID NOT IN (
                        ...
                        WHERE
                                IBJE.STATUS = :completedStatus
                          AND IBJI.JOB_NAME = :jobName);
                    """, Map.of("jobCustomParamKeyName", jobCustomParamKeyName, "filterParameterName", parameterKeyName, "filterParameterValue", parameterValue, "dateParameterName", dateParameterName, "statuses", List.of(STARTED.name(), STARTING.name()), "jobName", jobName, "completedStatus", COMPLETED.name()), LocalDate.class);
            } else {
    ...
    @java.lang.SuppressWarnings("all")
        public JobExecutionRepository(final NamedParameterJdbcTemplate namedParameterJdbcTemplate, final FineractProperties fineractProperties, final DatabaseTypeResolver databaseTypeResolver) {
