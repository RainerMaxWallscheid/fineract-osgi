...
public class LoanCOBJobParameterProvider extends AbstractJobParameterProvider<Long> {
    ...
    private Set<JobParameterDTO> getJobParameterDTOListWithCorrectBusinessDate(Set<JobParameterDTO> jobParameterDTOset) {
        ...
        Optional<JobParameterDTO> optionalBusinessDateJobParameter = jobParameterDTOListWithCorrectBusinessDate.stream().filter(jobParameterDTO -> COBConstant.BUSINESS_DATE_PARAMETER_NAME.equals(jobParameterDTO.getParameterName())).findFirst();
        if (optionalBusinessDateJobParameter.isEmpty()) {
            jobParameterDTOListWithCorrectBusinessDate.add(new JobParameterDTO(COBConstant.BUSINESS_DATE_PARAMETER_NAME, ThreadLocalContextUtil.getBusinessDateByType(BusinessDateType.COB_DATE).format(DateTimeFormatter.ISO_DATE)));
        }
