 ...
 * under the License.
 */
package org.apache.fineract.portfolio.tax.impl.osgi;
...
/**
 * Spring ↔ OSGi bridge for Tax catalog (ADR-022 / tax plan).
 *
 * <p>Uses reflection so this class loads under plain Spring Boot without OSGi on the classpath.
 */
@Component
public class TaxOsgiServiceRegistrar implements InitializingBean, DisposableBean {

    private static final Logger LOG = LoggerFactory.getLogger(TaxOsgiServiceRegistrar.class);

    private final ObjectProvider<TaxCatalogPort> taxCatalogPort;
    private final List<Object> registrations = new ArrayList<>();

    public TaxOsgiServiceRegistrar(final ObjectProvider<TaxCatalogPort> taxCatalogPort) {
    ...
    public void afterPropertiesSet() {
        try {
            ...
            final Object bundle = getBundle.invoke(null, TaxOsgiServiceRegistrar.class);
            if (bundle == null) {
                LOG.debug("OSGi Bundle not available; skipping tax service registration");
                return;
            }
            ...
            if (context == null) {
                LOG.debug("OSGi BundleContext is null; skipping tax service registration");
                return;
            }

            register(context, TaxCatalogPort.class, taxCatalogPort.getIfAvailable());
            LOG.info("Registered {} tax OSGi service(s)", registrations.size());
        } catch (final ClassNotFoundException ex) {
            LOG.debug("OSGi framework classes not present; Spring-only tax wiring");
        } catch (final ReflectiveOperationException ex) {
            LOG.warn("Failed to register tax OSGi services: {}", ex.toString());
        }
    }
    ...
    private <T> void register(final Object context, final Class<T> type, final T service) throws ReflectiveOperationException {
        ...
        props.put("provider", "fineract-tax-impl");
