...
| `…mix.impl.osgi` | **impl** — `MixOsgiServiceRegistrar` (Spring) + DS `OSGI-INF/mix.xml` (Equinox start; empty taxonomy, lowest ranking). Composition-root hosted port: `osgi/CompositionRootOsgiBridge`. |
...
### Step 0–7, 9 ✅
Mechanical api/impl/test + registrar + Equinox DS `OSGI-INF/mix.xml` + consumer Gradle (provider / war).
