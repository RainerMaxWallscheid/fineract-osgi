...
| **Residual hardening** | Journal DTOs + client transaction residual closed; guarantor closed; event FD/RD mappers + share dividend event closed; **accrual write** + `JournalEntriesApiResource` → accounting-impl via `LoanPeriodicAccrualPort` (loan-api); still: bulkimport share residual, share/FD/RD entity-typed events + serializers, journal write/processors, provisioning write, notification listeners, security filter chains + TwoFactorServiceImpl |
