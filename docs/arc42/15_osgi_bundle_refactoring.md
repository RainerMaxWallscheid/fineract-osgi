...
| **Residual hardening** | TwoFactor closed; **share/FD/RD business events** → event-impl (`PortfolioAccountEventData`); **notification domain listeners** → notification-impl; **share/FD/RD serializers+mappers** → event-impl (share DTOs/read ports → core; `DepositAccountReadPlatformService` → savings-api; interest-transfer DTO residual stays provider); **bulkimport share** handler/populators → bulkimport-impl; still: journal **write service** (investor cycle residual), `BulkImportWorkbookPopulatorServiceImpl` (group/center read ports residual), security filter-chain configs (AuthorizationServer/OIDC) |
