...
| OSGi → Spring | Optional beans / ObjectFactory that resolve services via tracker; missing → null / no-op policy. `ChargeDefinitionPort`, `FloatingRatePort`, `TaxCatalogPort`, `ContentStoreService`, `CashierTxnValidationPort`, `LoanOriginatorReadPlatformService`, `MixTaxonomyReadService`, and `DelayedSettlementAttributeService` are `@Primary` lookup façades when Equinox is on |
