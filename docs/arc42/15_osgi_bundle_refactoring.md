...
3. **OSGi hardening** — impl `Export-Package` is registrar-only (`*.impl.osgi`); `osgi/check-manifests.py` / `./gradlew checkOsgiManifests` guards BSN, Fragment-Host, and split packages; `./gradlew osgiStageBundles` copies api/impl/core jars and writes `osgi/config/config.ini`. Remaining Equinox resolve blocker: `fineract-core` has no export list. Catalog: [`osgi/README.md`](../../osgi/README.md).
