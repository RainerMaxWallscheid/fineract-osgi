...
| OSGi → Spring | Optional beans / ObjectFactory that resolve services via tracker; missing → null / no-op policy. `ChargeDefinitionPort` and `FloatingRatePort` are `@Primary` lookup façades when Equinox is on |
