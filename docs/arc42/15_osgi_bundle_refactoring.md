...
3. **OSGi hardening** — impl `Export-Package` is registrar-only (`*.impl.osgi`); `fineract-core` BSN is `org.apache.fineract.core` and exports unique kernel packages only; `osgi/check-manifests.py` / `./gradlew checkOsgiManifests` guards BSN, Fragment-Host, split packages, the core export list, and api `Import-Package` completeness (named exported `org.apache.fineract.*` packages the sources actually import; `*` is not `DynamicImport-Package`); `./gradlew osgiStageBundles` copies api/impl/core jars and writes `osgi/config/config.ini`; `./gradlew equinoxResolveSmoke` resolves the catalog; `./gradlew equinoxStartSmoke` starts it (no Spring). Command, Wave-1 catalogs, Wave-2, Wave-3, Wave-4, core-slice, and later provider-peel ports are visible in Equinox via Bundle-Activators. Composition-root hosted ports: Wave-1 catalogs + Wave-2 `ContentStoreService` + `CashierTxnValidationPort` via `osgi/CompositionRootOsgiBridge` (`./gradlew equinoxSpringBridgeSmoke`). Remaining B3 work: optional provider embed of Equinox; later hosted Wave-2 ports (`LoanOriginatorReadPlatformService`, …). Catalog: [`osgi/README.md`](../../osgi/README.md).
