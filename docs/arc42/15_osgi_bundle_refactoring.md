...
| **B5** | Rollout | Further modules by coupling (small → large) | **Done** — ranked waves + leftover peels 1–30 closed; `./gradlew checkArchUnitFreeze` holds the leftover freeze-store budget. Freeze shrink **started** (987 leftover JPA/entity lines; enum / JSON-param leftovers closed; already-on-*-api INTERNAL ports on `moduleapi`) |
