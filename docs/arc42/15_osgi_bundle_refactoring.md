...
| **B5** | Rollout | Further modules by coupling (small → large) | **Done** — ranked waves + leftover peels 1–30 closed; `./gradlew checkArchUnitFreeze` holds the leftover freeze-store budget. Freeze shrink **started** (1166 leftover lines; GSIM / `ClientTransactionDTO` / journal `SavingsEnumerations` leftover enum retargets) |
...
2. **ArchUnit (done for B5)** — leftover freeze-store lines are JPA/entity residuals; `./gradlew checkArchUnitFreeze` / `osgi/check-archunit-freeze.py` fails if the store grows or the budget is stale. Freeze shrink started (1166). Do not peel `fineract-core` or the composition root to force a shrink.
