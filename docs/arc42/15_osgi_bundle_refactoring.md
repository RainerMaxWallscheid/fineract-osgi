...
| **B6** | Optional polish | DS-only for pure extensions; tighten exports | **Done** — Equinox-safe empty catalog ports register via `Service-Component` + staged Felix SCR. Loan / progressive-loan / WC-loan / COB / security keep Bundle-Activators. Command keeps its activator (dispatcher graph). No-port modules stay empty-activator. Boot registrars stay Spring |
