...
|------|--------|-------------|----------|------------------------|
| **1** | **`fineract-charge`** | ~40 | **In progress / largely done** — api/impl/test + registrar + Gradle retarget shipped; Step 8 residual on **LoanProduct** / **savings** (LoanCharge/WC/tranche off `Charge`). **Detailed plan:** [15_osgi_bundle_refactoring_fineract-charge.md](15_osgi_bundle_refactoring_fineract-charge.md) | `ChargeDefinitionPort` + catalog enums/exceptions; façade still for provider |
