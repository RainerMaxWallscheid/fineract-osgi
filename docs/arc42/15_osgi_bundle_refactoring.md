...
3. **OSGi hardening** — impl `Export-Package` is registrar-only (`*.impl.osgi`); `fineract-core` BSN is `org.apache.fineract.core` and exports unique kernel packages only; `osgi/check-manifests.py` / `./gradlew checkOsgiManifests` guards BSN, Fragment-Host, split packages, and the core export list; `./gradlew osgiStageBundles` copies api/impl/core jars and writes `osgi/config/config.ini`; `./gradlew equinoxResolveSmoke` resolves the catalog; `./gradlew equinoxStartSmoke` starts it (no Spring). Remaining B3 work: make a pilot port visible in the Service Registry (registrars need a Spring context inside the impl bundle). Catalog: [`osgi/README.md`](../../osgi/README.md).
