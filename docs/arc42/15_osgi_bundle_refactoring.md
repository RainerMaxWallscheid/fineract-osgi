...
| Priority | Action |
|----------|--------|
| **Stabilize** | Lower `osgi/archunit-freeze-budget.txt` when a residual fix shrinks the store; refresh module READMEs when manifests change |
...
| **Tests** | `ApiVerificationTest` (ClassGraph scan of `@Path` types on the full composition-root classpath) + OSGi slice tests (`EquinoxFrameworkLifecycleTest`, `EquinoxBootEmbedIT`) + test `logback.xml` | **No** — inventory of the assembled app and Boot-embed ranking |
...
1. **Provider hygiene (done for cucumber)** — dead cucumber plugin/`check.dependsOn('cucumber')`, cucumber test deps, and leftover testcontainers/suite-only test deps removed; provider tests are ClassGraph inventory plus OSGi slice tests (`EquinoxFrameworkLifecycleTest`, `EquinoxBootEmbedIT`).
2. **ArchUnit (done for B5)** — leftover freeze-store lines are JPA/entity residuals; `./gradlew checkArchUnitFreeze` / `osgi/check-archunit-freeze.py` fails if the store grows or the budget is stale. Freeze shrink done (0 leftover JPA/entity lines). Do not peel `fineract-core` or the composition root to force a shrink.
3. **OSGi hardening** — impl bundles have no `Export-Package` (registrar / DS types stay private); `fineract-core` BSN is `org.apache.fineract.core` and exports unique kernel packages only; `osgi/check-manifests.py` / `./gradlew checkOsgiManifests` guards BSN, Fragment-Host, split packages, the core export list, and api `Import-Package` completeness (named exported `org.apache.fineract.*` packages the sources actually import; `*` is not `DynamicImport-Package`); `./gradlew osgiStageBundles` copies api/impl/core jars and writes `osgi/config/config.ini`; `./gradlew equinoxResolveSmoke` resolves the catalog; `./gradlew equinoxStartSmoke` starts it (no Spring) and fails if a PILOT_PORT is unpublished. Command, Wave-1 catalogs, Wave-2, Wave-3, Wave-4, core-slice, and later provider-peel ports are visible in Equinox via Declarative Services. Composition-root hosted ports: every Equinox-safe PILOT_PORT via `osgi/CompositionRootOsgiBridge` (`./gradlew equinoxSpringBridgeSmoke`). Optional Boot embed: `fineract.osgi.enabled=true` starts in-process Equinox and registers every Equinox-safe hosted Spring port via `SpringOsgiPortBridge`. `OsgiServiceLookup` resolves the highest-ranked Service Registry entry for Boot callers; missing Equinox or service is empty. When Boot has no bean for an Equinox-safe hosted port, `OsgiBackedPortFactory` supplies a lazy no-op proxy that delegates after Equinox starts. Those proxies are not published back to the Service Registry. `fineract.osgi.catalog-dir` optionally installs a staged catalog (`:osgiStageBundles`) before those Spring ports. Default off. `EquinoxBootEmbedIT` covers Boot embed: `fineract.osgi.enabled=true` injects the ChargeDefinitionPort façade and ranks Spring over the staged catalog. Remaining B3 work: none. B4: `./gradlew checkForeignImplDeps` / `osgi/check-foreign-impl-deps.py` forbids new domain `*-impl` → foreign `*-impl` edges and any `*-api` → `*-impl`; `osgi/foreign-impl-allowlist.txt` is empty — no leftover-JPA foreign `-impl` edges remain. Full OSGi lookup complete for Equinox-safe hosted PILOT_PORTs (`ChargeDefinitionPort`, `FloatingRatePort`, `TaxCatalogPort`, `ContentStoreService`, `ContentStreamPort`, `CashierTxnValidationPort`, `LoanOriginatorReadPlatformService`, `MixTaxonomyReadService`, `DelayedSettlementAttributeService`, `GLClosureReadPlatformService`, `SavingsDropdownReadPlatformService`, `LoanProductLookupReadPort`, `BuyDownFeeReadPlatformService`, `WorkingCapitalLoanPeriodPaymentRateChangeReadService`, `ConfigJobParameterService`, `AccessTokenGenerationService`, `BusinessDateReadPlatformService`, `CodeReadPlatformService`, `ProvisioningCategoryReadPlatformService`, `CurrencyWritePlatformService`, `PasswordValidationPolicyReadPlatformService`, `AdHocReadPlatformService`, `TemplateMergeService`, `UserNotificationService`, `ScorecardReadPlatformService`, `FundReadPlatformService`, `AccountNumberFormatReadPlatformService`, `ReadLikelihoodService`, `TransferWritePlatformService`, `PaymentTypeReadService`, `SearchReadService`, `CollectionSheetWritePlatformService`, `StandingInstructionWritePlatformService`, `ShareProductDropdownReadPlatformService`, `GroupLevelReadPlatformService`, `ClientIdentifierWritePlatformService`, `RepaymentWithPostDatedChecksWritePlatformService`, `ProductCommandsService`, `CacheWritePlatformService`, `FineractEntityAccessReadService`, `CalendarDropdownReadPlatformService`, `MeetingAttendanceDropdownReadService`, `FieldConfigurationReadPlatformService`, `CreditBureauReadPlatformService`, `CollateralWritePlatformService`, `CollateralManagementReadService`, `NoteReadPlatformService`, `HookReadPlatformService`, `SmsWritePlatformService`, `ReportMailingJobConfigurationReadPlatformService`, `SmsCampaignDropdownReadPlatformService`, `NotificationConfigurationReadService`, `ReportWritePlatformService`, `ExternalServicesReadPlatformService`, `StuckJobExecutorService`, `PropertyService`, `PaymentDetailWritePlatformService`, `CommandDispatcher`, `PortfolioNotificationEventPort`, and `SmsCampaignTriggerEventPort` `@Primary` façades when Equinox is on). B5: `./gradlew checkArchUnitFreeze` holds the leftover freeze-store budget. B6: Equinox-safe empty catalog ports and the command dispatcher graph are DS-only; Felix SCR stages at start level 1. `osgi/check-manifests.py` requires each DS `Service-Component` file, implementation class, and provide interface on `*-api` / `fineract-core` that is also a PILOT_PORT; DS impls `Require-Capability` `osgi.extender=osgi.component`; DS / no-port impls must not Import-Package `org.osgi.framework`. Equinox start path is DS or a no-port stem. No Bundle-Activator remains. Catalog: [`osgi/README.md`](../../osgi/README.md).
...
| Export packages | api: `org.apache.fineract…moduleapi` (+ explicit contract packages); impl: none |
