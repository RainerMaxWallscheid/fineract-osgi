...
| **Residual hardening** | TwoFactor closed; **share/FD/RD business events** → event-impl (`PortfolioAccountEventData`); **notification domain listeners** → notification-impl; **share/FD/RD serializers+mappers** → event-impl; **bulkimport share** + **BulkImportWorkbookPopulatorServiceImpl** → bulkimport-impl (`Center`/`Group` read ports + `StaffCenterData` → core); still: journal **write service** (investor cycle residual), security filter-chain configs (AuthorizationServer/OIDC) |
