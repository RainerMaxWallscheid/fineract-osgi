...
3. **OSGi hardening** — impl `Export-Package` is registrar-only (`*.impl.osgi`); `fineract-core` BSN is `org.apache.fineract.core` and exports unique kernel packages only; `osgi/check-manifests.py` / `./gradlew checkOsgiManifests` guards BSN, Fragment-Host, split packages, and the core export list; `./gradlew osgiStageBundles` copies api/impl/core jars and writes `osgi/config/config.ini`. Remaining Equinox resolve work: residual same-package leftovers stay unpublished from core + install smoke. Catalog: [`osgi/README.md`](../../osgi/README.md).
