...
| OSGi → Spring | Optional beans / ObjectFactory that resolve services via tracker; missing → null / no-op policy. `ChargeDefinitionPort` is a `@Primary` lookup façade when Equinox is on |
