# 15. OSGi Bundle Refactoring (api / impl / test + Service Registry)

Stepwise refactoring of Gradle domain modules into **OSGi bundles**, with **OSGi services** as the only inter-bundle access path. Decision: [ADR-022](decisions/ADR-022-osgi-api-impl-test-bundles-services.md).

**Not in scope as integration technology:** Apache **Karaf Features** (or equivalent feature-install descriptors). Install sets may use Equinox layout/scripts only; they are not the module API.

---

## 15.1 Goals

| Goal | Meaning |
|------|---------|
| Subprojects are OSGi bundles | Each deployable unit has a proper manifest (`Bundle-SymbolicName`, versions, Import/Export) |
| api / impl / test split | Contracts, implementation, and tests are separate bundles/subprojects |
| Inter-bundle access via OSGi services | Service Registry under api interfaces; no foreign impl compile deps |
| Fragment-Host tests | Test fragments attach to the impl host |
| Keep Spring inside impl | Do **not** remove Spring before or as a prerequisite for this refactor |
| Stepwise | Pilot first; no big-bang rewrite of `fineract-provider` |

---

## 15.2 Target structure

```text
fineract-<name>/                    # optional parent / grouping
  fineract-<name>-api/              # OSGi interface bundle
  fineract-<name>-impl/             # OSGi implementation bundle
  fineract-<name>-test/             # OSGi fragment (tests)
```

Example (pilot candidates):

```text
fineract-charge-api/
fineract-charge-impl/
fineract-charge-test/

fineract-command/api/
fineract-command/impl/
fineract-command/test/              # Fragment-Host → command.impl (unit)
fineract-command-integrationtest/  # shared IT fixtures
```

```mermaid
flowchart LR
  subgraph Consumer["Consumer module"]
    CImpl[consumer-impl]
  end
  subgraph Charge["fineract-charge"]
    API[charge-api<br/>Export-Package moduleapi]
    IMPL[charge-impl<br/>registers services]
    TST[charge-test<br/>Fragment-Host → impl]
  end
  REG[(OSGi Service Registry)]

  CImpl -->|Import-Package api only| API
  IMPL --> API
  IMPL -->|register under api interface| REG
  CImpl -->|lookup service| REG
  TST -.->|Fragment-Host| IMPL
```

---

## 15.3 Bundle responsibilities

### 15.3.1 Interface bundle (`-api`)

| Include | Exclude |
|---------|---------|
| Port interfaces (`..moduleapi..`) | JPA `@Entity`, repositories |
| Stable DTOs / value objects / IDs | Spring `@Configuration`, `@Service` |
| OSGi service interfaces (often same as ports) | REST resources (`..api..` historical HTTP) |
| Public contract exceptions | EclipseLink / Jersey / servlet types |

**OSGi:** `Export-Package` only these packages. Versioned carefully (semantic version of the contract).

### 15.3.2 Implementation bundle (`-impl`)

| Include | Notes |
|---------|--------|
| Domain, handlers, services | Module-internal |
| JPA / JDBC adapters | Driven adapters ([ADR-017](decisions/ADR-017-hexagonale-architektur.md)) |
| Spring Boot / Spring configuration | **Allowed** for local DI |
| Service registration | DS component or activator / bridge registers api interfaces |

**OSGi:** Import api packages; **do not** export internal packages for use by other domain modules. Other modules must use the Service Registry.

### 15.3.3 Test bundle (`-test`)

| Pattern | When |
|---------|------|
| **`Fragment-Host: <impl-bsn>`** | White-box tests needing host packages / Spring context of impl |
| Plain JUnit on `-api` | Pure contract tests with mocks only |

Manifest (illustrative):

```text
Bundle-SymbolicName: org.apache.fineract.charge.test
Fragment-Host: org.apache.fineract.charge.impl
```

Rules:

- One host per fragment (prefer impl).  
- Do not use fragments to smuggle production code.  
- Do not require exporting impl packages solely for tests.

---

## 15.4 Access rules (mandatory)

### Allowed between bundles

1. **OSGi services** – lookup of interfaces defined in `-api`  
2. **Exported api packages** – types of the public contract only  
3. **Domain / business events** – published language ([12 Event Catalog](12_event_catalog.md))  
4. **Shared kernel** – narrow types in `fineract-core` ([ADR-021](decisions/ADR-021-modul-kommunikation-nur-ueber-module-api.md))

### Forbidden between domain bundles

| Forbidden | Why |
|-----------|-----|
| Compile dependency on foreign `-impl` | Breaks replaceability and OSGi isolation |
| `@Autowired` of foreign impl types across modules | Spring is not the inter-bundle bus |
| Import of foreign `..domain..` entities | ADR-021 / ArchUnit |
| Call foreign REST resources as module API | Driving adapter ≠ port |
| **Karaf Features** as the contract between modules | Deploy packaging ≠ service contract ([ADR-022](decisions/ADR-022-osgi-api-impl-test-bundles-services.md)) |

### Spring placement

| Location | Spring? |
|----------|---------|
| `-api` | **No** |
| `-impl` (local wiring) | **Yes** |
| Cross-bundle | **No** — OSGi Service Registry |
| Composition root bridge | Yes (thin Spring↔OSGi adapter) |

**Do not remove Spring before OSGi.** Spring remains the DI foundation inside impl and provider ([ADR-003](decisions/ADR-003-spring-boot-gradle-module-als-kern-beibehalten.md)). Optional later shrink of Spring applies only to pure extension bundles, never as a global first step.

---

## 15.5 Spring↔OSGi bridge

Place a small bridge in the composition root (`fineract-provider` or dedicated `fineract-osgi-bridge`):

| Direction | Behavior |
|-----------|----------|
| Spring → OSGi | Register selected Spring beans under **api** interfaces in the Service Registry |
| OSGi → Spring | Optional beans / ObjectFactory that resolve services via tracker; missing → null / no-op policy |
| Lifecycle | Register on start; unregister on stop; no hard fail if optional extension missing |

```mermaid
sequenceDiagram
    participant Spring as Spring Context
    participant Bridge as OSGi Bridge
    participant Reg as Service Registry
    participant Cons as Consumer impl

    Spring->>Bridge: bean implements ChargeDefinitionPort
    Bridge->>Reg: register(ChargeDefinitionPort)
    Cons->>Reg: getService(ChargeDefinitionPort)
    Reg-->>Cons: service or empty
```

---

## 15.6 Evolution stages

| Stage | Name | Deliverables | Exit criteria |
|-------|------|--------------|---------------|
| **B0** | Guardrails | `moduleapi` ports, ArchUnit freeze ([14](14_module_api_boundaries.md)) | New cross-module features only via Module API |
| **B1** | Bundle tooling | bnd (or equivalent), naming, Export-Package whitelist, Equinox install smoke | Pilot module builds as bundle JAR |
| **B2** | Pilot split | One module → `-api` / `-impl` / `-test` + Fragment-Host | Pilot tests green; no foreign impl deps for pilot consumers |
| **B3** | Bridge | Spring↔OSGi registration for pilot ports | Service visible in Equinox; optional unbind degrades cleanly |
| **B4** | OSGi-only consumers | Domain consumers use service (or bridge façade), not impl project | ArchUnit / Gradle checks block foreign `-impl` |
| **B5** | Rollout | Further modules by coupling (small → large) | Freeze store shrinks; more services registered |
| **B6** | Optional polish | DS-only for pure extensions; tighten exports | Core banking still Spring-in-impl |

### Suggested rollout order (post–command pilot)

**B2 pilot status:** `fineract-command` is the completed pilot (api / impl / test fragment + integrationtest fixtures). As-built plan: [15_osgi_bundle_refactoring_fineract-command.md](15_osgi_bundle_refactoring_fineract-command.md).

**Wave 1 status:** **complete** (charge, rates, tax).
- **charge** — **complete** (api/impl/test; façade removed; chargeId/port retargets; domain consumers **charge-api only**). Plan: [15_osgi_bundle_refactoring_fineract-charge.md](15_osgi_bundle_refactoring_fineract-charge.md).
- **rates** — **complete** (api/impl/test + `FloatingRatePort`; loan **rates-api only** via `floatingRateId`). Plan: [15_osgi_bundle_refactoring_fineract-rates.md](15_osgi_bundle_refactoring_fineract-rates.md).
- **tax** — **complete** (api/impl/test + ports; charge/loan/savings **tax-api only** via catalog ids). Plan: [15_osgi_bundle_refactoring_fineract-tax.md](15_osgi_bundle_refactoring_fineract-tax.md).

Further modules follow the same recipe, ordered by **port clarity**, **size**, **reverse dependency cost**, and **hexagonal value** (replaceable adapters as OSGi services). Metrics below are approximate main-source scale (order of magnitude); re-measure before a PR series.

#### Selection criteria

| Criterion | Why it matters |
|-----------|----------------|
| **Clear ports** | Interfaces / read-write services already exist → easy `-api` |
| **Small surface** | Prefer ≲ ~100 main classes for the next 1–2 modules |
| **Few JPA aggregates** | Less entity leakage into the contract |
| **Bounded reverse deps** | Consumers can switch to `-api` without a monorepo-wide rewrite |
| **Hexagonal fit** | Driven adapters already swappable (storage, messaging) |
| **Value of OSGi service** | Optional / replaceable impl is a real benefit |
| **Not composition root** | Leave `fineract-provider` and most of `fineract-core` last |

Per module: complete **Module API ports first** ([14.6](14_module_api_boundaries.md)), then physical api/impl/test split.

#### Wave 1 — complete (command-sized)

| Rank | Module | ~main scale | Status | Typical `-api` surface |
|------|--------|-------------|--------|------------------------|
| **1** | **`fineract-charge`** | ~40 | **Complete** — api/impl/test; chargeId/port retargets; façade **removed**; ArchUnit charge freeze green. [charge plan](15_osgi_bundle_refactoring_fineract-charge.md) | `ChargeDefinitionPort`; composition roots use api+impl |
| **2** | **`fineract-rates`** | ~20 | **Complete** — api/impl/test + `FloatingRatePort`; loan rates-api only. [rates plan](15_osgi_bundle_refactoring_fineract-rates.md) | `FloatingRatePort` + DTOs / service interfaces |
| **3** | **`fineract-tax`** | ~30 | **Complete** — api/impl/test + ports; charge/loan/savings tax-api only. [tax plan](15_osgi_bundle_refactoring_fineract-tax.md) | `TaxCatalogPort` + `ChargeTaxApplicationService` |

Suggested directory layout (mirror command):

```text
fineract-charge/
  api/   → :fineract-charge-api      BSN org.apache.fineract.charge.api
  impl/  → :fineract-charge-impl     BSN org.apache.fineract.charge.impl  (Spring stays here)
  test/  → :fineract-charge-test     Fragment-Host → charge.impl
```

Optional `fineract-<name>-integrationtest` only if several modules need shared fixtures (command needed it; charge may not).

#### Wave 2 — strong hexagonal, medium size

| Rank | Module | ~main scale | Why | Caveat |
|------|--------|-------------|-----|--------|
| **4** | **`fineract-document`** | ~85 | **Complete** — api/impl/test + `ContentStoreService` / `ContentStreamPort`; provider on ports. [document plan](15_osgi_bundle_refactoring_fineract-document.md) | Do not leak servlet/AWS types into `-api` |
| **5** | **`fineract-branch`** | ~50 | **Complete** — api/impl/test + teller ports + `CashierTxnValidationPort`; residual closed. [branch plan](15_osgi_bundle_refactoring_fineract-branch.md) | Less “optional extension” value than charge/document |
| **6** | **`fineract-loan-origination`** | ~60 | **Complete** — api/impl/test; `LoanOriginatorData` on api; loan/WC api-only. [loan-origination plan](15_osgi_bundle_refactoring_fineract-loan-origination.md) | Impl still couples to loan/WC entities for attach/detach |
| **7** | **`fineract-mix`** | ~35 | **Complete** — api/impl/test; provider composition root. [mix plan](15_osgi_bundle_refactoring_fineract-mix.md) | Niche XBRL; residual domain export for JDBC scan |

#### Wave 3 — full domain BCs (after Module API hardening)

| Rank | Module | ~main scale | Why wait |
|------|--------|-------------|----------|
| **8** | **`fineract-investor`** | ~100 | **Complete** — api/impl/test; pure ports + DTO residual journal. [investor plan](15_osgi_bundle_refactoring_fineract-investor.md) |
| **9** | **`fineract-accounting`** | ~150 | **Complete** — api/impl/test; entity residual for loan/journal consumers. [accounting plan](15_osgi_bundle_refactoring_fineract-accounting.md) |
| **10** | **`fineract-savings`** | ~180 | **Complete** — api/impl/test; provider entity residual. [savings plan](15_osgi_bundle_refactoring_fineract-savings.md) |

#### Wave 4 — last (high coupling / composition)

| Module | ~main scale | Verdict |
|--------|-------------|---------|
| **`fineract-loan`** | ~700 | **Complete** — api/impl/test; entity residual. [loan plan](15_osgi_bundle_refactoring_fineract-loan.md) |
| **`fineract-progressive-loan`** | ~95 | **Complete** — api/impl/test; schedule residual. [progressive plan](15_osgi_bundle_refactoring_fineract-progressive-loan.md) |
| **`fineract-working-capital-loan`** | ~340 | **Complete** — api/impl/test; entity residual. [WC plan](15_osgi_bundle_refactoring_fineract-working-capital-loan.md) |
| **`fineract-cob`** | ~65 | **Complete** — api/impl/test; entity-typed step residual. [cob plan](15_osgi_bundle_refactoring_fineract-cob.md) |
| **`fineract-security`** | ~70 | **Complete** — api/impl/test; Spring Security residual. [security plan](15_osgi_bundle_refactoring_fineract-security.md) |
| **`fineract-core`** | ~800 | Shared kernel — **do not** full api/impl; optional slices extracted ([core slices](15_osgi_bundle_refactoring_fineract-core-slices.md)) |
| **`fineract-provider`** | ~2500 | Composition root — hosts the Spring↔OSGi bridge; **never** the next pilot |

#### Explicit non-candidates (for OSGi BC split)

| Module | Reason |
|--------|--------|
| **`fineract-validation`** | Tiny utility / shared-kernel-ish — keep as library |
| **`fineract-architecture`** | ArchUnit rules only |
| **`fineract-report`** | Thin reporting glue |
| **command satellites** (jdbc / async / disruptor / audit) | Already modular; command Step 8 **closed** (no façade; consumers on api / api+impl) |

#### Quick decision matrix

```text
Wave 1 done             → charge, rates, tax     ★★★ complete
Wave 2 complete         → document, branch, loan-origination, mix  ★★★
Wave 3 complete         → investor, accounting, savings  ★★★
Wave 4 complete         → loan + progressive + WC + cob + security ★★★
Core slices complete    → businessdate, codes, organisation
                          (office/staff/holiday/workingdays/provisioning),
                          monetary admin, security residual  ★★★
Huge / core banking     → provider peels optional (useradmin, client, …)
Shared kernel           → core, validation         don't force BC split
```

#### After waves + core slices (optional follow-ons)

| Priority | Action |
|----------|--------|
| **Stabilize** | Keep ArchUnit freeze store shrinking; refresh docs when peels land |
| **Optional provider peels** | **`useradministration`**, **`adhocquery`**, **`template`**, **`notification`**, **`spm`**, **`fund`**, **`accountnumberformat`**, **`survey`**, **`entityaccess`**, **`calendar`**, **`meeting`**, **`address`**, **`creditbureau`**, **`collateral` (legacy)**, **`collateralmanagement`**, **`note`**, **`hooks`**, **`sms`**, **`reportmailingjob`**, **`campaigns`**, **`gcm`**, **`dataqueries`**, **`configuration`**, **`bulkimport`**, **`instancemode`**, **`jobs`**, **`s3`**, **`openapi`**, **`springbatch`**, **`event`**, **`interoperation` complete**; residual share events + optional peels deferred |
| **Residual hardening** | Documented residual set closed. Follow-ups: security configs/tests; LoanJournalEntryPosterImpl; ContentS3Config; Loan COB business steps; loan lock residual; lock/business-step REST; LoanCOBConstant + tasklets; loan/WC catch-up; retrieve-id; item readers; inline executor; savings retrieve-id; loan COB manager; WC inline config; loan COB processors; leftover COB JUnit tests; interest-rate chart; FD/RD product write/read; FD/RD account entities; leftover savings command handlers; deposit-account chart read + FD interest calc; product REST; account transaction/charge REST; savings/RD account-collection REST; peelable savings jobs; leftover savings charge-read; transaction validators; leftover GSIM write; **leftover savings accrual job + write port** → savings-impl/api (impl stays on provider: leftover assembler). **leftover post-interest job** → savings-impl (leftover poster + leftover read port already there). **leftover adhoc job** → adhocquery-impl. **leftover savings batch strategies** → savings-impl (apply/get/transaction/adjust/charge/pay-charge; `DisburseToSavings` stays on provider: leftover loan REST). **leftover FD account REST** → savings-impl after leftover `AccountAssociationsReadPlatformService` (+ data) → core (impl stays on provider). **leftover `ConvertChargeDataToSpecificChargeData`** → core. **leftover savings-product read/write** → savings-impl via leftover entityaccess-api office ports (`FineractEntityAccessReadService` / `OfficeProductRestrictionService`). **leftover savings transaction search** → savings-impl (leftover `SavingsAccountTransactionsMapper` extracted). **leftover account-associations read impl** (+ leftover `AccountAssociationType`) → core. **leftover pure account-transfer enums** → core. **leftover transfer-interest job** → savings-impl via thin `InterestTransferData` + leftover read/write ports (provider adapter builds leftover `AccountTransferDTO`). **leftover pure account-transfer support types** (enumerations helper, constants, thin DTOs/request params, exceptions, portfolio/SI-history read ports) → core. **leftover portfolio-account/SI-history read impls + transfer mapper + transfer/SI validators + SI write port + `ColumnValidator`** → core. **leftover `OfficeReadPlatformService` + account-transfer read impl + SI data/read + SI-history REST** → core. **leftover account-transfer/SI REST + SI command handlers** → core. **leftover transfer command handlers** → core via thin `AccountTransfersCommandWritePort` (provider write port keeps leftover `AccountTransferDTO` entity methods). **leftover execute-SI job** → core via thin `AccountTransferFundsData` + leftover funds write port (provider adapter). **leftover account read Spring config + reverse methods on command write port** → core. **leftover transfer-fee-charge job** → loan-impl via thin `AccountTransferFundsData` + leftover funds write port. **leftover deposit-account read** → savings-impl. **leftover interest-transfer adapter** → funds write port. **leftover savings template read** → savings-impl via thin `EntityDatatableTemplatesReadService` on dataqueries-api. **leftover savings-account read** → savings-impl (inline interest-posting assemble; accrual by `Long savingsId`). **leftover interest-transfer adapter** → savings-impl. **leftover create-account-transfer batch strategy** → core. **leftover datatable batch strategies** → dataqueries-impl (+ tests in dataqueries-test). **leftover pure client-transfer support types + write port + command handlers** → core (entity write impl stays on provider). **leftover loan interest-pause + reschedule batch strategies** → loan-impl (+ reschedule strategy test in loan-test). **leftover working-capital-loan batch strategies** → working-capital-loan-impl (+ tests in working-capital-loan-test). **leftover pure share status enums + SharesEnumerations + share write port + command handlers** → core (entity write/read/job stay on provider). **leftover pure client write ports + command handlers** → core (family/address handlers stay: entity/`AddressWritePlatformService`). **leftover pure share-product write port + dropdown + exceptions + handlers** → core. **leftover client family write port + handlers** → core (thinned; no `Client` entity on port). **leftover client address handlers** → address-impl. **leftover pure share JDBC reads** (purchased shares + account/product dividend) → core. **leftover client address read** → address-impl. **leftover client family/identifier read** (+ DTO/exceptions) → core. **leftover post-dividends job + ShareAccountSchedularService + ShareDividendApiResource** → core (entity schedular impl stays). **leftover client family REST + address REST** → core/address-impl. **leftover pure client exceptions + ClientEnumerations + LoanStatusMapper** → core. **leftover share/product command ports + ShareProductCommandsServiceImpl** → core. **leftover client-transaction external-id lookup** thinned to `Long`. **leftover client identifiers + transactions REST** (+ txn JDBC read, request/command/deserializers) → core. **leftover ShareAccountCommandsServiceImpl** stub + **ShareAccountApiConstants** → core. **leftover accounts package** (AccountsApiResource + swagger, AccountsApiConstants, AccountRequest/ChargesRequest, ShareAccountNotFoundException) → core. **leftover products package** (ProductsApiResource + swagger, ProductsApiConstants, ProductNotFoundException) → core. **leftover client search v2** (+ ClientChargeData) → core. **leftover pure group residual** (exceptions, GroupTypes/enumerations, level/role repos+wrappers, write ports, level/roles JDBC reads, validators, GroupsLevelApiResource, non-collection-sheet handlers) → core. **leftover client validators + internal client REST** → core. **leftover collectionsheet pure** (request/DTO bulk, write port, handlers, deserializers; JLG sheet DTO stays: LoanProductData) + group collection-sheet handlers + AllGroupTypesDataMapper → core. **leftover AdHocQuerySearchRequest** → core. **leftover postdated-check pure** (data/status/exceptions/ports/handlers/REST) → core (entity/assembler/impls stay with loan). **leftover client family/nonperson/transfer domain** + family/identifier write impls + ClientMapper → core. **leftover group center/group JDBC reads + roles write impl** → core. **leftover jersey converters/serializers + command deserializers + performance sampling** → core. **leftover StaffReadService** port → core (enables group JDBC reads). **leftover product-mix package** → loan-impl. **leftover postdated-check entity residual** (repo/assembler/impls/config) → loan-impl. **leftover ShareProduct domain** (+ market price/repos) → core. **leftover ClientTemplateReadPlatformService** port + pure jersey/ExternalIdConverter/CustomDateTimeProvider/SchemaUpgradeNeededException → core. **leftover loan-product residual** (REST, validator, read/write impls, config, assembler/update util) → loan-impl via `OfficeProductRestrictionService` + `ProductToGLAccountMappingValidator`; **RateRepository/Wrapper/NotFound** → core. **RateReadService** → core. **Advanced payment/credit allocation parsers+validators+mergers** (from progressive-loan) + loan product business events → loan-impl. **leftover search+collectionsheet read/REST** → core via thin `LoanProductLookupData`/`LoanProductLookupReadPort`; **CalendarRepository/Wrapper + CalendarNotFoundException**; **meeting attendance types/dropdown port**; **share dividend domain/repos** → core. **StaffRepositoryWrapper** → core; **ShareProductDividendAssembler** → core; **AccountNumberFormatRepositoryWrapper** + not-found; pure auditors/password encoder/jpa customizer/diagnostics → core; SavingsConfiguration thinned to OfficeProductRestrictionService. **leftover pure loan schedule/point-in-time DTOs** (`LoanRepaymentScheduleService`, `LoanSchedulePeriodDataWrapper`, `LoanPointInTimeData` + fee/interest/penalty/principal/total/accrual balances, re-age/re-amortization preview requests, `LoanInterestPaymentWaiverCommandHandler`) → loan-impl; **leftover pure Spring configs** (`FineractCorsConfiguration`, `FineractStartupValidationConfig`, `SpringConfig`) → core. **leftover pure loan officer/summary residual** (`LoanOfficerService`/`LoanOfficerValidator`, `InterestRefundServiceDelegate`, `CommonLoanSummaryDataProvider`/`CumulativeLoanSummaryDataProvider`, `LoanAdjustmentService`+`LoanAdjustmentParameter`, point-in-time request DTOs, `NoProvisioningCriteriaDefinitionFoundException`) → loan-impl; **leftover pure Metrics/OkHttp configs** → core. **HolidayRepository/Wrapper/NotFound + WorkingDaysRepository/Wrapper/NotFound** → core (unlocks loan holiday/util). **leftover LoanUtilService** (drop unused NoteRepository; calendar via thin `CalendarInstanceLookupPort`) + expected-disbursement validator + apply-holidays job + PIT impl → loan-impl. **leftover loan charge/bulk/disbursement REST** + charge batch strategies → loan-impl; **leftover Hikari/Compatibility/JPA/Jersey configs** → core. **leftover LoanChargeReadPlatformServiceImpl + GuarantorAccountSummaryData** → loan-impl; **leftover InternalLoanInformationApiResource** → progressive-loan-impl; **leftover Gmail email service** → configuration-impl; **leftover JdbcTransactionConfig + AbandonedConnectionCleanupShutdownListener** → core. **leftover loan accrual processing impls** → loan-impl; **leftover DefaultLoanScheduleGeneratorFactory** → progressive-loan-impl. **leftover progressive amortization/summary/refund/schedule-generator impls** → progressive-loan-impl; **leftover LoanAccountAutoStarter + transaction processing/reprocess impls** → progressive-loan-impl; **leftover guarantor read/REST** → loan-impl; **leftover TransactionBoundCacheManager** → core; **leftover client-charge read/REST + share-account charge read** → charge-impl; **leftover LoanChargeAssembler + reschedule preview/write impls** → loan-impl; **leftover cache/JDBC/liquibase residual** → core; **leftover share-product residual** → charge-impl (via core `ShareProductDividentsCreateBusinessEvent`). **leftover re-age/re-amortize/termination/adjustment + loan-transaction REST/batch** → progressive-loan-impl; **leftover generate-loan-loss-provisioning job** → accounting-impl. **leftover loanaccount peel** (foreclosure/schedule validators, disbursement assemblers, PIT API+port, accrual/buy-down/capitalized-income event services, `LoanAccountServiceImpl`, bulk-loan DTOs+read port, domain helper, schedule-history impls, accrual/overdue-charge/recalculate-interest jobs) → loan-impl. FD account REST stays (leftover `AccountAssociationsReadPlatformService`). Savings-product write stays on provider (entityaccess-impl already depends on savings-impl). Cucumber COB stepdefs stay on provider. **leftover account-details DTO+read port + LoansApiResourceSwagger** → loan-impl; **leftover account-details JDBC impl + loan/bulk/GLIM read impls** → progressive-loan-impl (via leftover WC application read). FD account REST stays (leftover `AccountAssociationsReadPlatformService`). Savings-product write stays on provider (entityaccess-impl already depends on savings-impl). Cucumber COB stepdefs stay on provider. **leftover client charge/transaction entities + write impls** → charge-impl (next to leftover client-charge read/REST). **leftover share-account entity/serializer/note** → savings-impl (read stays: leftover charge-impl share-charge read; write/schedular stay: leftover `AccountNumberGenerator` + leftover assembler). **leftover client/groups/centers REST + swagger + client batch** → progressive-loan-impl (composes leftover loan-impl + leftover savings-impl). **leftover account-transfer/associations entities + guarantor domain** → progressive-loan-impl (Loan + SavingsAccount binding; write/assemblers stay). **leftover LoansApiResource + loan batch + schedule/validators + client/share read + transfer write port** → progressive-loan-impl. **leftover write/assembler residual** → progressive-loan-impl. **leftover batch/service tests** for already-moved progressive/loan/accounting production → progressive-loan-test / loan-test / accounting-test. **leftover jobs tests** → jobs-test. **leftover event tests** → event-test (producers/serializers + BulkBusinessEvent) / core (notifier/config/jobs/mappers + instance-mode filter) / investor-test (ExternalEventService + Enricher). **leftover businessdate tests** → businessdate-test. **leftover notification publisher tests** → notification-test. **leftover bulkimport workbook/API tests** → bulkimport-test. **leftover campaigns email/SMS DTO tests** → campaigns-test. **leftover core/command/batch-utils/client-validator/SI serialization tests** → core (cucumber and testcontainers visibility stay). **leftover `CommandStrategyProviderTest`** → progressive-loan-test (composes leftover loan/savings/WC/dataqueries/charge/core strategies). **leftover `CommandProcessingResultSerializationTest`** → core (thinned off leftover `TestConfiguration`). **leftover organisation provisioning tests** → organisation-test. **leftover loan journal-entry tests** → accounting-test. **leftover COB bulk-event junit** → cob-test. **leftover client-read/savings-write/account-number tests** → progressive-loan-test. **leftover loan-impl/delinquency tests** → loan-test. **leftover progressive charge-write/re-age/re-amortize/adjustment/LoansApiResource tests** → progressive-loan-test. **leftover dataqueries datatable tests** → dataqueries-test (leftover `DatatableReadServiceImplTest` thinned off leftover `TestConfiguration`). **leftover `SqlValidatorTest` + `SqlValidator` cucumber** → security-test (constructs leftover `DefaultSqlValidator`; junit-platform cucumber suite). **leftover Liquibase/InstanceType cucumber** → core (shared non-Spring `CucumberFineractProperties`; junit-platform cucumber suite). **leftover mix/template/accounting/springbatch cucumber** → mix-test / template-test / accounting-test / springbatch-test. **leftover cob-impl cucumber** (`COBBusinessStepService`, initialisation, apply-lock) → cob-test. **leftover loan-item COB cucumber + loan-product validation cucumber** → loan-test. **leftover self-contained core cucumber** (SQLBuilder, log-escape, classpath scan, batch builder) → core. **leftover LoanCOB API filter tests** → jobs-test. **leftover JPA/JDBC visibility test** → core. **leftover LoanWrite + `LoanBuilder`** → progressive-loan-test. **leftover helper-bound calendar/schedule tests** (`CalendarUtilsTest`, `DefaultScheduledDateGeneratorTest` + junit tenant/timezone helpers) → loan-test. **leftover MultiException cucumber** → core. **leftover command cucumber** (handler provider/exception + service retry fallback + `ValidCommandHandler`) → core (mock `ApplicationContext` / constructed `SynchronousCommandProcessingService`; junit-platform cucumber suite). **leftover `ApiVerificationTest`** thinned off leftover `TestConfiguration`/`AbstractSpringTest` (ClassGraph scan of `@Path` types on the composition-root classpath). Leftover Java residual is the Spring Boot composition root only: 3 boot classes (`ServerApplication`, `FineractWebApplicationConfiguration`, `FineractLiquibaseOnlyApplicationConfiguration`) + composition-root `ApiVerificationTest`. Leftover provider resources (tenant Liquibase master chain, Spring Boot properties, sample SQL) stay on the composition root; they are not a cycle-safe api/impl/test peel. |
| **Do not start** | full provider / whole-core api/impl |

---

## 15.7 Migration playbook (single module)

1. **Inventory** public types used by other modules; move or create ports in `moduleapi`.  
2. **Create** `-api` project; move interfaces + stable DTOs only.  
3. **Rename/split** remaining code into `-impl`; keep Spring config there.  
4. **Wire** Gradle: other domain modules depend on `-api` only; provider may runtime-include `-impl`.  
5. **Register** OSGi service(s) for each public port (DS or bridge).  
6. **Create** `-test` with `Fragment-Host` = impl; migrate unit tests.  
7. **Switch consumers** from direct types / Spring of foreign impl to port + service lookup (or bridge).  
8. **Shrink** ArchUnit freeze entries for that module.  
9. **Document** Bundle-SymbolicName and exported packages in module README if present.

---

## 15.8 Naming conventions (recommended)

| Item | Convention |
|------|------------|
| Gradle project | `fineract-<name>-api` / `-impl` / `-test` |
| Bundle-SymbolicName | `org.apache.fineract.<name>.api` / `.impl` / `.test` |
| Export packages | `org.apache.fineract…moduleapi` (+ explicit contract packages) |
| Service interface | Prefer the Module API port type itself |

Exact symbolic names may be refined in B1 tooling; keep them stable once published.

---

## 15.9 Relation to existing architecture

| Artifact | Relation |
|----------|----------|
| [ADR-002](decisions/ADR-002-osgi-equinox-fuer-laufzeitmodularitaet.md) | Equinox + optional services |
| [ADR-003](decisions/ADR-003-spring-boot-gradle-module-als-kern-beibehalten.md) | Spring stays in core / impl |
| [ADR-017](decisions/ADR-017-hexagonale-architektur.md) | OSGi = pluggable adapters; stage E4 |
| [ADR-021](decisions/ADR-021-modul-kommunikation-nur-ueber-module-api.md) | Module API = export surface |
| [ADR-022](decisions/ADR-022-osgi-api-impl-test-bundles-services.md) | This refactor’s decision |
| [06.7 OSGi](06_crosscutting_concepts.md) | Runtime mechanism |
| [04.4 Bundle lifecycle](04_runtime_view.md) | Start/stop/bind sequences |
| [`osgi/`](../../osgi/README.md) | Equinox scaffold |

---

## 15.10 Checklist (PR / module review)

- [ ] No new dependency on foreign `-impl` from a domain module  
- [ ] Public types live in `-api` / `moduleapi`  
- [ ] Impl registers OSGi service under api interface (or bridge does)  
- [ ] `-api` has no Spring / JPA / REST types  
- [ ] Tests: Fragment-Host on impl **or** pure api contract tests  
- [ ] Optional service missing → documented degradation path  
- [ ] No Karaf Feature descriptor introduced as module contract  
- [ ] ArchUnit freeze only shrinks or stays equal (no new frozen internal imports)

---

*Navigation:* [README](README.md) · [ADR-022](decisions/ADR-022-osgi-api-impl-test-bundles-services.md) · [14 Module API](14_module_api_boundaries.md) · [command pilot](15_osgi_bundle_refactoring_fineract-command.md) · [charge plan](15_osgi_bundle_refactoring_fineract-charge.md)
