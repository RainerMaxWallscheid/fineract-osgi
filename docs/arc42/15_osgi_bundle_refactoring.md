...
| **B6** | Optional polish | DS-only for pure extensions; tighten exports | **Done** — Equinox-safe empty catalog ports and the command dispatcher graph register via `Service-Component` + staged Felix SCR. Impl `Export-Package` is empty. Loan / progressive-loan / WC-loan / COB / security keep Bundle-Activators. No-port modules have no Equinox activator. Boot registrars stay Spring |
