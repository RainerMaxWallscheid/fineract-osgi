...
Exact counts: **918** frozen violation lines in **13** non-empty files (19 empty = rule green). Remaining lines are leftover JPA / entity graphs (Loan.client, investor journals, accounting processors) plus leftover service types that share constructors with those graphs (`GSIMReadPlatformService`, impl-only writes). Enum / JSON-param leftovers and already-on-*-api ports that lived in INTERNAL packages are on `moduleapi`. WC product reads use `ProductToGLAccountMappingReadPlatformService` instead of the leftover advanced-mapping helper.
