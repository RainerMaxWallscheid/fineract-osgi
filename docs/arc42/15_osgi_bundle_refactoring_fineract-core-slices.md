...
| `fineract-instancemode` | **complete** (api/impl/test); Equinox no port (swagger DTO + servlet filter only); test-profile REST + swagger DTO + `FineractInstanceModeApiFilter` on api; constants residual in core |
