...
| client residual (pure write) | `ClientWritePlatformService` / charge / identifier / transaction write ports + command handlers in core; family (entity) + address handlers stay on provider |
| share product residual (pure) | write port, dropdown read, exceptions, command handlers in core; entity write/read stay on provider |
