...
| client residual (pure write/read/REST) | family/identifiers/transactions REST + search v2 + ClientChargeData + validators + internal client REST + pure support types + JDBC txn read in core; address REST/handlers/read in address-impl; main clients/charges REST + write/read impls stay on provider |
| group residual (pure) | exceptions, GroupTypes/enumerations, level/role repos+wrappers, write ports, level/roles JDBC reads, validators, GroupsLevelApiResource, handlers (incl. collection-sheet) + AllGroupTypesDataMapper in core; Centers/Groups REST + entity write/read stay on provider |
...
| postdated-check residual (pure) | data/status/exceptions/ports/handlers/REST in core; entity/assembler/impl stay with loan residual |
