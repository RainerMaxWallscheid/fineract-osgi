...
| `fineract-bulkimport` | **complete** (api/impl/test); Equinox no port (jersey / jakarta.ws.rs / POI); workbook ports + `GlobalEntityType`/`ImportData`/`LookupMode` on bulkimport-api; `LookupModeTest` on bulkimport-test; populator impl residual provider (share) |
