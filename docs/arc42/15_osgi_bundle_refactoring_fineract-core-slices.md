...
| **12** | **Payment detail write** | ~8 | `fineract-paymentdetail` api/impl/test | **Done** — write port/impl/assembler; Equinox DS `OSGI-INF/paymentdetail.xml` (`PaymentDetailWritePlatformService`); write port Object-typed leftover persistable; entity + PaymentDetailData residual core |
...
| **27** | **Calendar leftover** | ~2 | `fineract-calendar` api | **Done** — `CalendarRequest` + `CalendarInstanceLookupPort` on calendar-api; Equinox DS `CalendarDropdownReadPlatformService`; loan-impl api-only; entity/`CalendarData` residual core |
...
12. **Payment detail write** ✅ (`fineract-paymentdetail` api/impl/test; Equinox DS `OSGI-INF/paymentdetail.xml` (`PaymentDetailWritePlatformService`); write port Object-typed leftover persistable; entity residual core).  
...
27. **Calendar leftover** ✅ (`CalendarRequest` + `CalendarInstanceLookupPort` → calendar-api; Equinox DS `CalendarDropdownReadPlatformService`).  
...
| `fineract-bulkimport` | **complete** (api/impl/test); Equinox DS `OSGI-INF/bulkimport-*.xml` (`BulkImportWorkbookService`, `BulkImportWorkbookPopulatorService`); jersey/JAX-RS leftover Object-typed; POI `WorkbookPopulator`/`ImportHandler` unpublished; `LookupModeTest` on bulkimport-test; populator impl residual provider (share) |
| `fineract-instancemode` | **complete** (api/impl/test); Equinox DS `OSGI-INF/instancemode.xml` (`InstanceModePort`); test-profile REST + swagger DTO + `FineractInstanceModeApiFilter` on api; constants residual in core |
...
| `fineract-s3` | **complete** (api/impl/test); Equinox no port (AWS SDK `S3ClientBuilder`); S3 client SPI + Amazon/Localstack config; report export bean via dataqueries-impl |
| `fineract-openapi` | **complete** (api/impl/test); Equinox no port (swagger reader/filter utilities only); OperationId reader + spec filter for swagger-gradle-plugin; tests on fragment |
...
| `fineract-event` | **complete** (api/impl/test) + external jobs/config API peel; Equinox DS `OSGI-INF/event-*.xml` (`PortfolioNotificationEventPort`, `SmsCampaignTriggerEventPort`); leftover `ExternalEventProducer` in core only; domain events + producers/serializers + config REST/jobs; core residual: notifier + outbox entity/service + serializer SPI |
| `fineract-interoperation` | **complete** (api/impl/test); Equinox DS `OSGI-INF/interoperation.xml` (`InteropService`); leftover savings catalog enums Object-typed; `InteropIdentifier` entity residual savings-impl |
...
| `fineract-paymentdetail` | **complete** (api/impl/test); Equinox DS `OSGI-INF/paymentdetail.xml` (`PaymentDetailWritePlatformService`); write port Object-typed leftover persistable; PaymentDetail entity/repo + PaymentDetailData residual core |
...
| `fineract-calendar` leftover close-in | **complete**; Equinox DS `CalendarDropdownReadPlatformService`; entity/`CalendarData` residual core |
