...
| **B5** | Roll out further domain modules — **done** (ranked waves + leftover peels 1–30 closed; `./gradlew checkArchUnitFreeze` holds the leftover freeze-store budget). Freeze shrink **started** (555 leftover JPA/entity lines; leftover catalog enums closed onto `moduleapi`; WC journals use `WorkingCapitalLoanJournalPort`; savings journals use `SavingsJournalPort`; loan-transaction journals use `LoanJournalPort`; investor transfer status checks use `LoanDataForExternalTransfer.getLoanStatusName()`; transfer journals use `ExternalOwnerTransferJournalPort`; transfer data via `LoanDataForExternalTransferPort`; product existence via `LoanProductExistencePort`; sale deferred income via `LoanSaleDeferredIncomePort`; outstanding interest via `LoanOutstandingInterestPort`; transferability via `LoanTransferBalancePort`; transfer snapshots via `LoanTransferSnapshotPort`; ownership event data via `LoanOwnershipEventDataPort`; COB product id via `LoanTransferBalancePort`; no core peel). Do not peel `fineract-provider` or remaining `fineract-core` kernel types |
