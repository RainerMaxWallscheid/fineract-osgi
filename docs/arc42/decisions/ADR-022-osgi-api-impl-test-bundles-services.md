...
| **B3** | Spring↔OSGi bridge in composition root — **in progress** (empty-catalog activators catalog-wide; every Equinox-safe PILOT_PORT hosted via `osgi/CompositionRootOsgiBridge`; `ContentStreamPort` empty-catalog only; per-module `*OsgiServiceRegistrar` remains the Boot classpath path) |
