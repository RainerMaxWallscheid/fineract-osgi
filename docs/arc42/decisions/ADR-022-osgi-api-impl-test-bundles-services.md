...
| **B3** | Spring↔OSGi bridge in composition root — **in progress** (empty-catalog activators catalog-wide; every Equinox-safe PILOT_PORT hosted via `osgi/CompositionRootOsgiBridge`; optional Boot embed `fineract.osgi.enabled` registers Wave-1 Spring ports; `ContentStreamPort` empty-catalog only; per-module `*OsgiServiceRegistrar` remains the Boot classpath path) |
