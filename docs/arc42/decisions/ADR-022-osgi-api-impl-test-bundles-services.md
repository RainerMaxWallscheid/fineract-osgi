...
| **B6** | Optional: DS-only for pure logic extension bundles (not for loan/COB/security) — **in progress** (`fineract-calendar-impl` empty `CalendarDropdownReadPlatformService` via `Service-Component` + staged Felix SCR; Boot registrar stays Spring) |
