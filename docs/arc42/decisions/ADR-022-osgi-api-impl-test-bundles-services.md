...
| **B5** | Roll out further domain modules — **done** (ranked waves + leftover peels 1–30 closed; `./gradlew checkArchUnitFreeze` holds the leftover freeze-store budget). Freeze shrink **started** (1071 leftover JPA/entity lines; enum / JSON-param leftovers closed; no core peel). Do not peel `fineract-provider` or remaining `fineract-core` kernel types |
