...
| **B5** | Roll out further domain modules — **done** (ranked waves + leftover peels 1–30 closed; `./gradlew checkArchUnitFreeze` holds the leftover freeze-store budget). Freeze shrink **started** (684 leftover JPA/entity lines; leftover catalog enums closed onto `moduleapi`; WC journals use `WorkingCapitalLoanJournalPort`; savings journals use `SavingsJournalPort`; loan-transaction journals use `LoanJournalPort`; no core peel). Do not peel `fineract-provider` or remaining `fineract-core` kernel types |
