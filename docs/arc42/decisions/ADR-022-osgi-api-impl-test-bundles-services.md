...
| **B3** | Spring↔OSGi bridge in composition root — **in progress** (empty-catalog activators catalog-wide; hosted Wave-1 catalogs + Wave-2 `ContentStoreService` + `CashierTxnValidationPort` + `LoanOriginatorReadPlatformService` via `osgi/CompositionRootOsgiBridge`; per-module `*OsgiServiceRegistrar` remains the Boot classpath path) |
