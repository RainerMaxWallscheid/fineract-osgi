...
| 6 Spring↔OSGi bridge | **done** | `ChargeOsgiServiceRegistrar` (Spring path) + DS `OSGI-INF/charge.xml` (Equinox start; empty catalog, lowest ranking). Composition-root hosted port: `osgi/CompositionRootOsgiBridge` |
...
3. [x] `./gradlew :fineract-charge-test:test` green — **16 tests** (after Step 6 registrar tests)  
...
6. [x] Equinox start: DS `OSGI-INF/charge.xml` + `OsgiChargeDefinitionPort` (empty catalog, no Spring/JPA)  
...
#### Done (slice 2 — residual progress)

8. [x] **LoanCharge* exceptions** moved `charge-impl` → `loanaccount.exception` (provider/loan/WC imports updated)  
9. [x] **ChargeEnumerations** moved to **charge-api** (`…charge.service`)  
10. [x] **ChargeReadPlatformService** interface moved to **charge-api** (impl stays charge-impl)  
11. [x] **ChargeCalculationTypeConverter / ChargePaymentModeConverter** on charge-api (JPA attribute converters for pure enums)  
12. [x] **LoanChargeApiJsonValidator** uses `ChargeDefinitionPort` for new catalog charge attach (not `ChargeRepositoryWrapper`)  
13. [x] **WC** uses `WorkingCapitalLoanChargeNotFoundException` instead of loan exception  

#### Done (slice 3 — residual next)
...
#### Done (slice 5 — LoanProduct charges)

19. [x] **Loan:** `LoanProduct` stores `List<Long> chargeIds` (`@ElementCollection` on `m_product_loan_charge`); no `@ManyToMany Charge`  
20. [x] **Provider:** product write assembles charge ids; overdue penalty resolved via ids / product charge read  

#### Done (slice 6 — loan create + savings product)

21. [x] **Loan:** `LoanChargeService.create` takes `ChargeDefinitionData`; **Gradle charge-api only** (dropped charge-impl)  
...
#### Done (slice 8 — savings charge-api only)

27. [x] **Savings:** product/account/deposit charge assemblers use `ChargeDefinitionPort` + `ChargeAppliesTo` (no `ChargeRepositoryWrapper`)  
28. [x] **Gradle:** `:fineract-savings` depends on **charge-api only** (dropped charge-impl)  
29. [x] **SavingsAccount.addCharge** takes `ChargeDefinitionData`  

#### Done (slice 9 — client / share account charges)

30. [x] **ClientCharge** stores `chargeId` + name/currency/incomeAccount snapshots; create via `ChargeDefinitionData`  
...
- Leftover share-product serializer/read/write/config closed into charge-impl.
- Leftover client charge/transaction entities, repos, and write impls closed into charge-impl.
