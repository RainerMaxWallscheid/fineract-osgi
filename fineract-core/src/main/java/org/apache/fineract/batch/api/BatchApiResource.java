...
public class BatchApiResource {
    ...
    public List<BatchResponse> handleBatchRequests(@DefaultValue("false") @QueryParam("enclosingTransaction") @Parameter(description = "enclosingTransaction", required = false) final boolean enclosingTransaction, @Parameter(hidden = true) List<BatchRequest> requestList, @Context UriInfo uriInfo) {
        ...
        return enclosingTransaction ? service.handleBatchRequestsWithEnclosingTransaction(requestList, uriInfo) : service.handleBatchRequestsWithoutEnclosingTransaction(requestList, uriInfo);
    }
    ...
    private void validateRequestMethodsAllowedOnInstanceType(final List<BatchRequest> requestList) {
        ...
        if (fineractProperties.getMode().isReadOnlyMode()) {
            final Optional<BatchRequest> nonGetRequest = requestList.stream().filter(batchRequest -> !HttpMethod.GET.equals(batchRequest.getMethod())).findFirst();
    ...
    @java.lang.SuppressWarnings("all")
        public BatchApiResource(final PlatformSecurityContext context, final BatchApiService service, final FineractProperties fineractProperties) {
