...
public class TenantDatabaseStateVerifier {
    ...
    public boolean isTenantStoreOnLatestUpgradableVersion(DataSource dataSource) {
        return isOnLatestFlywayVersion(TENANT_STORE_LATEST_FLYWAY_VERSION, TENANT_STORE_LATEST_FLYWAY_SCRIPT_NAME, TENANT_STORE_LATEST_FLYWAY_SCRIPT_CHECKSUM, dataSource);
    }

    public boolean isTenantOnLatestUpgradableVersion(DataSource dataSource) {
        return isOnLatestFlywayVersion(TENANT_LATEST_FLYWAY_VERSION, TENANT_LATEST_FLYWAY_SCRIPT_NAME, TENANT_LATEST_FLYWAY_SCRIPT_CHECKSUM, dataSource);
    }

    private boolean isOnLatestFlywayVersion(int version, String scriptName, int checksum, DataSource dataSource) {
        Map<String, Object> paramMap = Map.of("latestFlywayVersion", version, "latestFlywayScriptName", scriptName, "latestFlywayScriptChecksum", checksum);
        NamedParameterJdbcTemplate jdbcTemplate = new NamedParameterJdbcTemplate(dataSource);
        Integer result = jdbcTemplate.queryForObject("SELECT COUNT(script) FROM schema_version WHERE version = :latestFlywayVersion " + "AND script = :latestFlywayScriptName AND checksum = :latestFlywayScriptChecksum AND success = 1", paramMap, Integer.class);
        return Objects.equals(result, 1);
    }

    @java.lang.SuppressWarnings("all")
        public TenantDatabaseStateVerifier(final LiquibaseProperties liquibaseProperties, final DatabaseIndependentQueryService dbQueryService, final DatabaseTypeResolver databaseTypeResolver) {
