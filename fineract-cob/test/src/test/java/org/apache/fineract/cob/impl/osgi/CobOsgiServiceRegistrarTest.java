...
class CobOsgiServiceRegistrarTest {

    @Test
    void afterPropertiesSet_withoutOsgi_doesNotThrow() {
        @SuppressWarnings("unchecked")
        final ObjectProvider<ConfigJobParameterService> jobParam = mock(ObjectProvider.class);
        when(jobParam.getIfAvailable()).thenReturn(null);

        final CobOsgiServiceRegistrar registrar = new CobOsgiServiceRegistrar(jobParam);
