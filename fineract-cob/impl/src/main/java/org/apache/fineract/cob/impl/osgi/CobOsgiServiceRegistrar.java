...
/**
 * Spring ↔ OSGi bridge for pure COB ports on cob-api (ADR-022).
 *
 * <p>Entity-typed step runners remain composition-root residual on impl.
 */
@Component
public class CobOsgiServiceRegistrar implements InitializingBean, DisposableBean {

    private static final Logger LOG = LoggerFactory.getLogger(CobOsgiServiceRegistrar.class);

    private final ObjectProvider<ConfigJobParameterService> jobParameterService;
    private final List<Object> registrations = new ArrayList<>();

    public CobOsgiServiceRegistrar(final ObjectProvider<ConfigJobParameterService> jobParameterService) {
    ...
    public void afterPropertiesSet() {
        try {
            ...
            final Object bundle = getBundle.invoke(null, CobOsgiServiceRegistrar.class);
            if (bundle == null) {
                LOG.debug("OSGi Bundle not available; skipping COB service registration");
                return;
            }
            ...
            if (context == null) {
                LOG.debug("OSGi BundleContext is null; skipping COB service registration");
                return;
            }
            register(context, ConfigJobParameterService.class, jobParameterService.getIfAvailable());
            LOG.info("Registered {} COB OSGi service(s)", registrations.size());
        } catch (final ClassNotFoundException ex) {
            LOG.debug("OSGi framework classes not present; Spring-only COB wiring");
        } catch (final ReflectiveOperationException ex) {
            LOG.warn("Failed to register COB OSGi services: {}", ex.toString());
        }
    }
