...
/** Spring ↔ OSGi bridge for client office/group transfer ports. */
@Component
public class TransferOsgiServiceRegistrar implements InitializingBean, DisposableBean {

    private static final Logger LOG = LoggerFactory.getLogger(TransferOsgiServiceRegistrar.class);

    private final ObjectProvider<TransferWritePlatformService> write;
    private final List<Object> registrations = new ArrayList<>();

    public TransferOsgiServiceRegistrar(final ObjectProvider<TransferWritePlatformService> write) {
        this.write = write;
    }
    ...
    public void afterPropertiesSet() {
        try {
            ...
            final Object bundle = getBundle.invoke(null, TransferOsgiServiceRegistrar.class);
            if (bundle == null) {
                return;
            }
            ...
            register(context, TransferWritePlatformService.class, write.getIfAvailable());
            LOG.info("Registered {} transfer OSGi service(s)", registrations.size());
        } catch (final ClassNotFoundException ex) {
            LOG.debug("OSGi framework classes not present; Spring-only transfer wiring");
        } catch (final ReflectiveOperationException ex) {
            LOG.warn("Failed to register transfer OSGi services: {}", ex.toString());
        }
    }
