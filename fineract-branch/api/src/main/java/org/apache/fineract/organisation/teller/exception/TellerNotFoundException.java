...
public class TellerNotFoundException extends AbstractPlatformResourceNotFoundException {
    ...
    /**
     * @param cause
     *            underlying cause (e.g. empty JDBC result); typed as {@link Exception} so api stays free of Spring DAO
     */
    public TellerNotFoundException(Long id, Exception cause) {
        super("error.msg.teller.id.invalid", "Teller with identifier " + id + " does not exist", id, cause);
    }
}
