...
class PostDatedChecksOsgiServiceRegistrarTest {

    @Test
    @SuppressWarnings("unchecked")
    void afterPropertiesSet_withoutOsgi_doesNotThrow() {
        ObjectProvider<RepaymentWithPostDatedChecksReadPlatformService> r = mock(ObjectProvider.class);
        ObjectProvider<RepaymentWithPostDatedChecksWritePlatformService> w = mock(ObjectProvider.class);
        when(r.getIfAvailable()).thenReturn(null);
        when(w.getIfAvailable()).thenReturn(null);
        var reg = new PostDatedChecksOsgiServiceRegistrar(r, w);
