...
class WorkingCapitalLoanOsgiServiceRegistrarTest {

    @Test
    void afterPropertiesSet_withoutOsgi_doesNotThrow() {
        @SuppressWarnings("unchecked")
        final ObjectProvider<WorkingCapitalLoanProductWritePlatformService> productWrite = mock(ObjectProvider.class);
        @SuppressWarnings("unchecked")
        final ObjectProvider<WorkingCapitalLoanApplicationWritePlatformService> applicationWrite = mock(ObjectProvider.class);
        @SuppressWarnings("unchecked")
        final ObjectProvider<WorkingCapitalLoanWritePlatformService> loanWrite = mock(ObjectProvider.class);
        when(productWrite.getIfAvailable()).thenReturn(null);
        when(applicationWrite.getIfAvailable()).thenReturn(null);
        when(loanWrite.getIfAvailable()).thenReturn(null);

        final WorkingCapitalLoanOsgiServiceRegistrar registrar = new WorkingCapitalLoanOsgiServiceRegistrar(productWrite, applicationWrite,
                loanWrite);
