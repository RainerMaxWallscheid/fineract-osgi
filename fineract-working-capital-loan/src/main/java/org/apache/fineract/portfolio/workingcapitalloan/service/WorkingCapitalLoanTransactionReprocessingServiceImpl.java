...
public class WorkingCapitalLoanTransactionReprocessingServiceImpl implements WorkingCapitalLoanTransactionReprocessingService {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(WorkingCapitalLoanTransactionReprocessingServiceImpl.class);
    ...
    private static final Comparator<WorkingCapitalLoanTransaction> TRANSACTION_ORDER = Comparator.comparing(WorkingCapitalLoanTransaction::getTransactionDate).thenComparing(WorkingCapitalLoanTransaction::getSubmittedOnDate, Comparator.nullsLast(Comparator.naturalOrder())).thenComparing(WorkingCapitalLoanTransaction::getId, Comparator.nullsLast(Comparator.naturalOrder()));
    ...
    public void reprocessTransactions(final WorkingCapitalLoan loan) {
        final List<WorkingCapitalLoanTransaction> allTransactions = transactionRepository.findByWcLoan_IdOrderByTransactionDateAscIdAsc(loan.getId());
    ...
    private void reprocessTransactions(final WorkingCapitalLoan loan, final List<WorkingCapitalLoanTransaction> allTransactions, final boolean forceEvenWithoutCharges) {
        ...
        if (charges.isEmpty() && !forceEvenWithoutCharges && !isOverpaidByReplayableTransactions(allTransactions, balance)) {
            log.debug("Skipping transaction reprocessing for WC loan {}: no active charges and not overpaid, allocations are order-independent", loan.getId());
            return;
        }
        ...
        final List<WorkingCapitalLoanTransaction> replayable = allTransactions.stream().filter(txn -> !txn.isReversed() && txn.getTransactionType().isRepaymentType()).sorted(TRANSACTION_ORDER).toList();
        ...
        final Map<Long, WorkingCapitalLoanTransactionAllocation> allocationsByTxnId = replayable.isEmpty() ? Map.of() : allocationRepository.findByWcLoanTransactionIdIn(replayable.stream().map(WorkingCapitalLoanTransaction::getId).toList()).stream().collect(Collectors.toMap(allocation -> allocation.getWcLoanTransaction().getId(), Function.identity()));
        ...
        for (final WorkingCapitalLoanTransaction txn : replayable) {
            ...
            final WorkingCapitalLoanAllocationRequest request = allocationRequestFactory.build(loan, balance, charges, txn.getTransactionDate(), txn.getTransactionAmount(), txn.getTypeOf());
        ...
        final BigDecimal creditBalanceRefundTotal = allTransactions.stream().filter(txn -> !txn.isReversed() && txn.getTypeOf() == LoanTransactionType.CREDIT_BALANCE_REFUND).map(WorkingCapitalLoanTransaction::getTransactionAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    private boolean isOverpaidByReplayableTransactions(final List<WorkingCapitalLoanTransaction> allTransactions, final WorkingCapitalLoanBalance balance) {
        final BigDecimal replayableTotal = allTransactions.stream().filter(txn -> !txn.isReversed() && txn.getTransactionType().isRepaymentType()).map(WorkingCapitalLoanTransaction::getTransactionAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
        return replayableTotal.compareTo(balance.getPrincipal()) > 0;
    }

    @java.lang.SuppressWarnings("all")
        public WorkingCapitalLoanTransactionReprocessingServiceImpl(final WorkingCapitalLoanTransactionRepository transactionRepository, final WorkingCapitalLoanChargeRepository chargeRepository, final WorkingCapitalLoanBalanceRepository balanceRepository, final WorkingCapitalLoanTransactionAllocationRepository allocationRepository, final WorkingCapitalLoanPaymentAllocationProcessor allocationProcessor, final WorkingCapitalLoanAllocationRequestFactory allocationRequestFactory, final WorkingCapitalLoanAllocationApplier allocationApplier, final WorkingCapitalLoanBalanceUpdater balanceUpdater, final WorkingCapitalLoanAmortizationScheduleWriteService amortizationScheduleWriteService) {
