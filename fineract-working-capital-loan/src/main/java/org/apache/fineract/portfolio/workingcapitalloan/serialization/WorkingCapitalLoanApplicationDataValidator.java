...
public class WorkingCapitalLoanApplicationDataValidator {
    ...
    public void validateForCreate(final JsonCommand command) {
        ...
        final DataValidatorBuilder baseDataValidator = new DataValidatorBuilder(dataValidationErrors).resource(WorkingCapitalLoanConstants.WCL_RESOURCE_NAME);
        ...
        if (productId != null) {
            product = this.productRepository.findById(productId).orElseThrow(() -> new WorkingCapitalLoanProductNotFoundException(productId));
        }
        // Mandatory: principal
        final BigDecimal principal = this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.principalAmountParamName, element) ? this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed(WorkingCapitalLoanConstants.principalAmountParamName, element) : null;
        baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.principalAmountParamName).value(principal).notNull().positiveAmount();
        ...
        baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.periodPaymentRateParamName).value(periodPaymentRate).notNull().zeroOrPositiveAmount();
        // Mandatory: totalPaymentVolume
        final BigDecimal totalPaymentVolume = this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.totalPaymentVolumeParamName, element) ? this.fromApiJsonHelper.extractBigDecimalNamed(WorkingCapitalLoanConstants.totalPaymentVolumeParamName, element, new HashSet<>()) : null;
        baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.totalPaymentVolumeParamName).value(totalPaymentVolume).notNull().zeroOrPositiveAmount();
        // Optional: discount
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.submittedOnDateParameterName, element)) {
            final Object submittedOnDate = this.fromApiJsonHelper.extractLocalDateNamed(WorkingCapitalLoanConstants.submittedOnDateParameterName, element);
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.submittedOnNoteParameterName, element)) {
            final String submittedOnNote = this.fromApiJsonHelper.extractStringNamed(WorkingCapitalLoanConstants.submittedOnNoteParameterName, element);
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.submittedOnNoteParameterName).value(submittedOnNote).ignoreIfNull().notExceedingLengthOf(500);
        }
        // Mandatory: expectedDisbursementDate
        final LocalDate expectedDisbursementDate = this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.expectedDisbursementDateParameterName, element) ? this.fromApiJsonHelper.extractLocalDateNamed(WorkingCapitalLoanConstants.expectedDisbursementDateParameterName, element) : null;
        baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.expectedDisbursementDateParameterName).value(expectedDisbursementDate).notNull();
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.delinquencyGraceDaysParamName, element)) {
            final Integer delinquencyGraceDays = this.fromApiJsonHelper.extractIntegerWithLocaleNamed(WorkingCapitalLoanProductConstants.delinquencyGraceDaysParamName, element);
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.breachGraceDaysParamName, element)) {
            final Integer breachGraceDays = this.fromApiJsonHelper.extractIntegerWithLocaleNamed(WorkingCapitalLoanProductConstants.breachGraceDaysParamName, element);
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.delinquencyStartTypeParamName, element)) {
            ...
            baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.delinquencyStartTypeParamName).value(delinquencyStartTypeValue).ignoreIfNull().notBlank();
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.accountNoParameterName, element)) {
            ...
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.accountNoParameterName).value(accountNo).ignoreIfNull().notExceedingLengthOf(20);
        }
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.externalIdParameterName, element)) {
            ...
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.externalIdParameterName).value(externalIdStr).ignoreIfNull().notExceedingLengthOf(100);
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.fundIdParameterName, element)) {
            ...
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.fundIdParameterName).value(fundId).ignoreIfNull().integerGreaterThanZero();
        }
    ...
    public void validateForModify(final WorkingCapitalLoan loan) {
        ...
        if (expectedDisbursementDate != null && DateUtils.isAfter(submittedOnDate, expectedDisbursementDate)) {
            throw new WorkingCapitalLoanApplicationDateException("submitted.on.date.cannot.be.after.expected.disbursement.date", "submittedOnDate cannot be after expectedDisbursementDate.", submittedOnDate, expectedDisbursementDate);
        }
    }

    private void validateForUpdate(final JsonCommand command, final Long existingProductId, final Long existingClientId, final String existingExternalId, final String existingAccountNo, final LocalDate existingSubmittedOnDate, final LocalDate existingExpectedDisbursementDate) {
        final String json = command.json();
        if (StringUtils.isBlank(json)) {
            throw new InvalidJsonException();
        }
        ...
        final Long productIdFromRequest = this.fromApiJsonHelper.extractLongNamed(WorkingCapitalLoanConstants.productIdParameterName, element);
        ...
        if (productIdFromRequest != null) {
            ...
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.productIdParameterName).value(productIdFromRequest).notNull().longGreaterThanZero();
        }
        WorkingCapitalLoanProduct product = null;
        if (resolvedProductId != null) {
            product = this.productRepository.findById(resolvedProductId).orElseThrow(() -> new WorkingCapitalLoanProductNotFoundException(resolvedProductId));
        }
        final Long clientIdFromRequest = this.fromApiJsonHelper.extractLongNamed(WorkingCapitalLoanConstants.clientIdParameterName, element);
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.principalAmountParamName, element)) {
            atLeastOneParameterPassedForUpdate = true;
            final BigDecimal principal = this.fromApiJsonHelper.extractBigDecimalWithLocaleNamed(WorkingCapitalLoanConstants.principalAmountParamName, element);
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.principalAmountParamName).value(principal).notNull().positiveAmount();
            validatePrincipalMinMax(principal, product, baseDataValidator);
        }
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.periodPaymentRateParamName, element)) {
            atLeastOneParameterPassedForUpdate = true;
            final BigDecimal periodPaymentRate = this.fromApiJsonHelper.extractBigDecimalNamed(WorkingCapitalLoanProductConstants.periodPaymentRateParamName, element, new HashSet<>());
            baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.periodPaymentRateParamName).value(periodPaymentRate).notNull().zeroOrPositiveAmount();
            validatePeriodPaymentRateMinMax(periodPaymentRate, product, baseDataValidator);
        }
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.totalPaymentVolumeParamName, element)) {
            atLeastOneParameterPassedForUpdate = true;
            final BigDecimal totalPaymentVolume = this.fromApiJsonHelper.extractBigDecimalNamed(WorkingCapitalLoanConstants.totalPaymentVolumeParamName, element, new HashSet<>());
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.totalPaymentVolumeParamName).value(totalPaymentVolume).notNull().zeroOrPositiveAmount();
        }
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.discountParamName, element)) {
            atLeastOneParameterPassedForUpdate = true;
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.submittedOnDateParameterName, element)) {
            atLeastOneParameterPassedForUpdate = true;
            final LocalDate submittedOnDate = this.fromApiJsonHelper.extractLocalDateNamed(WorkingCapitalLoanConstants.submittedOnDateParameterName, element);
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.submittedOnDateParameterName).value(submittedOnDate).notNull();
        }
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.submittedOnNoteParameterName, element)) {
            atLeastOneParameterPassedForUpdate = true;
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.expectedDisbursementDateParameterName, element)) {
            atLeastOneParameterPassedForUpdate = true;
            expectedDisbursementDateFromRequest = this.fromApiJsonHelper.extractLocalDateNamed(WorkingCapitalLoanConstants.expectedDisbursementDateParameterName, element);
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.expectedDisbursementDateParameterName).value(expectedDisbursementDateFromRequest).notNull();
        }
        final LocalDate resolvedExpectedDisbursementDate = expectedDisbursementDateFromRequest != null ? expectedDisbursementDateFromRequest : existingExpectedDisbursementDate;
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanConstants.accountNoParameterName, element)) {
            atLeastOneParameterPassedForUpdate = true;
            final String accountNo = this.fromApiJsonHelper.extractStringNamed(WorkingCapitalLoanConstants.accountNoParameterName, element);
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.accountNoParameterName).value(accountNo).notBlank().notExceedingLengthOf(20);
        ...
        if (product != null && product.getConfigurableAttributes() != null && existsAnyOverridableParameter(element)) {
            atLeastOneParameterPassedForUpdate = true;
            validateOverridables(element, baseDataValidator, product.getConfigurableAttributes(), product.getBreach(), product.getNearBreach());
        }
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.delinquencyGraceDaysParamName, element)) {
            atLeastOneParameterPassedForUpdate = true;
            final Integer delinquencyGraceDays = this.fromApiJsonHelper.extractIntegerWithLocaleNamed(WorkingCapitalLoanProductConstants.delinquencyGraceDaysParamName, element);
    ...
    private void validateSubmittedOnDate(final JsonElement element, final WorkingCapitalLoanProduct product, final Client client, final LocalDate expectedDisbursementDate, final LocalDate existingSubmittedOnDate) {
        ...
        if (product != null) {
            ...
            if (DateUtils.isBefore(submittedOnDate, startDate)) {
                throw new LoanApplicationDateException("submitted.on.date.cannot.be.before.the.loan.product.start.date", "submittedOnDate cannot be before the loan product startDate.", submittedOnDate.toString(), startDate.toString());
            }
            final LocalDate closeDate = product.getCloseDate();
            if (closeDate != null && DateUtils.isAfter(submittedOnDate, closeDate)) {
                throw new LoanApplicationDateException("submitted.on.date.cannot.be.after.the.loan.product.close.date", "submittedOnDate cannot be after the loan product closeDate.", submittedOnDate.toString(), closeDate.toString());
            }
        }
        ...
        if (expectedDisbursementDate != null && DateUtils.isAfter(submittedOnDate, expectedDisbursementDate)) {
            throw new InvalidLoanStateTransitionException("submittal", "cannot.be.after.expected.disbursement.date", "The date on which a loan is submitted cannot be after its expected disbursement date: " + expectedDisbursementDate, submittedOnDate, expectedDisbursementDate);
        }
    }

    private boolean existsAnyOverridableParameter(final JsonElement element) {
        return Stream.of(WorkingCapitalLoanProductConstants.delinquencyBucketIdParamName, WorkingCapitalLoanProductConstants.repaymentEveryParamName, WorkingCapitalLoanProductConstants.repaymentFrequencyTypeParamName, WorkingCapitalLoanProductConstants.discountParamName, WorkingCapitalLoanProductConstants.breachIdParamName, WorkingCapitalLoanProductConstants.nearBreachIdParamName).anyMatch(param -> this.fromApiJsonHelper.parameterExists(param, element));
    }

    private void validateOverridables(final JsonElement element, final DataValidatorBuilder baseDataValidator, final WorkingCapitalLoanProductConfigurableAttributes config, final WorkingCapitalBreach currentBreach, final WorkingCapitalNearBreach currentNearBreach) {
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.delinquencyBucketIdParamName, element)) {
            if (config.isDelinquencyBucketClassification()) {
                final Long bucketId = this.fromApiJsonHelper.extractLongNamed(WorkingCapitalLoanProductConstants.delinquencyBucketIdParamName, element);
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.delinquencyBucketIdParamName).value(bucketId).ignoreIfNull().integerGreaterThanZero();
            } else {
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.delinquencyBucketIdParamName).failWithCode("override.not.allowed.by.product");
            }
        }
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.repaymentEveryParamName, element)) {
            if (config.isPeriodPaymentFrequency()) {
                final Integer repaymentEvery = this.fromApiJsonHelper.extractIntegerWithLocaleNamed(WorkingCapitalLoanProductConstants.repaymentEveryParamName, element);
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.repaymentEveryParamName).value(repaymentEvery).ignoreIfNull().integerGreaterThanZero();
            } else {
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.repaymentEveryParamName).failWithCode("override.not.allowed.by.product");
            }
        }
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.repaymentFrequencyTypeParamName, element)) {
            if (config.isPeriodPaymentFrequencyType()) {
                ...
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.repaymentFrequencyTypeParamName).value(repaymentFrequencyTypeValue).ignoreIfNull().notBlank();
                ...
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.repaymentFrequencyTypeParamName).failWithCode("override.not.allowed.by.product");
            }
        }
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.discountParamName, element)) {
            if (config.isDiscountDefaultOverridable()) {
                final BigDecimal discount = this.fromApiJsonHelper.extractBigDecimalNamed(WorkingCapitalLoanProductConstants.discountParamName, element, new java.util.HashSet<>());
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.discountParamName).value(discount).ignoreIfNull().zeroOrPositiveAmount();
            } else {
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.discountParamName).failWithCode("override.not.allowed.by.product");
            }
        }
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.breachIdParamName, element)) {
            if (config.isBreach()) {
                ...
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.breachIdParamName).failWithCode("override.not.allowed.by.product");
            }
        }
        ...
        if (this.fromApiJsonHelper.parameterExists(WorkingCapitalLoanProductConstants.nearBreachIdParamName, element)) {
            if (config.isBreach()) {
                ...
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.nearBreachIdParamName).value(nearBreachId).ignoreIfNull().longGreaterThanZero();
            } else {
                baseDataValidator.reset().parameter(WorkingCapitalLoanProductConstants.nearBreachIdParamName).failWithCode("override.not.allowed.by.product");
            }
        }
    ...
    private void validatePrincipalMinMax(final BigDecimal principal, final WorkingCapitalLoanProduct product, final DataValidatorBuilder baseDataValidator) {
        ...
        if (minPrincipal != null && MathUtil.isLessThan(principal, minPrincipal)) {
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.principalAmountParamName).failWithCode("must.be.greater.than.or.equal.to.min");
        }
        if (maxPrincipal != null && MathUtil.isGreaterThan(principal, maxPrincipal)) {
            baseDataValidator.reset().parameter(WorkingCapitalLoanConstants.principalAmountParamName).failWithCode("must.be.less.than.or.equal.to.max");
        }
    }

    private void validatePeriodPaymentRateMinMax(final BigDecimal periodPaymentRate, final WorkingCapitalLoanProduct product, final DataValidatorBuilder baseDataValidator) {
    ...
    private void validateAccountNumberUniqueness(final JsonElement element) {
        ...
        if (StringUtils.isNotBlank(accountNo) && this.workingCapitalLoanRepository.existsByAccountNumber(accountNo)) {
            throw new GeneralPlatformDomainRuleException("error.msg.loan.with.accountNo.already.used", "Loan with account number is already registered.");
        }
    }
    ...
    public void handleDataIntegrityIssues(final JsonCommand command, final Throwable realCause, final Exception dve) {
        if (realCause != null) {
            ...
            if ((message != null && message.contains("wc_loan_account_no_UNIQUE")) || (causeMessage != null && causeMessage.contains("wc_loan_account_no_UNIQUE"))) {
                final String accountNo = command.stringValueOfParameterNamed("accountNo");
                throw new PlatformDataIntegrityException("error.msg.wc.loan.duplicate.accountNo", "Working Capital Loan with accountNo `" + accountNo + "` already exists", "accountNo", accountNo);
            }
            if ((message != null && (message.contains("wc_loan_externalid_UNIQUE") || message.toLowerCase().contains("external_id_unique"))) || (causeMessage != null && (causeMessage.contains("wc_loan_externalid_UNIQUE") || causeMessage.toLowerCase().contains("external_id_unique")))) {
                final String externalId = command.stringValueOfParameterNamed("externalId");
                throw new PlatformDataIntegrityException("error.msg.wc.loan.duplicate.externalId", "Working Capital Loan with externalId `" + externalId + "` already exists", "externalId", externalId);
            }
        }
