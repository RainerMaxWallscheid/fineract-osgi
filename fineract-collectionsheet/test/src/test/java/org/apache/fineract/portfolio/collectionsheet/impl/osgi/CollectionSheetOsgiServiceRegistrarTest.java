...
class CollectionSheetOsgiServiceRegistrarTest {

    @Test
    @SuppressWarnings("unchecked")
    void afterPropertiesSet_withoutOsgi_doesNotThrow() {
        ObjectProvider<CollectionSheetReadPlatformService> r = mock(ObjectProvider.class);
        ObjectProvider<CollectionSheetWritePlatformService> w = mock(ObjectProvider.class);
        when(r.getIfAvailable()).thenReturn(null);
        when(w.getIfAvailable()).thenReturn(null);
        var reg = new CollectionSheetOsgiServiceRegistrar(r, w);
