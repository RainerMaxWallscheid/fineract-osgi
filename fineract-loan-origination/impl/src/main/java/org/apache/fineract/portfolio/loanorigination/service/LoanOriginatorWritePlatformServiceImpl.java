...
public class LoanOriginatorWritePlatformServiceImpl implements LoanOriginatorWritePlatformService {
    ...
    public CommandProcessingResult create(final JsonCommand command) {
        ...
        //
        //
        new CommandProcessingResultBuilder().withCommandId(command.commandId()).withEntityId(originator.getId()).withEntityExternalId(externalId).build();
    }
    ...
    public CommandProcessingResult update(final Long id, final JsonCommand command) {
        ...
        final LoanOriginator originator = this.loanOriginatorRepository.findById(id).orElseThrow(() -> new LoanOriginatorNotFoundException(id));
        ...
        //
        //
        new CommandProcessingResultBuilder().withCommandId(command.commandId()).withEntityId(originator.getId()).withEntityExternalId(originator.getExternalId()).with(changes).build();
    }
    ...
    public CommandProcessingResult delete(final Long id) {
        ...
        //
        //
        new CommandProcessingResultBuilder().withEntityId(id).withEntityExternalId(externalId).build();
    }
    ...
    public CommandProcessingResult attachOriginatorToLoan(final Long loanId, final Long originatorId) {
        if (!this.loanExistencePort.isSubmittedAndPendingApproval(loanId)) {
            throw new LoanNotInSubmittedStatusException(loanId, this.loanExistencePort.statusCode(loanId));
        }
    ...
    public CommandProcessingResult detachOriginatorFromLoan(final Long loanId, final Long originatorId) {
        ...
        final LoanOriginator originator = this.loanOriginatorRepository.findById(originatorId).orElseThrow(() -> new LoanOriginatorNotFoundException(originatorId));
        final LoanOriginatorMapping mapping = this.loanOriginatorMappingRepository.findByLoanIdAndOriginatorId(loanId, originatorId).orElseThrow(() -> new LoanOriginatorMappingNotFoundException(loanId, originatorId));
