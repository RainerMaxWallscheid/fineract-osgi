...
/**
 * Spring ↔ OSGi bridge for loan-origination ports (ADR-022 / loan-origination plan).
 *
 * <p>Uses reflection so this class loads under plain Spring Boot without OSGi on the classpath.
 */
@Component
public class LoanOriginationOsgiServiceRegistrar implements InitializingBean, DisposableBean {

    private static final Logger LOG = LoggerFactory.getLogger(LoanOriginationOsgiServiceRegistrar.class);

    private final ObjectProvider<LoanOriginatorReadPlatformService> read;
    private final ObjectProvider<LoanOriginatorWritePlatformService> write;
    private final ObjectProvider<WorkingCapitalLoanOriginatorWritePlatformService> wcWrite;
    private final List<Object> registrations = new ArrayList<>();

    public LoanOriginationOsgiServiceRegistrar(final ObjectProvider<LoanOriginatorReadPlatformService> read,
            final ObjectProvider<LoanOriginatorWritePlatformService> write,
            final ObjectProvider<WorkingCapitalLoanOriginatorWritePlatformService> wcWrite) {
        this.read = read;
        this.write = write;
        this.wcWrite = wcWrite;
    }
    ...
    public void afterPropertiesSet() {
        try {
            ...
            final Object bundle = getBundle.invoke(null, LoanOriginationOsgiServiceRegistrar.class);
            if (bundle == null) {
                LOG.debug("OSGi Bundle not available; skipping loan-origination service registration");
                return;
            }
            ...
            if (context == null) {
                LOG.debug("OSGi BundleContext is null; skipping loan-origination service registration");
                return;
            }

            register(context, LoanOriginatorReadPlatformService.class, read.getIfAvailable());
            register(context, LoanOriginatorWritePlatformService.class, write.getIfAvailable());
            register(context, WorkingCapitalLoanOriginatorWritePlatformService.class, wcWrite.getIfAvailable());
            LOG.info("Registered {} loan-origination OSGi service(s)", registrations.size());
        } catch (final ClassNotFoundException ex) {
            LOG.debug("OSGi framework classes not present; Spring-only loan-origination wiring");
        } catch (final ReflectiveOperationException ex) {
            LOG.warn("Failed to register loan-origination OSGi services: {}", ex.toString());
        }
    }
