...
public class LoanAccountingBridgeMapper {
    ...
    public LoanAccountingBridgeMapper(final LoanAmortizationAllocationMappingRepository loanAmortizationAllocationMappingRepository,
            final LoanTransactionRepository loanTransactionRepository, final TaxCatalogPort taxCatalogPort) {
    ...
    public AccountingBridgeDataDTO toBridge(final LoanTransaction loanTransaction, final boolean isAccountTransfer) {
        ...
        if (loan.isClosedWrittenOff() && loan.getWriteOffReasonId() != null) {
            writeOffReasonAdvancedMappingData = new AdvancedMappingtDTO(loan.getWriteOffReasonId(), BigDecimal.ZERO);
        }
    ...
    private List<AdvancedMappingtDTO> getLoanTransactionClassificationId(final LoanTransaction loanTransaction) {
        ...
        if (loanTransaction.isCapitalizedIncomeAmortization() || loanTransaction.isBuyDownFeeAmortization()) {
            ...
            loanTransactionAllocations.forEach(loanTransactionAllocation -> {
                final Long classificationId = loanTransactionRepository
                        .fetchClassificationIdByTransactionId(loanTransactionAllocation.getBaseLoanTransactionId());
                ...
                advancedMappingData.add(new AdvancedMappingtDTO(classificationId, allocationAmount));
            });
        }
        return advancedMappingData;
    }

    private AccountingBridgeLoanTransactionDTO convertToAccountingBridgeTransaction(final LoanTransaction loanTransaction) {
