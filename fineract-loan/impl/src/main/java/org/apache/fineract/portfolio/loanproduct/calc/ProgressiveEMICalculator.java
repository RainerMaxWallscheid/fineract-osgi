...
public final class ProgressiveEMICalculator implements EMICalculator {
    ...
    private LoanApplicationTerms buildLoanApplicationTerms(final ILoanConfigurationDetails loanProductRelatedDetail,
        ...
        // Flags & options
        // Technical
        new LoanApplicationTerms.Builder().currency(loanProductRelatedDetail.getCurrencyData()).principal(disbursedAmount)
                ...
                .daysInYearCustomStrategy(loanProductRelatedDetail.getDaysInYearCustomStrategy()).isDownPaymentEnabled(false)
                .allowPartialPeriodInterestCalculation(loanProductRelatedDetail.isAllowPartialPeriodInterestCalculation()).mc(mc).build();
    }
    ...
    private List<RepaymentPeriod> findPossiblyOverdueRepaymentPeriods(final LocalDate targetDate,
        ...
        return //
        model.repaymentPeriods().stream().filter(repaymentPeriod -> !repaymentPeriod.isReAgedEarlyRepaymentHolder()
                && DateUtils.isAfter(targetDate, repaymentPeriod.getDueDate())).toList();
    }
    ...
    private void checkAndAdjustEmiIfNeededOnRelatedRepaymentPeriods(final ProgressiveLoanInterestScheduleModel scheduleModel,
        ...
        do {
            ...
            final Iterator<RepaymentPeriod> relatedPeriodFromNewModelIterator = //
                    //
                    newScheduleModel.repaymentPeriods().stream()
    ...
    private BigDecimal calculateFnResultForEmi(final List<RepaymentPeriod> periods,
        ...
        //
        //
        periods.stream().skip(1).map(period -> getRateFactorPlus1ForEmi(period, scheduleModel)).reduce(BigDecimal.ONE,
    ...
    public Money getSumOfDueInterestsOnDate(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate subjectDate) {
        ...
        //
        //
        scheduleModel.repaymentPeriods().stream().map(
                repaymentPeriod -> getDueAmounts(scheduleModel, repaymentPeriod.getFromDate(), repaymentPeriod.getDueDate(), subjectDate)
                        .getDueInterest())
                .reduce(scheduleModel.zero(), Money::add); //
    }
