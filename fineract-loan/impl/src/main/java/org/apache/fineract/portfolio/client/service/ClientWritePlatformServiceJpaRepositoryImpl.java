...
public class ClientWritePlatformServiceJpaRepositoryImpl implements ClientWritePlatformService {
    ...
    public CommandProcessingResult deleteClient(final Long clientId) {
        try {
            ...
            //
            //
            new CommandProcessingResultBuilder().withOfficeId(client.officeId()).withEntityExternalId(client.getExternalId())
    ...
    public CommandProcessingResult createClient(final JsonCommand command) {
        try {
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withEntityExternalId(newClient.getExternalId())
                    .withOfficeId(clientOffice.getId()).withClientId(newClient.getId()).withGroupId(groupId).withEntityId(newClient.getId())
                    .withSavingsId(result.getSavingsId()).setRollbackTransaction(rollbackTransaction)
                    .setRollbackTransaction(result.isRollbackTransaction()).build();
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
    ...
    public CommandProcessingResult updateClient(final Long clientId, final JsonCommand command) {
        try {
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withEntityExternalId(clientForUpdate.getExternalId())
                    .withOfficeId(clientForUpdate.officeId()).withClientId(clientId).withEntityId(clientId).with(changes).build();
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            handleDataIntegrityIssues(command, dve.getMostSpecificCause(), dve);
            return CommandProcessingResult.empty();
        } catch (final PersistenceException dve) {
    ...
    public CommandProcessingResult activateClient(final Long clientId, final JsonCommand command) {
        try {
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withOfficeId(client.officeId())
                    .withEntityExternalId(client.getExternalId()).withClientId(clientId).withEntityId(clientId)
                    .withSavingsId(result.getSavingsId()).setRollbackTransaction(result.isRollbackTransaction()).build();
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            handleDataIntegrityIssues(command, dve.getMostSpecificCause(), dve);
            return CommandProcessingResult.empty();
        }
    ...
    public CommandProcessingResult unassignClientStaff(final Long clientId, final JsonCommand command) {
        ...
        //
        //
        new CommandProcessingResultBuilder().withCommandId(command.commandId()).withOfficeId(clientForUpdate.officeId())
                .withEntityId(clientForUpdate.getId()).withEntityExternalId(clientForUpdate.getExternalId()).withClientId(clientId)
                .with(actualChanges).build();
    }
    ...
    public CommandProcessingResult assignClientStaff(final Long clientId, final JsonCommand command) {
        ...
        //
        new CommandProcessingResultBuilder().withCommandId(command.commandId()).withOfficeId(clientForUpdate.officeId())
                .withEntityExternalId(clientForUpdate.getExternalId()).withEntityId(clientForUpdate.getId()).withClientId(clientId)
                .with(actualChanges).build();
    }

    /**
     * TODO Vishwas: We maintain history of chage of loan officer w.r.t loan in a history table, should we do the same
     * for a client? Especially useful when the change happens due to a transfer etc
     */
    @Transactional
    @Override
    public CommandProcessingResult closeClient(final Long clientId, final JsonCommand command) {
        try {
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withClientId(clientId).withEntityId(clientId)
    ...
    public CommandProcessingResult rejectClient(final Long entityId, final JsonCommand command) {
        ...
        //
        //
        new CommandProcessingResultBuilder().withCommandId(command.commandId()).withClientId(entityId).withEntityId(entityId)
                .withEntityExternalId(client.getExternalId()).build();
    }
