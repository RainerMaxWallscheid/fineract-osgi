...
public class LoanAccountDomainServiceJpa implements LoanAccountDomainService {
    ...
    public LoanTransaction makeRepayment(final LoanTransactionType repaymentTransactionType, Loan loan, final LocalDate transactionDate,
        ...
        if (StringUtils.isNotBlank(noteText)) {
            this.noteWritePlatformService.saveLoanTransactionNote(loan.getId(), newRepaymentTransaction.getId(), loan.getClientId(),
                    noteText);
        }
    ...
    public LoanTransaction makeChargePayment(final Loan loan, final Long chargeId, final LocalDate transactionDate,
        ...
        if (StringUtils.isNotBlank(noteText)) {
            this.noteWritePlatformService.saveLoanTransactionNote(loan.getId(), newPaymentTransaction.getId(), loan.getClientId(),
                    noteText);
        }
        loanAccrualsProcessingService.processAccrualsOnInterestRecalculation(loan, loan.isInterestBearingAndInterestRecalculationEnabled(),
                true);
    ...
    public LoanTransaction makeDisburseTransaction(final Long loanId, final LocalDate transactionDate, final BigDecimal transactionAmount,
        ...
        loanAccountService.saveAndFlushLoanWithDataIntegrityViolationChecks(loan);
        if (StringUtils.isNotBlank(noteText)) {
            this.noteWritePlatformService.saveLoanTransactionNote(loan.getId(), disbursementTransaction.getId(), loan.getClientId(),
                    noteText);
        }
    ...
    public LoanTransaction creditBalanceRefund(final Loan loan, final LocalDate transactionDate, final BigDecimal transactionAmount,
        ...
        if (StringUtils.isNotBlank(noteText)) {
            this.noteWritePlatformService.saveLoanTransactionNote(loan.getId(), newCreditBalanceRefundTransaction.getId(),
                    loan.getClientId(), noteText);
        }
        loanAccrualsProcessingService.processAccrualsOnInterestRecalculation(loan, loan.isInterestBearingAndInterestRecalculationEnabled(),
                true);
    ...
    public LoanAccountDomainServiceJpa(final LoanAssembler loanAccountAssembler, final LoanRepositoryWrapper loanRepositoryWrapper,
            final LoanTransactionRepository loanTransactionRepository, final ConfigurationDomainService configurationDomainService,
            final HolidayRepository holidayRepository, final WorkingDaysRepositoryWrapper workingDaysRepository,
            final NoteWritePlatformService noteWritePlatformService, final BusinessEventNotifierService businessEventNotifierService,
            final LoanUtilService loanUtilService, final StandingInstructionRepository standingInstructionRepository,
            ...
            final LoanLifecycleStateMachine loanLifecycleStateMachine, final ExternalIdFactory externalIdFactory,
            ...
            final LoanForeclosureValidator loanForeclosureValidator,
            final LoanDownPaymentTransactionValidator loanDownPaymentTransactionValidator, final LoanChargeService loanChargeService,
            final LoanScheduleService loanScheduleService, final LoanDownPaymentHandlerService loanDownPaymentHandlerService,
            final LoanChargeValidator loanChargeValidator, final LoanRefundService loanRefundService,
            final LoanAccountService loanAccountService, final ReprocessLoanTransactionsService reprocessLoanTransactionsService,
            final LoanTransactionProcessingService loanTransactionProcessingService, final LoanBalanceService loanBalanceService,
            final LoanTransactionService loanTransactionService, final LoanAccountDomainServiceJpaHelper loanAccountDomainServiceJpaHelper,
