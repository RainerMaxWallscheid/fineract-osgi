...
public final class LoanDownPaymentTransactionValidator {
    ...
    public void validateLoanStatusIsActiveOrFullyPaidOrOverpaid(final Loan loan) {
        if (!(loan.isOpen() || loan.isClosedObligationsMet() || loanBalanceService.isOverPaid(loan))) {
            ...
            final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.must.be.active.fully.paid.or.overpaid", defaultUserMessage);
    ...
    public void validateRepaymentTypeAccountStatus(final Loan loan, final LoanTransaction repaymentTransaction, final LoanEvent event) {
        if (repaymentTransaction.isGoodwillCredit() || repaymentTransaction.isInterestPaymentWaiver() || repaymentTransaction.isMerchantIssuedRefund() || repaymentTransaction.isPayoutRefund() || repaymentTransaction.isChargeRefund() || repaymentTransaction.isRepayment() || repaymentTransaction.isDownPayment() || repaymentTransaction.isInterestRefund()) {
        ...
        switch (event) {
            ...
            case LOAN_APPROVAL_UNDO -> {
                if (!loan.isApproved()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.undo.approval.account.is.not.approved", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_DISBURSED -> {
                if ((!(loan.isApproved() && loan.isNotDisbursed()) && !loan.getLoanProduct().isMultiDisburseLoan()) || (loan.getLoanProduct().isMultiDisburseLoan() && !loan.isAllTranchesNotDisbursed())) {
            ...
            case LOAN_DISBURSAL_UNDO -> {
                if (!loan.isOpen()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.undo.disbursal.account.is.not.active", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
                if (loan.isOpen() && loan.isTopup()) {
                    final String defaultUserMessage = "Loan Undo disbursal is not allowed on Topup Loans";
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.undo.disbursal.not.allowed.on.topup.loan", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_REPAYMENT_OR_WAIVER -> {
                if (!loan.isOpen()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.repayment.or.waiver.account.is.not.active", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case WRITE_OFF_OUTSTANDING -> {
                if (!loan.isOpen()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.writtenoff.account.is.not.active", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case WRITE_OFF_OUTSTANDING_UNDO -> {
                if (!loan.isClosedWrittenOff()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.undo.writtenoff.account.is.not.written.off", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_CHARGE_PAYMENT -> {
                if (!loan.isOpen()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.charge.payment.account.is.not.active", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_CLOSED -> {
                if (!loan.isOpen()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.close.account.is.not.active", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_EDIT_MULTI_DISBURSE_DATE -> {
                if (loan.isClosed()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.edit.disbursement.account.is.not.active", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_RECOVERY_PAYMENT -> {
                if (!loan.isClosedWrittenOff()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.account.is.not.written.off", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_REFUND -> {
                if (!loan.isOpen()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.refund.account.is.not.active", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_DISBURSAL_UNDO_LAST -> {
                if (!loan.isOpen()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.undo.last.disbursal.account.is.not.active", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_FORECLOSURE -> {
                if (!loan.isOpen()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.foreclosure.account.is.not.active", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_CREDIT_BALANCE_REFUND -> {
                if (!loan.getStatus().isOverpaid()) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.credit.balance.refund.account.is.not.overpaid", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            case LOAN_CHARGE_ADJUSTMENT -> {
                if (!(loan.getStatus().isActive() || loan.getStatus().isClosedObligationsMet() || loan.getStatus().isOverpaid())) {
                    ...
                    final ApiParameterError error = ApiParameterError.generalError("error.msg.loan.charge.adjustment.account.is.not.in.valid.state", defaultUserMessage);
                    dataValidationErrors.add(error);
                }
            }
            default -> {
    ...
    @java.lang.SuppressWarnings("all")
        public LoanDownPaymentTransactionValidator(final LoanBalanceService loanBalanceService) {
