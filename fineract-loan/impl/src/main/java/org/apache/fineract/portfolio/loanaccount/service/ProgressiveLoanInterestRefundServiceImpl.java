...
public class ProgressiveLoanInterestRefundServiceImpl implements InterestRefundService {
    ...
    public Money totalInterestByTransactions(LoanRepaymentScheduleTransactionProcessor processor, final Long loanId,
        ...
        List<LoanTransaction> transactions = //
                ...
                //
                Stream.concat(
                        loan.getLoanTransactions().stream().filter(
                                lt -> isTransactionNeededForInterestRefundCalculations(lt) && oldTransactionIds.contains(lt.getId())),
                        newTransactions.stream().filter(this::isTransactionNeededForInterestRefundCalculations)
                                .map(LoanTransaction::copyTransactionProperties))
                        .toList();
        final AtomicReference<BigDecimal> refundFinal = new AtomicReference<>( //
                transactions.stream().filter(lt -> interestRefundTypes.contains(lt.getTypeOf())).map(LoanTransaction::getAmount)
    ...
    @java.lang.SuppressWarnings("all")
    public ProgressiveLoanInterestRefundServiceImpl(final EMICalculator emiCalculator, final LoanAssembler loanAssembler,
            final LoanScheduleService loanScheduleService, final LoanUtilService loanUtilService,
