...
public class PermissionsApiResource {
    private static final Set<String> RESPONSE_DATA_PARAMETERS = new HashSet<>(Arrays.asList("grouping", "code", "entityName", "actionName", "selected", "isMakerChecker"));
    ...
    @Produces({MediaType.APPLICATION_JSON})
    @Operation(summary = "Enable/Disable Permissions for Maker Checker", operationId = "updatePermissions", tags = {"Permissions"}, description = "")
    ...
    @ApiResponses({@ApiResponse(responseCode = "200", description = "OK", content = @Content(schema = @Schema(implementation = CommandProcessingResult.class)))})
    public String updatePermissionsDetails(@Parameter(hidden = true) final String apiRequestBodyAsJson) {
        final CommandWrapper commandRequest =  //
        //
        //
        new CommandWrapperBuilder().updatePermissions().withJson(apiRequestBodyAsJson).build();
        final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
        return this.toApiJsonSerializer.serialize(result);
    }

    @java.lang.SuppressWarnings("all")
        public PermissionsApiResource(final PlatformSecurityContext context, final PermissionReadPlatformService permissionReadPlatformService, final DefaultToApiJsonSerializer<PermissionData> toApiJsonSerializer, final ApiRequestParameterHelper apiRequestParameterHelper, final PortfolioCommandSourceWritePlatformService commandsSourceWritePlatformService) {
