...
public class PermissionReadPlatformServiceImpl implements PermissionReadPlatformService {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PermissionReadPlatformServiceImpl.class);
    ...
    private static final class PermissionUsageDataMapper implements RowMapper<PermissionData> {
        ...
        public String permissionSchema() {
            ...
            return "select p.grouping, p.code, p.entity_name as entityName, p.action_name as actionName, true as selected" + " from m_permission p " + " where code not like \'%\\_CHECKER\'" + " order by p.grouping, coalesce(entity_name, \'\'), p.code";
        }
        ...
        public String makerCheckerablePermissionSchema() {
             ...
             */
            return "select p.grouping, p.code, p.entity_name as entityName, p.action_name as actionName, p.can_maker_checker as selected" + " from m_permission p " + " where " + sqlGenerator.escape("grouping") + " != \'special\' and code not like \'READ_%\' and code not like \'%\\_CHECKER\'" + " order by p.grouping, coalesce(entity_name, \'\'), p.code";
        }

        public String rolePermissionSchema() {
            return "select p.grouping, p.code, p.entity_name as entityName, p.action_name as actionName, rp.role_id IS NOT NULL as selected " + " from m_permission p " + " left join m_role_permission rp on rp.permission_id = p.id and rp.role_id = ? " + " order by p.grouping, COALESCE(entity_name, \'\'), p.code";
        }
    }

    @java.lang.SuppressWarnings("all")
        public PermissionReadPlatformServiceImpl(final PlatformSecurityContext context, final JdbcTemplate jdbcTemplate, final DatabaseSpecificSQLGenerator sqlGenerator) {
