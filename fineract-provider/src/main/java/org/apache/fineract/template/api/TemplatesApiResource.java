...
public class TemplatesApiResource {
    ...
    public List<TemplateData> retrieveAllTemplates(@DefaultValue("-1") @QueryParam("typeId") @Parameter(description = "typeId") final int typeId, @DefaultValue("-1") @QueryParam("entityId") @Parameter(description = "entityId") final int entityId) {
    ...
    public TemplateUpdateResponse saveTemplate(@PathParam("templateId") @Parameter(description = "templateId") final Long templateId, @RequestBody(required = true) @Valid final TemplateUpdateRequest request) {
    ...
    public String mergeTemplate(@PathParam("templateId") @Parameter(description = "templateId") final Long templateId, @Context final UriInfo uriInfo, @RequestBody(required = true) final Map<String, Object> result) {
    ...
    private List<TemplateItemData> retrieveEntities() {
        return Stream.of(TemplateEntity.values()).map(entity -> TemplateItemData.builder().id(entity.getId()).name(entity.getName()).build()).toList();
    }

    private List<TemplateItemData> retrieveTypes() {
        return Stream.of(TemplateType.values()).map(type -> TemplateItemData.builder().id(type.getId()).name(type.getName()).build()).toList();
    }

    private TemplateEntity findTemplateEntity(final int entityId) {
        return Stream.of(TemplateEntity.values()).filter(entity -> entity.getId() == entityId).findFirst().orElseThrow(() -> new IllegalArgumentException("Invalid template entity id: " + entityId));
    }

    private TemplateType findTemplateType(final int typeId) {
        return Stream.of(TemplateType.values()).filter(type -> type.getId() == typeId).findFirst().orElseThrow(() -> new IllegalArgumentException("Invalid template type id: " + typeId));
    }

    @java.lang.SuppressWarnings("all")
        public TemplatesApiResource(final TemplateDomainService templateService, final TemplateMergeServiceImpl templateMergeService, final CommandDispatcher dispatcher) {
