...
public class FixedDepositAccountTransactionsApiResource {
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    public String transaction(@PathParam("fixedDepositAccountId") final Long fixedDepositAccountId, @QueryParam("command") final String commandParam, final String apiRequestBodyAsJson) {
    ...
    @ApiResponses({@ApiResponse(responseCode = "200", description = "OK", content = @Content(schema = @Schema(implementation = FixedDepositAccountTransactionsApiResourceSwagger.PostFixedDepositAccountsFixedDepositAccountIdTransactionsTransactionIdResponse.class))), @ApiResponse(responseCode = "default", description = "error occurred")})
    public String adjustTransaction(@PathParam("fixedDepositAccountId") final Long fixedDepositAccountId, @PathParam("transactionId") final Long transactionId, @QueryParam("command") final String commandParam, final String apiRequestBodyAsJson) {
        ...
        if (is(commandParam, DepositsApiConstants.COMMAND_UNDO_TRANSACTION)) {
            ...
            final CommandWrapper commandRequest = builder.adjustFixedDepositAccountTransaction(fixedDepositAccountId, transactionId).build();
            result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
        }
        if (result == null) {
            //
