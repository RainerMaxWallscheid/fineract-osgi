...
public class CalendarReadPlatformServiceImpl implements CalendarReadPlatformService {
    ...
    private static final class CalendarDataMapper implements RowMapper<CalendarData> {
        ...
        public CalendarData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return CalendarData.instance(id, calendarInstanceId, entityId, entityType, title, description, location, startDate, endDate, duration, type, repeating, recurrence, frequency, interval, repeatsOnDay, repeatsOnNthDayOfMonth, remindBy, firstReminder, secondReminder, humanReadable, createdDate, lastModifiedDate, createdByUserId, createdByUserName, lastUpdatedByUserId, lastUpdatedByUserName, meetingTime, monthOnDay);
        }
    }
    ...
    @Override
    public Collection<CalendarData> retrieveCalendarsByEntity(final Long entityId, final Integer entityTypeId, final List<Integer> calendarTypeOptions) {
        final CalendarDataMapper rm = new CalendarDataMapper();
    ...
    public CalendarData retrieveCollctionCalendarByEntity(final Long entityId, final Integer entityTypeId) {
        final CalendarDataMapper rm = new CalendarDataMapper();
        final String sql = rm.schema() + " and ci.entity_id = ? and ci.entity_type_enum = ? and calendar_type_enum = ? order by c.start_date ";
    ...
    @Override
    public Collection<CalendarData> retrieveParentCalendarsByEntity(final Long entityId, final Integer entityTypeId, final List<Integer> calendarTypeOptions) {
        final CalendarDataMapper rm = new CalendarDataMapper();
        Collection<CalendarData> result = null;
        String sql = "";
    ...
    private Collection<LocalDate> generateRecurringDate(final CalendarData calendarData, final LocalDate fromDate, final LocalDate tillDate, final int maxCount) {
        ...
        final Collection<LocalDate> recurringDates = CalendarUtils.getRecurringDates(rrule, seedDate, periodStartDate, periodEndDate, maxCount, isSkipRepaymentOnFirstMonthEnabled, numberOfDays);
        return recurringDates;
    }
    ...
    public LocalDate generateNextEligibleMeetingDateForCollection(final CalendarData calendarData, final MeetingData lastMeetingData) {
        ...
        if (lastMeetingDate != null && !calendarData.isBetweenStartAndEndDate(lastMeetingDate) && !calendarData.isBetweenStartAndEndDate(DateUtils.getLocalDateOfTenant())) {
            ...
            nextEligibleMeetingDate = CalendarUtils.getRecentEligibleMeetingDate(applicableCalendarData.getRecurrence(), lastMeetingDate, isSkipRepaymentOnFirstMonthEnabled, numberOfDays);
        }
        if (nextEligibleMeetingDate == null) {
            ...
            nextEligibleMeetingDate = CalendarUtils.getRecentEligibleMeetingDate(applicableCalendarData.getRecurrence(), seedDate, isSkipRepaymentOnFirstMonthEnabled, numberOfDays);
        } else if (calendarData.isBetweenStartAndEndDate(nextEligibleMeetingDate)) {
            nextEligibleMeetingDate = CalendarUtils.getRecentEligibleMeetingDate(applicableCalendarData.getRecurrence(), calendarData.getStartDate(), isSkipRepaymentOnFirstMonthEnabled, numberOfDays);
        }
    ...
    public List<CalendarData> updateWithRecurringDates(final Collection<CalendarData> calendarsData) {
        ...
        for (final CalendarData calendarData : calendarsData) {
            ...
            final CalendarData updatedCalendarData = CalendarData.withRecurringDates(calendarData, recurringDates, nextTenRecurringDates, recentEligibleMeetingDate);
    ...
    private static final class CalendarDataFromHistoryMapper implements RowMapper<CalendarData> {
        public String schema() {
            return " select c.calendar_id as id, c.title as title, c.description as description, c.location as location, c.start_date as startDate, " + " c.end_date as endDate, c.duration as duration, c.calendar_type_enum as typeId, c.repeating as repeating, " + " c.recurrence as recurrence, c.remind_by_enum as remindById, c.first_reminder as firstReminder, c.second_reminder as secondReminder " + " from m_calendar_history c ";
        }
        ...
        public CalendarData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            Integer monthOnDay = CalendarUtils.getMonthOnDay(recurrence);
            return CalendarData.instance(id, calendarInstanceId, entityId, entityType, title, description, location, startDate, endDate, duration, type, repeating, recurrence, frequency, interval, repeatsOnDay, repeatsOnNthDayOfMonth, remindBy, firstReminder, secondReminder, humanReadable, createdDate, lastUpdatedDate, createdByUserId, createdByUserName, lastUpdatedByUserId, lastUpdatedByUserName, meetingTime, monthOnDay);
        }
    }
