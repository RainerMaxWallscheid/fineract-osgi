...
public class AddressReadPlatformServiceImpl implements AddressReadPlatformService {
    ...
    private static final class AddFieldsMapper implements RowMapper<AddressData> {
        ...
        public AddressData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return AddressData.instance1(addressId, street, address_line_1, address_line_2, address_line_3, town_village, city, county_district, state_province_id, country_id, postal_code, latitude, longitude, created_by, created_on_local_date, updated_by, update_on_local_date);
        }
    }
    ...
    private static final class AddMapper implements RowMapper<AddressData> {
        public String schema() {
            return "cv2.code_value as addressType,ca.client_id as client_id,addr.id as id,ca.address_type_id as addresstyp,ca.is_active as is_active,addr.street as street,addr.address_line_1 as address_line_1,addr.address_line_2 as address_line_2," + "addr.address_line_3 as address_line_3,addr.town_village as town_village, addr.city as city,addr.county_district as county_district," + "addr.state_province_id as state_province_id,cv.code_value as state_name, addr.country_id as country_id,c.code_value as country_name,addr.postal_code as postal_code,addr.latitude as latitude," + "addr.longitude as longitude,addr.created_by as created_by,addr.created_on as created_on,addr.updated_by as updated_by," + "addr.updated_on as updated_on" + " from m_address addr left join m_code_value cv on addr.state_province_id=cv.id" + " left join  m_code_value c on addr.country_id=c.id" + " join m_client_address ca on addr.id= ca.address_id" + " join m_code_value cv2 on ca.address_type_id=cv2.id";
        }
        ...
        public AddressData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            final Date updated_on = rs.getDate("updated_on");
            final LocalDate update_on_local_date = updated_on != null ? updated_on.toLocalDate() : null;
            return AddressData.instance(addressType, client_id, addressId, address_type_id, is_active, street, address_line_1, address_line_2, address_line_3, town_village, city, county_district, state_province_id, country_id, state_name, country_name, postal_code, latitude, longitude, created_by, created_on_local_date, updated_by, update_on_local_date);
        }
    }
    ...
    public List<AddressData> retrieveBySearchParam(ClientAddressSearchParam params) {
        return getFilterRules().stream().filter(rule -> rule.matches(params)).map(r -> r.execute(params)).findFirst().orElse(retrieveAddressbyStatus(params.getClientId(), params.getStatus()));
    }
    ...
    private List<FetcherRule<ClientAddressSearchParam, List<AddressData>>> getFilterRules() {
        return List.of(new FetcherRule<>(p -> p.getAddressTypeId() == 0 && p.getStatus() == null, p -> retrieveAllClientAddress(p.getClientId())), new FetcherRule<>(p -> p.getAddressTypeId() != 0 && p.getStatus() == null, p -> retrieveAddressbyType(p.getClientId(), p.getAddressTypeId())), new FetcherRule<>(p -> p.getAddressTypeId() != 0 && p.getStatus() != null, p -> retrieveAddressbyTypeAndStatus(p.getClientId(), p.getAddressTypeId(), p.getStatus())));
    }

    @java.lang.SuppressWarnings("all")
        public AddressReadPlatformServiceImpl(final JdbcTemplate jdbcTemplate, final PlatformSecurityContext context, final CodeValueReadPlatformService readService) {
