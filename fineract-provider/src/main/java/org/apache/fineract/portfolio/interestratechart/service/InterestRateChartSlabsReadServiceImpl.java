...
public class InterestRateChartSlabsReadServiceImpl implements InterestRateChartSlabsReadService {
    ...
    public InterestRateChartSlabData retrieveWithTemplate(InterestRateChartSlabData chartSlab) {
        final List<CodeValueData> genderOptions = new ArrayList<>(codeValueReadPlatformService.retrieveCodeValuesByCode(ClientApiConstants.GENDER));
        final List<CodeValueData> clientTypeOptions = new ArrayList<>(codeValueReadPlatformService.retrieveCodeValuesByCode(ClientApiConstants.CLIENT_TYPE));
        final List<CodeValueData> clientClassificationOptions = new ArrayList<>(codeValueReadPlatformService.retrieveCodeValuesByCode(ClientApiConstants.CLIENT_CLASSIFICATION));
        return InterestRateChartSlabData.withTemplate(chartSlab, chartDropdownReadPlatformService.retrievePeriodTypeOptions(), interestIncentiveDropdownReadService.retrieveEntityTypeOptions(), interestIncentiveDropdownReadService.retrieveAttributeNameOptions(), interestIncentiveDropdownReadService.retrieveConditionTypeOptions(), interestIncentiveDropdownReadService.retrieveIncentiveTypeOptions(), genderOptions, clientTypeOptions, clientClassificationOptions);
    }
    ...
    private static final class InterestRateChartSlabsMapper implements RowMapper<InterestRateChartSlabData> {
        ...
        public InterestRateChartSlabData mapRow(ResultSet rs, @SuppressWarnings("unused") int rowNum) throws SQLException {
            ...
            return InterestRateChartSlabData.instance(id, description, periodType, fromPeriod, toPeriod, amountRangeFrom, amountRangeTo, annualInterestRate, currency);
        }
    }
