...
public class ClientWritePlatformServiceJpaRepositoryImpl implements ClientWritePlatformService {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ClientWritePlatformServiceJpaRepositoryImpl.class);
    ...
    public CommandProcessingResult deleteClient(final Long clientId) {
        try {
            ...
            //
            //
            new CommandProcessingResultBuilder().withOfficeId(client.officeId()).withEntityExternalId(client.getExternalId()).withClientId(clientId).withEntityId(clientId).build();
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            Throwable throwable = ExceptionUtils.getRootCause(dve.getCause());
            log.error("Error occured.", throwable);
            throw ErrorHandler.getMappable(dve, "error.msg.client.unknown.data.integrity.issue", "Unknown data integrity issue with resource.");
        }
    }
    ...
    private void handleDataIntegrityIssues(final JsonCommand command, final Throwable realCause, final Exception dve) {
        if (realCause.getMessage().contains("external_id")) {
            ...
            throw new PlatformDataIntegrityException("error.msg.client.duplicate.externalId", "Client with externalId `" + externalId + "` already exists", "externalId", externalId);
        } else if (realCause.getMessage().contains("account_no_UNIQUE")) {
            final String accountNo = command.stringValueOfParameterNamed("accountNo");
            throw new PlatformDataIntegrityException("error.msg.client.duplicate.accountNo", "Client with accountNo `" + accountNo + "` already exists", "accountNo", accountNo);
        } else if (realCause.getMessage().contains("mobile_no")) {
            final String mobileNo = command.stringValueOfParameterNamed("mobileNo");
            throw new PlatformDataIntegrityException("error.msg.client.duplicate.mobileNo", "Client with mobileNo `" + mobileNo + "` already exists", "mobileNo", mobileNo);
        }
    ...
    public CommandProcessingResult createClient(final JsonCommand command) {
        try {
            ...
            if (clientTypeId != null) {
                clientType = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.CLIENT_TYPE, clientTypeId);
            }
            ...
            if (clientClassificationId != null) {
                clientClassification = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.CLIENT_CLASSIFICATION, clientClassificationId);
            }
            ...
            entityDatatableChecksWritePlatformService.runTheCheck(newClient.getId(), EntityTables.CLIENT.getName(), StatusEnum.CREATE.getValue(), EntityTables.CLIENT.getForeignKeyColumnNameOnDatatable(), legalForm.getLabel());
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withEntityExternalId(newClient.getExternalId()).withOfficeId(clientOffice.getId()).withClientId(newClient.getId()).withGroupId(groupId).withEntityId(newClient.getId()).withSavingsId(result.getSavingsId()).setRollbackTransaction(rollbackTransaction).setRollbackTransaction(result.isRollbackTransaction()).build();
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
    ...
    public void extractAndCreateClientNonPerson(Client client, JsonCommand command) {
        ...
        if (clientNonPersonElement != null && !isEmpty(clientNonPersonElement)) {
            ...
            if (clientNonPersonConstitutionId != null) {
                clientNonPersonConstitution = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.CLIENT_NON_PERSON_CONSTITUTION, clientNonPersonConstitutionId);
            }
            ...
            if (clientNonPersonMainBusinessLineId != null) {
                clientNonPersonMainBusinessLine = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.CLIENT_NON_PERSON_MAIN_BUSINESS_LINE, clientNonPersonMainBusinessLineId);
            }
    ...
    public CommandProcessingResult updateClient(final Long clientId, final JsonCommand command) {
        try {
            ...
            final ExternalId externalId = externalIdFactory.createFromCommand(command, ClientApiConstants.externalIdParamName);
            if (command.isChangeInStringParameterNamed(ClientApiConstants.externalIdParamName, clientForUpdate.getExternalId().getValue())) {
            ...
            if (command.isChangeInLongParameterNamed(ClientApiConstants.clientClassificationIdParamName, clientForUpdate.clientClassificationId())) {
            ...
            if (command.isChangeInLocalDateParameterNamed(ClientApiConstants.activationDateParamName, clientForUpdate.getActivationDate())) {
            ...
            if (command.isChangeInLocalDateParameterNamed(ClientApiConstants.submittedOnDateParamName, clientForUpdate.getSubmittedOnDate())) {
            ...
            if (changes.containsKey(ClientApiConstants.clientTypeIdParamName)) {
                ...
                if (newValue != null) {
                    newCodeVal = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.CLIENT_TYPE, newValue);
                }
            ...
            if (changes.containsKey(ClientApiConstants.clientClassificationIdParamName)) {
                ...
                CodeValue newCodeVal = null;
                if (newValue != null) {
                    newCodeVal = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.CLIENT_CLASSIFICATION, newValue);
                }
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withEntityExternalId(clientForUpdate.getExternalId()).withOfficeId(clientForUpdate.officeId()).withClientId(clientId).withEntityId(clientId).with(changes).build();
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            handleDataIntegrityIssues(command, dve.getMostSpecificCause(), dve);
            return CommandProcessingResult.empty();
        } catch (final PersistenceException dve) {
    ...
    public CommandProcessingResult activateClient(final Long clientId, final JsonCommand command) {
        try {
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withOfficeId(client.officeId()).withEntityExternalId(client.getExternalId()).withClientId(clientId).withEntityId(clientId).withSavingsId(result.getSavingsId()).setRollbackTransaction(result.isRollbackTransaction()).build();
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            handleDataIntegrityIssues(command, dve.getMostSpecificCause(), dve);
            return CommandProcessingResult.empty();
        }
    ...
    public CommandProcessingResult unassignClientStaff(final Long clientId, final JsonCommand command) {
        ...
        //
        //
        new CommandProcessingResultBuilder().withCommandId(command.commandId()).withOfficeId(clientForUpdate.officeId()).withEntityId(clientForUpdate.getId()).withEntityExternalId(clientForUpdate.getExternalId()).withClientId(clientId).with(actualChanges).build();
    }
    ...
    public CommandProcessingResult assignClientStaff(final Long clientId, final JsonCommand command) {
        ...
        //
        //
        new CommandProcessingResultBuilder().withCommandId(command.commandId()).withOfficeId(clientForUpdate.officeId()).withEntityExternalId(clientForUpdate.getExternalId()).withEntityId(clientForUpdate.getId()).withClientId(clientId).with(actualChanges).build();
    }
    ...
    public CommandProcessingResult closeClient(final Long clientId, final JsonCommand command) {
        try {
            ...
            final CodeValue closureReason = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.CLIENT_CLOSURE_REASON, closureReasonId);
            ...
            if (client.isNotPending() && DateUtils.isAfter(client.getActivationDate(), closureDate)) {
                ...
                throw new InvalidClientStateTransitionException("close", "date.cannot.before.client.actvation.date", errorMessage, closureDate, client.getActivationDate());
            }
            final LegalForm legalForm = LegalForm.fromInt(client.getLegalForm());
            entityDatatableChecksWritePlatformService.runTheCheck(clientId, EntityTables.CLIENT.getName(), StatusEnum.CLOSE.getValue(), EntityTables.CLIENT.getForeignKeyColumnNameOnDatatable(), legalForm.getLabel());
            ...
            for (final Loan loan : clientLoans) {
                ...
                if (loanStatus.isOpen() || loanStatus.isPendingApproval() || loanStatus.isAwaitingDisbursal()) {
                    ...
                    throw new InvalidClientStateTransitionException("close", "date.cannot.before.loan.closed.date", errorMessage, closureDate, loan.getClosedOnDate());
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withClientId(clientId).withEntityId(clientId).withEntityExternalId(client.getExternalId()).build();
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            handleDataIntegrityIssues(command, dve.getMostSpecificCause(), dve);
            return CommandProcessingResult.empty();
        }
    ...
    private void runEntityDatatableCheck(final Long clientId, final Integer legalFormId) {
        ...
        entityDatatableChecksWritePlatformService.runTheCheck(clientId, EntityTables.CLIENT.getName(), StatusEnum.ACTIVATE.getValue(), EntityTables.CLIENT.getForeignKeyColumnNameOnDatatable(), legalForm.getLabel());
    }
    ...
    public CommandProcessingResult rejectClient(final Long entityId, final JsonCommand command) {
        ...
        final CodeValue rejectionReason = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.CLIENT_REJECT_REASON, rejectionReasonId);
        if (client.isNotPending()) {
            final String errorMessage = "Only clients pending activation may be withdrawn.";
            throw new InvalidClientStateTransitionException("rejection", "on.account.not.in.pending.activation.status", errorMessage, rejectionDate, client.getSubmittedOnDate());
            ...
            throw new InvalidClientStateTransitionException("rejection", "date.cannot.before.client.submitted.date", errorMessage, rejectionDate, client.getSubmittedOnDate());
        }
        ...
        //
        //
        new CommandProcessingResultBuilder().withCommandId(command.commandId()).withClientId(entityId).withEntityId(entityId).withEntityExternalId(client.getExternalId()).build();
    }
    ...
    public CommandProcessingResult withdrawClient(Long entityId, JsonCommand command) {
        ...
        final CodeValue withdrawalReason = this.codeValueRepository.findOneByCodeNameAndIdWithNotFoundDetection(ClientApiConstants.CLIENT_WITHDRAW_REASON, withdrawalReasonId);
        if (client.isNotPending()) {
            final String errorMessage = "Only clients pending activation may be withdrawn.";
            throw new InvalidClientStateTransitionException("withdrawal", "on.account.not.in.pending.activation.status", errorMessage, withdrawalDate, client.getSubmittedOnDate());
            ...
            throw new InvalidClientStateTransitionException("withdrawal", "date.cannot.before.client.submitted.date", errorMessage, withdrawalDate, client.getSubmittedOnDate());
        }
    ...
    public CommandProcessingResult reActivateClient(Long entityId, JsonCommand command) {
        ...
        if (!client.isClosed()) {
            ...
            throw new InvalidClientStateTransitionException("reactivation", "date.cannot.before.client.closed.date", errorMessage, reactivateDate, client.getClosureDate());
        }
    ...
    public CommandProcessingResult undoRejection(Long entityId, JsonCommand command) {
        ...
        if (!client.isRejected()) {
            ...
            throw new InvalidClientStateTransitionException("reopened", "date.cannot.before.client.rejected.date", errorMessage, undoRejectDate, client.getRejectedDate());
        }
    ...
    public CommandProcessingResult undoWithdrawal(Long entityId, JsonCommand command) {
        ...
        if (!client.isWithdrawn()) {
            ...
            throw new InvalidClientStateTransitionException("reopened", "date.cannot.before.client.withdrawal.date", errorMessage, undoWithdrawalDate, client.getWithdrawalDate());
        }
