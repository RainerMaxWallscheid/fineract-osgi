...
public class AccountTransfersApiResource {
    ...
    public CommandProcessingResult create(@Parameter(hidden = true) AccountTransferRequest accountTransferRequest) {
        final CommandWrapper commandRequest = new CommandWrapperBuilder().createAccountTransfer().withJson(toApiJsonSerializer.serialize(accountTransferRequest)).build();
    ...
    @Produces({MediaType.APPLICATION_JSON})
    @Operation(summary = "Retrieve Refund of an Active Loan by Transfer Template", operationId = "retrieveTemplateRefundByTransfer", description = "Retrieves Refund of an Active Loan by Transfer Template" + "Example Requests :\n\n" + "\n\n" + "accounttransfers/templateRefundByTransfer?fromAccountId=2&fromAccountType=1& toAccountId=1&toAccountType=2&toClientId=1&toOfficeId=1")
    ...
    public CommandProcessingResult templateRefundByTransferPost(@Parameter(hidden = true) AccountTransferRequest accountTransferRequest) {
        final CommandWrapper commandRequest = new CommandWrapperBuilder().refundByTransfer().withJson(toApiJsonSerializer.serialize(accountTransferRequest)).build();
        return commandsSourceWritePlatformService.logCommandSource(commandRequest);
    }
    ...
    public CommandProcessingResult accountTransferOperation(@PathParam("accountTransferId") @Parameter(description = "accountTransferId") final Long accountTransferId, @QueryParam("command") @Parameter(description = "command") final String commandParam) {
    ...
    @java.lang.SuppressWarnings("all")
        public AccountTransfersApiResource(final PlatformSecurityContext context, final DefaultToApiJsonSerializer<AccountTransferData> toApiJsonSerializer, final PortfolioCommandSourceWritePlatformService commandsSourceWritePlatformService, final AccountTransfersReadPlatformService accountTransfersReadPlatformService) {
