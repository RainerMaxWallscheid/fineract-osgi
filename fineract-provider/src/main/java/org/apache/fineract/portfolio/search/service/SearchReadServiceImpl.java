...
public class SearchReadServiceImpl implements SearchReadService {
    ...
    private static final class SearchMapper implements RowMapper<SearchData> {
        ...
        public SearchData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return new SearchData(entityId, entityAccountNo, entityExternalId, entityName, entityType, parentId, parentName, parentType, entityMobileNo, entityStatus, subEntityTypeCode.getCode());
        }
    }
    ...
    private AdHocQuerySearchConditions convertToSearchConditions(final AdHocQuerySearchRequest request) {
        ...
        return AdHocQuerySearchConditions.instance(request.getLoanStatus(), request.getLoanProducts(), request.getOffices(), request.getLoanDateOption(), request.getLoanFromDate(), request.getLoanToDate(), request.getIncludeOutStandingAmountPercentage() != null ? request.getIncludeOutStandingAmountPercentage() : false, request.getOutStandingAmountPercentageCondition(), request.getMinOutStandingAmountPercentage(), request.getMaxOutStandingAmountPercentage(), request.getOutStandingAmountPercentage(), request.getIncludeOutstandingAmount() != null ? request.getIncludeOutstandingAmount() : false, request.getOutstandingAmountCondition(), request.getMinOutstandingAmount(), request.getMaxOutstandingAmount(), request.getOutstandingAmount());
    }
    ...
    private static final class AdHocQuerySearchMapper implements RowMapper<AdHocSearchQueryData> {
        ...
        public String schema(final AdHocQuerySearchConditions searchConditions, final MapSqlParameterSource params) {
            final StringBuilder sql = new StringBuilder();
            sql.append("Select a.name as officeName, a.Product as productName, a.cnt as \'count\', a.outstandingAmt as outstanding, a.percentOut as percentOut  ").append("from (select mo.name, mp.name Product, SUM(COALESCE(ml.total_expected_repayment_derived,0.0)) TotalAmt, count(*) cnt, ").append("SUM(COALESCE(ml.total_outstanding_derived,0.0)) outstandingAmt,  ").append("(SUM(COALESCE(ml.total_outstanding_derived,0.0)) * 100 / SUM(COALESCE(ml.total_expected_repayment_derived,0.0))) percentOut ").append("from m_loan ml inner join m_product_loan mp on mp.id=ml.product_id  ").append("inner join m_client mc on mc.id=ml.client_id  ").append("inner join m_office mo on mo.id=mc.office_id  ");
            ...
            if (searchConditions.getIncludeOutStandingAmountPercentage()) {
                if (searchConditions.getOutStandingAmountPercentageCondition().equals("between")) {
                    ...
                    sql.append(" a.percentOut ").append(searchConditions.getOutStandingAmountPercentageCondition()).append(" :outStandingAmountPercentage ");
                }
            }
            if (searchConditions.getIncludeOutstandingAmount()) {
                if (searchConditions.getOutstandingAmountCondition().equals("between")) {
                    ...
                    sql.append(" a.outstandingAmt ").append(searchConditions.getOutstandingAmountCondition()).append(" :outstandingAmount ");
                }
            }
            return sql.toString();
        }
        ...
        public AdHocSearchQueryData mapRow(ResultSet rs, @SuppressWarnings("unused") int rowNum) throws SQLException {
            ...
            final BigDecimal loanOutStanding = JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "outstanding").setScale(2, MoneyHelper.getRoundingMode());
            final Double percentage = JdbcSupport.getBigDecimalDefaultToZeroIfNull(rs, "percentOut").setScale(2, MoneyHelper.getRoundingMode()).doubleValue();
