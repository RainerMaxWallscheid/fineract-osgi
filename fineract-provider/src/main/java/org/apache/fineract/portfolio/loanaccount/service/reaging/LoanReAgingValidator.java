...
public class LoanReAgingValidator {
    ...
    private final List<String> reAgeSupportedParameters = List.of(LoanReAgingApiConstants.externalIdParameterName, LoanReAgingApiConstants.startDate, LoanReAgingApiConstants.frequencyType, LoanReAgingApiConstants.frequencyNumber, LoanReAgingApiConstants.numberOfInstallments, LoanReAgingApiConstants.reAgeInterestHandlingParamName, LoanReAgingApiConstants.reasonCodeValueIdParamName, LoanReAgingApiConstants.transactionAmountParamName, LoanReAgingApiConstants.localeParameterName, LoanReAgingApiConstants.dateFormatParameterName, LoanReAgingApiConstants.noteParamName);
    ...
    private void validateReAgeRequest(Loan loan, JsonCommand command) {
        ...
        baseDataValidator.reset().parameter(LoanReAgingApiConstants.externalIdParameterName).ignoreIfNull().value(externalId).notExceedingLengthOf(100);
        ...
        if (loan.isProgressiveSchedule()) {
            ...
            baseDataValidator.reset().parameter(LoanReAgingApiConstants.startDate).value(startDate).notNull().validateDateAfterOrEqual(loan.getDisbursementDate());
        } else {
            baseDataValidator.reset().parameter(LoanReAgingApiConstants.startDate).value(startDate).notNull().validateDateAfter(loan.getMaturityDate());
        }
        ...
        baseDataValidator.reset().parameter(LoanReAgingApiConstants.frequencyNumber).value(frequencyNumber).notNull().integerGreaterThanZero();
        Integer numberOfInstallments = command.integerValueOfParameterNamed(LoanReAgingApiConstants.numberOfInstallments);
        baseDataValidator.reset().parameter(LoanReAgingApiConstants.numberOfInstallments).value(numberOfInstallments).notNull().integerGreaterThanZero();
        final LoanReAgeInterestHandlingType reAgeInterestHandlingType = command.enumValueOfParameterNamed(LoanReAgingApiConstants.reAgeInterestHandlingParamName, LoanReAgeInterestHandlingType.class);
        baseDataValidator.reset().parameter(LoanReAgingApiConstants.reAgeInterestHandlingParamName).value(reAgeInterestHandlingType).ignoreIfNull();
        ...
        baseDataValidator.reset().parameter(LoanReAgingApiConstants.transactionAmountParamName).value(transactionAmount).ignoreIfNull().positiveAmount();
    ...
    private void validateReAgeBusinessRules(Loan loan) {
        ...
        if (!loan.isProgressiveSchedule() && DateUtils.isBefore(getBusinessLocalDate(), loan.getMaturityDate())) {
            throw new GeneralPlatformDomainRuleException("error.msg.loan.reage.cannot.be.submitted.before.maturity", "Loan cannot be re-aged before maturity", loan.getId());
        }
        ...
        if (!(isProgressiveSchedule && isAdvancedPaymentSchedule)) {
            throw new GeneralPlatformDomainRuleException("error.msg.loan.reage.supported.only.for.progressive.loan.schedule.type", "Loan reaging is only available for progressive repayment schedule and Advanced payment allocation strategy", loan.getId());
        }
        // validate reaging is only done on an active loan
        if (!loan.getStatus().isActive()) {
            throw new GeneralPlatformDomainRuleException("error.msg.loan.reage.supported.only.for.active.loans", "Loan reaging can only be done on active loans", loan.getId());
        }
        // validate if there's already a re-aging transaction for today
        final boolean isReAgingTransactionForTodayPresent = loanTransactionRepository.existsNonReversedByLoanAndTypeAndDate(loan, LoanTransactionType.REAGE, getBusinessLocalDate());
        if (isReAgingTransactionForTodayPresent) {
            throw new GeneralPlatformDomainRuleException("error.msg.loan.reage.reage.transaction.already.present.for.today", "Loan reaging can only be done once a day. There has already been a reaging done for today", loan.getId());
        }
        // validate loan is not charged-off
        if (loan.isChargedOff()) {
            throw new GeneralPlatformDomainRuleException("error.msg.loan.reage.not.allowed.on.charged.off", "Loan re-aging is not allowed on charged-off loan.", loan.getId());
        }
        // validate loan is not contract terminated
        if (loan.isContractTermination()) {
            throw new GeneralPlatformDomainRuleException("error.msg.loan.reage.not.allowed.on.contract.terminated", "Loan re-aging is not allowed on contract terminated loan.", loan.getId());
        }
    }
    ...
    private void validateUndoReAgeBusinessRules(Loan loan) {
        ...
        if (optionalReAgingTx.isEmpty()) {
            throw new GeneralPlatformDomainRuleException("error.msg.loan.reage.reaging.transaction.missing", "Undoing a reaging can only be done if there was a reaging already", loan.getId());
        }
    }
    ...
    private void validateReAgeOutstandingBalance(final Loan loan, final JsonCommand command) {
        ...
        if (MathUtil.isZero(totalPrincipalOutstanding)) {
            throw new GeneralPlatformDomainRuleException("error.msg.loan.reage.no.outstanding.balance.to.reage", "Loan cannot be re-aged as there are no outstanding balances to be re-aged", loan.getId());
        }
    }
    ...
    private void validateReAgeRequest(final Loan loan, final ReAgePreviewRequest reAgePreviewRequest) {
        ...
        final Locale locale = reAgePreviewRequest.getLocale() != null ? Locale.forLanguageTag(reAgePreviewRequest.getLocale()) : Locale.getDefault();
        final LocalDate startDate = JsonParserHelper.convertFrom(reAgePreviewRequest.getStartDate(), LoanReAgingApiConstants.startDate, reAgePreviewRequest.getDateFormat(), locale);
        if (loan.isProgressiveSchedule()) {
            baseDataValidator.reset().parameter(LoanReAgingApiConstants.startDate).value(startDate).validateDateAfterOrEqual(loan.getDisbursementDate());
        } else {
            baseDataValidator.reset().parameter(LoanReAgingApiConstants.startDate).value(startDate).validateDateAfter(loan.getMaturityDate());
        }
        throwExceptionIfValidationErrorsExist(dataValidationErrors);
    }
    ...
    @java.lang.SuppressWarnings("all")
        public LoanReAgingValidator(final LoanTransactionRepository loanTransactionRepository, final CodeValueRepository codeValueRepository, final FromJsonHelper fromApiJsonHelper) {
