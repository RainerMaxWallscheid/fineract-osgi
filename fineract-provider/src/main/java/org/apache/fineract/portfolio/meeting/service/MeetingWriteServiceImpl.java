...
public class MeetingWriteServiceImpl implements MeetingWriteService {
    ...
    public MeetingCreateResponse createMeeting(@Valid final MeetingCreateRequest request) {
        ...
        try {
            ...
            var meeting = createNew(calendarInstance, meetingDate, isTransactionDateOnNonMeetingDate, isSkipRepaymentOnFirstMonth, numberOfDays);
    ...
    private Meeting createNew(CalendarInstance calendarInstance, LocalDate meetingDate, Boolean isTransactionDateOnNonMeetingDate, boolean isSkipRepaymentOnFirstMonth, int numberOfDays) {
    ...
    private RuntimeException handleMeetingDataIntegrityIssues(final LocalDate meetingDate, final Throwable realCause, final Exception dve) {
        ...
        return ErrorHandler.getMappable(dve, "error.msg.meeting.unknown.data.integrity.issue", "Unknown data integrity issue with resource: " + realCause.getMessage());
    }
