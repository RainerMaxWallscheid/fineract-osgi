...
public class ReadSurveyServiceImpl implements ReadSurveyService {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ReadSurveyServiceImpl.class);
    ...
    public List<SurveyDataTableData> retrieveAllSurveys() {
        ...
        while (rs.next()) {
            ...
            surveyDataTables.add(SurveyDataTableData.create(DatatableData.create(appTableName, registeredDatatableName, entitySubType, columnHeaderData), enabled));
        }
    ...
    public SurveyDataTableData retrieveSurvey(String surveyName) {
        ...
        if (rs.next()) {
            ...
            final boolean enabled = rs.getBoolean("enabled");
            final List<ResultsetColumnHeaderData> columnHeaderData = this.genericDataService.fillResultsetColumnHeaders(registeredDatatableName);
            datatableData = SurveyDataTableData.create(DatatableData.create(appTableName, registeredDatatableName, entitySubType, columnHeaderData), enabled);
        }
    ...
    public List<ClientScoresOverview> retrieveClientSurveyScoreOverview(String surveyName, Long clientId) {
        ...
        final String sql = "SELECT  tz.id, lkh.name, lkh.code, poverty_line, tz.date, tz.score FROM " + sqlGenerator.escape(registeredSurveyName) + " tz" + " JOIN ppi_likelihoods_ppi lkp on lkp.ppi_name = ? AND enabled = ? " + " JOIN ppi_scores sc on score_from  <= tz.score AND score_to >=tz.score" + " JOIN ppi_poverty_line pvl on pvl.likelihood_ppi_id = lkp.id AND pvl.score_id = sc.id" + " JOIN ppi_likelihoods lkh on lkh.id = lkp.likelihood_id " + " WHERE  client_id = ? ";
        final SqlRowSet rs = this.jdbcTemplate.queryForRowSet(sql, new Object[] {registeredSurveyName, LikelihoodStatus.ENABLED, clientId});
        List<ClientScoresOverview> scoresOverviews = new ArrayList<>();
        while (rs.next()) {
            scoresOverviews.add(new ClientScoresOverview().setLikelihoodCode(rs.getString("code")).setLikelihoodName(rs.getString("name")).setScore(rs.getLong("score")).setPovertyLine(rs.getDouble("poverty_line")).setDate(rs.getDate("date").toLocalDate()).setId(rs.getLong("id")).setSurveyName(registeredSurveyName));
        }
    ...
    public List<ClientScoresOverview> retrieveClientSurveyScoreOverview(Long clientId) {
        ...
        while (surveyNames.next()) {
            sqls.add("SELECT \'" + surveyNames.getString("name") + "\' as surveyName, tz.id, lkh.name, lkh.code, poverty_line, tz.date, tz.score FROM " + surveyNames.getString("name") + " tz" + " JOIN ppi_likelihoods_ppi lkp on lkp.ppi_name = \'" + surveyNames.getString("name") + "\' AND enabled = \'" + LikelihoodStatus.ENABLED + "\' JOIN ppi_scores sc on score_from  <= tz.score AND score_to >=tz.score" + " JOIN ppi_poverty_line pvl on pvl.likelihood_ppi_id = lkp.id AND pvl.score_id = sc.id" + " JOIN ppi_likelihoods lkh on lkh.id = lkp.likelihood_id " + " WHERE  client_id = ?");
        }
        List<ClientScoresOverview> scoresOverviews = new ArrayList<>();
        for (String sql : sqls) {
            final SqlRowSet rs = this.jdbcTemplate.queryForRowSet(sql, clientId);
            while (rs.next()) {
                scoresOverviews.add(new ClientScoresOverview().setLikelihoodCode(rs.getString("code")).setLikelihoodName(rs.getString("name")).setScore(rs.getLong("score")).setPovertyLine(rs.getDouble("poverty_line")).setDate(rs.getDate("date").toLocalDate()).setId(rs.getLong("id")).setSurveyName(rs.getString("surveyName")));
            }
        }
        return scoresOverviews;
    }
    ...
    @java.lang.SuppressWarnings("all")
        public ReadSurveyServiceImpl(final PlatformSecurityContext context, final JdbcTemplate jdbcTemplate, final SqlValidator sqlValidator, final GenericDataService genericDataService, final DatatableReadService datatableReadService, final DatabaseSpecificSQLGenerator sqlGenerator) {
