...
public class GlobalConfigurationApiResource {
    ...
    public String updateConfiguration(@PathParam("configId") @Parameter(description = "configId") final Long configId, @Parameter(hidden = true) final String apiRequestBodyAsJson) {
        final CommandWrapper commandRequest =  //
        //
        //
        new CommandWrapperBuilder().updateGlobalConfiguration(configId).withJson(apiRequestBodyAsJson).build();
    ...
    @RequestBody(required = true, content = @Content(schema = @Schema(implementation = GlobalConfigurationApiResourceSwagger.PutGlobalConfigurationsRequest.class)))
    @ApiResponse(responseCode = "200", description = "OK", content = @Content(schema = @Schema(implementation = GlobalConfigurationApiResourceSwagger.PutGlobalConfigurationsResponse.class)))
    public String updateConfigurationByName(@PathParam("configName") @Parameter(description = "configName") final String configName, @Parameter(hidden = true) final String apiRequestBodyAsJson) {
        ...
        //
        //
        new CommandWrapperBuilder().updateGlobalConfiguration(configurationData.getId()).withJson(apiRequestBodyAsJson).build();
        final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
        return this.toApiJsonSerializer.serialize(result);
    }

    @java.lang.SuppressWarnings("all")
        public GlobalConfigurationApiResource(final PlatformSecurityContext context, final ConfigurationReadPlatformService readPlatformService, final DefaultToApiJsonSerializer<GlobalConfigurationData> toApiJsonSerializer, final DefaultToApiJsonSerializer<GlobalConfigurationPropertyData> propertyDataJsonSerializer, final ApiRequestParameterHelper apiRequestParameterHelper, final PortfolioCommandSourceWritePlatformService commandsSourceWritePlatformService) {
