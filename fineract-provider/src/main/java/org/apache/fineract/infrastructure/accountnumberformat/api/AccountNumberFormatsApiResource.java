...
public class AccountNumberFormatsApiResource {
    ...
    private static final Set<String> ACCOUNT_NUMBER_FORMAT_RESPONSE_DATA_PARAMETERS = new HashSet<>(Arrays.asList(AccountNumberFormatConstants.idParamName, AccountNumberFormatConstants.accountTypeParamName, AccountNumberFormatConstants.prefixTypeParamName, AccountNumberFormatConstants.accountTypeOptionsParamName, AccountNumberFormatConstants.prefixTypeOptionsParamName));
    ...
    public String retrieveOne(@Context final UriInfo uriInfo, @PathParam("accountNumberFormatId") @Parameter(description = "accountNumberFormatId") final Long accountNumberFormatId) {
        ...
        if (settings.isTemplate()) {
            final AccountNumberFormatData templateData = this.accountNumberFormatReadPlatformService.retrieveTemplate(EntityAccountType.fromInt(accountNumberFormatData.getAccountType().getId().intValue()));
    ...
    public String create(@Parameter(hidden = true) final String apiRequestBodyAsJson) {
        ...
        //
        //
        new CommandWrapperBuilder().createAccountNumberFormat().withJson(apiRequestBodyAsJson).build(); //
    ...
    public String update(@PathParam("accountNumberFormatId") @Parameter(description = "accountNumberFormatId") final Long accountNumberFormatId, @Parameter(hidden = true) final String apiRequestBodyAsJson) {
        final CommandWrapper commandRequest =  //
        //
        //
        new CommandWrapperBuilder().updateAccountNumberFormat(accountNumberFormatId).withJson(apiRequestBodyAsJson).build(); //
        final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
        return this.toApiJsonSerializer.serialize(result);
    }
    ...
    public String delete(@PathParam("accountNumberFormatId") @Parameter(description = "accountNumberFormatId") final Long accountNumberFormatId) {
        final CommandWrapper commandRequest =  //
        //
        new CommandWrapperBuilder().deleteAccountNumberFormat(accountNumberFormatId).build(); //
        final CommandProcessingResult result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
        return this.toApiJsonSerializer.serialize(result);
    }

    @java.lang.SuppressWarnings("all")
        public AccountNumberFormatsApiResource(final PlatformSecurityContext context, final AccountNumberFormatReadPlatformService accountNumberFormatReadPlatformService, final ToApiJsonSerializer<AccountNumberFormatData> toApiJsonSerializer, final ApiRequestParameterHelper apiRequestParameterHelper, final PortfolioCommandSourceWritePlatformService commandsSourceWritePlatformService) {
