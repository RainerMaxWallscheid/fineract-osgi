...
public class LoanCommonAccountingHelper {
    ...
    public List<ChargePaymentDTO> computeNetChargePayments(final List<ChargePaymentDTO> chargePayments, final List<ChargeTaxPaymentDTO> taxPayments) {
        final Map<Long, BigDecimal> taxByChargeId = taxPayments.stream().collect(Collectors.groupingBy(ChargeTaxPaymentDTO::getLoanChargeId, Collectors.reducing(BigDecimal.ZERO, ChargeTaxPaymentDTO::getAmount, BigDecimal::add)));
    ...
    public void populateDebitAccountEntry(final Long loanProductId, final BigDecimal transactionPartAmount, final Integer debitAccountType, final Map<Integer, BigDecimal> accountMapForDebit, final Long paymentTypeId) {
        final Integer accountDebit = returnExistingDebitAccountInMapMatchingGLAccount(loanProductId, paymentTypeId, debitAccountType, accountMapForDebit);
    ...
    public Integer returnExistingDebitAccountInMapMatchingGLAccount(final Long loanProductId, final Long paymentTypeId, final Integer accountType, final Map<Integer, BigDecimal> accountMap) {
        final GLAccount glAccount = this.helper.getLinkedGLAccountForLoanProduct(loanProductId, accountType, paymentTypeId);
        return accountMap.entrySet().stream().filter(account -> this.helper.getLinkedGLAccountForLoanProduct(loanProductId, account.getKey(), paymentTypeId).getGlCode().equals(glAccount.getGlCode())).map(Map.Entry::getKey).findFirst().orElse(accountType);
    }

    @java.lang.SuppressWarnings("all")
        public LoanCommonAccountingHelper(final AccountingProcessorHelper helper) {
        this.helper = helper;
    }
}
