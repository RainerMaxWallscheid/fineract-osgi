...
class BusinessDateValidationTest {
    @java.lang.SuppressWarnings("all")
        private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(BusinessDateValidationTest.class);
    ...
    @Test
    void invalidLocale() {
        var request = BusinessDateUpdateRequest.builder().dateFormat("dd-MM-yyyy").type(BusinessDateType.BUSINESS_DATE.name()).date("12-05-2025").locale("INVALID").build();
        var errors = validator.validate(request);
    ...
    void invalidDateFormat() {
        var request = BusinessDateUpdateRequest.builder().dateFormat("dd/MM/yyyy").type(BusinessDateType.BUSINESS_DATE.name()).date("12-05-2025").locale("en").build();
        var errors = validator.validate(request);
        assertThat(errors).hasSize(1);
