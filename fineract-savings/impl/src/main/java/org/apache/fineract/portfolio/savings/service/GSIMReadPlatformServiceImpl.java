...
import org.apache.fineract.portfolio.savings.data.SavingsSummaryCustom;
...
public class GSIMReadPlatformServiceImpl implements GSIMReadPlatformService {
    ...
    private static final class GSIMFieldsMapper implements RowMapper<GroupSavingsIndividualMonitoringAccountData> {
        public String schema() {
            return "gsim.id as gsimId,sv.group_id as groupId,sv.client_id as clientId,gsim.account_number as accountNumber, sv.id as childAccountId,sv.account_no as childAccountNumber,sv.account_balance_derived as childBalance,gsim.parent_deposit as parentBalance,gsim.child_accounts_count as childAccountsCount," + "gsim.savings_status_id as savingsStatus from gsim_accounts gsim,m_savings_account sv where gsim.id=sv.gsim_id";
        }

        @Override
        public GroupSavingsIndividualMonitoringAccountData mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return GroupSavingsIndividualMonitoringAccountData.builder().gsimId(gsimId).groupId(groupId).clientId(clientId).accountNumber(accountNumber).childAccountId(childAccountId).childAccountNumber(childAccountNumber).childDeposit(parentBalance).parentDeposit(childBalance).childAccountsCount(childAccountsCount).savingsStatus(savingsStatus).build();
        }
    }
    ...
    private static final class GSIMMapper implements RowMapper<GroupSavingsIndividualMonitoringAccountData> {
        public String schema() {
            return "gsim.id as gsimId,gsim.group_id as groupId,gsim.account_number as accountNumber,gsim.parent_deposit as parentDeposit,gsim.child_accounts_count as childAccountsCount," + "gsim.savings_status_id as savingsStatus from gsim_accounts gsim";
        }
    ...
    public Collection<GSIMContainer> findGSIMAccountContainerByGroupId(Long groupId) {
        ...
        for (GroupSavingsIndividualMonitoringAccountData gsimAccount : gsimInfo) {
            ...
            gsimAccounts.add(new GSIMContainer(gsimAccount.getGsimId(), gsimAccount.getGroupId(), gsimAccount.getAccountNumber(), childSavings, gsimAccount.getParentDeposit(), gsimAccount.getSavingsStatus()));
        }
        return gsimAccounts;
    }
    ...
    public List<GSIMContainer> findGsimAccountContainerbyGsimAccountId(Long parentAccountId) {
        ...
        parentGsim.add(new GSIMContainer(gsimAccount.getGsimId(), gsimAccount.getGroupId(), gsimAccount.getAccountNumber(), childSavings, gsimAccount.getParentDeposit(), gsimAccount.getSavingsStatus()));
        return parentGsim;
    }
    ...
    private static final class SavingsAccountSummaryDataMapper implements RowMapper<SavingsSummaryCustom> {
        ...
        public SavingsSummaryCustom mapRow(final ResultSet rs, @SuppressWarnings("unused") final int rowNum) throws SQLException {
            ...
            return new SavingsSummaryCustom(id, displayName, accountNo, externalId, productId, productName, shortProductName, status, currency, accountBalance, accountTypeData, timeline, depositTypeData, subStatus, lastActiveTransactionDate);
        }
    }

    @java.lang.SuppressWarnings("all")
        public GSIMReadPlatformServiceImpl(final JdbcTemplate jdbcTemplate, final PlatformSecurityContext context, final ColumnValidator columnValidator) {
