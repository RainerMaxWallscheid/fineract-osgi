...
public class LoanScheduleGeneratorServiceImpl implements LoanScheduleGeneratorService {
    ...
    public BigDecimal calculateInteresOnlyWithFirtDisbursement(final Long loanId) {
        ...
        final BigDecimal disbursedAmount = loan.getDisbursementDetails().isEmpty() ? loan.getPrincipal().getAmount() : loan.getDisbursementDetails().get(0).getPrincipal();
        final LoanRepaymentScheduleModelData modelData = new LoanRepaymentScheduleModelData(disbursementDate, loan.getCurrency().toData(), disbursedAmount, disbursementDate, loanProductRelatedDetail.getNumberOfRepayments(),  //
