...
public final class ProgressiveEMICalculator implements EMICalculator {
    ...
    @Override
    @NotNull
    public ProgressiveLoanInterestScheduleModel generatePeriodInterestScheduleModel(@NotNull List<LoanScheduleModelRepaymentPeriod> periods, @NotNull ILoanConfigurationDetails loanProductRelatedDetail, final Integer installmentAmountInMultiplesOf, final MathContext mc) {
        return generateInterestScheduleModel(periods, LoanScheduleModelRepaymentPeriod::periodFromDate, LoanScheduleModelRepaymentPeriod::periodDueDate, loanProductRelatedDetail, installmentAmountInMultiplesOf, mc);
    }

    @Override
    @NotNull
    public ProgressiveLoanInterestScheduleModel generateInstallmentInterestScheduleModel(@NotNull List<RepaymentScheduleInstallmentData> installments, @NotNull ILoanConfigurationDetails loanProductRelatedDetail, final Integer installmentAmountInMultiplesOf, final MathContext mc) {
        installments = installments.stream().filter(installment -> !installment.isDownPayment() && !installment.isAdditional()).toList();
        return generateInterestScheduleModel(installments, RepaymentScheduleInstallmentData::getFromDate, RepaymentScheduleInstallmentData::getDueDate, loanProductRelatedDetail, installmentAmountInMultiplesOf, mc);
    }

    @NotNull
    private <T> ProgressiveLoanInterestScheduleModel generateInterestScheduleModel(@NotNull List<T> periods, Function<T, LocalDate> from, Function<T, LocalDate> to, @NotNull ILoanConfigurationDetails loanProductRelatedDetail, final Integer installmentAmountInMultiplesOf, final MathContext mc) {
    ...
    @Override
    public Optional<RepaymentPeriod> findRepaymentPeriod(final ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate repaymentPeriodFromDate, final LocalDate repaymentPeriodDueDate) {
    ...
    private void addDisbursement(final ProgressiveLoanInterestScheduleModel scheduleModel, final EmiChangeOperation operation) {
        scheduleModel.repaymentPeriods().stream().filter(rp -> !operation.getSubmittedOnDate().isAfter(rp.getFromDate())).forEach(rp -> rp.setTotalDisbursedAmount(rp.getTotalDisbursedAmount().add(operation.getAmount())));
        final int numberOfRepayments = scheduleModel.loanProductRelatedDetail().getNumberOfRepayments();
        if (scheduleModel.loanProductRelatedDetail().isAllowFullTermForTranche() && numberOfRepayments > 0 && operation.getAction().equals(EmiChangeOperation.Action.DISBURSEMENT)) {
            addFullTermTrancheDisbursement(scheduleModel, operation);
        } else {
            scheduleModel.changeOutstandingBalanceAndUpdateInterestPeriods(operation.getSubmittedOnDate(), operation.getAmount(), scheduleModel.zero(), scheduleModel.zero()).ifPresent(repaymentPeriod -> calculateEMIValueAndRateFactors(getEffectiveRepaymentDueDate(scheduleModel, repaymentPeriod, operation.getSubmittedOnDate()), scheduleModel, operation));
        }
    }

    private void addFullTermTrancheDisbursement(final ProgressiveLoanInterestScheduleModel scheduleModel, final EmiChangeOperation operation) {
        ...
        final Optional<RepaymentPeriod> firstDisbursedPeriod = scheduleModel.repaymentPeriods().stream().filter(period -> isInPeriodFromInclusiveToExclusive(disbursementDate, period.getFromDate(), period.getDueDate())).findFirst();
        final LocalDate firstDisbursedPeriodStartDate = firstDisbursedPeriod.isPresent() ? firstDisbursedPeriod.get().getFromDate() : scheduleModel.getMaturityDate();
        final LoanApplicationTerms loanApplicationTerms = buildLoanApplicationTerms(loanProductRelatedDetail, firstDisbursedPeriodStartDate, disbursedAmount, mc);
        final ProgressiveLoanInterestScheduleModel temporaryReAgedScheduleModel = generateTemporaryScheduleModel(loanApplicationTerms, mc, firstDisbursedPeriodStartDate, disbursementDate);
        mergeNewScheduleModelWithExistingOne(scheduleModel, temporaryReAgedScheduleModel, operation);
    }

    private LoanApplicationTerms buildLoanApplicationTerms(final ILoanConfigurationDetails loanProductRelatedDetail, final LocalDate firstDisbursedPeriodStartDate, final Money disbursedAmount, final MathContext mc) {
        return 
        // Loan basics
        // Term & frequency
        ...
        // Flags & options
        // Technical
        new LoanApplicationTerms.Builder().currency(loanProductRelatedDetail.getCurrencyData()).principal(disbursedAmount).repaymentsStartingFromDate(firstDisbursedPeriodStartDate).downPaymentPercentage(ZERO).seedDate(firstDisbursedPeriodStartDate).inArrearsTolerance(Money.zero(loanProductRelatedDetail.getCurrencyData())).loanTermFrequency(loanProductRelatedDetail.getNumberOfRepayments()).loanTermPeriodFrequencyType(loanProductRelatedDetail.getRepaymentPeriodFrequencyType()).numberOfRepayments(loanProductRelatedDetail.getNumberOfRepayments()).repaymentEvery(loanProductRelatedDetail.getRepayEvery()).repaymentPeriodFrequencyType(loanProductRelatedDetail.getRepaymentPeriodFrequencyType()).interestRatePerPeriod(loanProductRelatedDetail.getAnnualNominalInterestRate()).annualNominalInterestRate(loanProductRelatedDetail.getAnnualNominalInterestRate()).interestRatePeriodFrequencyType(loanProductRelatedDetail.getRepaymentPeriodFrequencyType()).interestMethod(loanProductRelatedDetail.getInterestMethod()).interestRecognitionOnDisbursementDate(loanProductRelatedDetail.isInterestRecognitionOnDisbursementDate()).daysInMonthType(DaysInMonthType.fromInt(loanProductRelatedDetail.getDaysInMonthType())).daysInYearType(DaysInYearType.fromInt(loanProductRelatedDetail.getDaysInYearType())).daysInYearCustomStrategy(loanProductRelatedDetail.getDaysInYearCustomStrategy()).isDownPaymentEnabled(false).allowPartialPeriodInterestCalculation(loanProductRelatedDetail.isAllowPartialPeriodInterestCalculation()).mc(mc).build();
    }

    private void mergeNewScheduleModelWithExistingOne(final ProgressiveLoanInterestScheduleModel scheduleModel, final ProgressiveLoanInterestScheduleModel temporaryScheduleModel, final EmiChangeOperation operation) {
        ...
        scheduleModel.changeOutstandingBalanceAndUpdateInterestPeriods(operation.getSubmittedOnDate(), operation.getAmount(), scheduleModel.zero(), scheduleModel.zero());
        for (final RepaymentPeriod newPeriod : newPeriods) {
            ...
            final Optional<RepaymentPeriod> existingRepaymentPeriod = existingRepaymentPeriods.stream().filter(ep -> ep.getFromDate().isEqual(newPeriod.getFromDate()) && ep.getDueDate().equals(newPeriod.getDueDate())).findFirst();
            if (existingRepaymentPeriod.isPresent()) {
                ...
                final RepaymentPeriod rp = RepaymentPeriod.create(!existingRepaymentPeriods.isEmpty() ? existingRepaymentPeriods.getLast() : null, (newPeriod.equals(newPeriods.getFirst()) && !existingRepaymentPeriods.isEmpty()) ? existingRepaymentPeriods.getLast().getDueDate() : newPeriod.getFromDate(), newPeriod.getDueDate(), newPrincipal.add(newInterest), scheduleModel.mc(), scheduleModel.loanProductRelatedDetail());
    ...
    private LocalDate getEffectiveRepaymentDueDate(final ProgressiveLoanInterestScheduleModel scheduleModel, final RepaymentPeriod changedRepaymentPeriod, final LocalDate operationDueDate) {
        ...
        if (isRelatedToNextRepaymentPeriod) {
            final Optional<RepaymentPeriod> nextRepaymentPeriod = scheduleModel.repaymentPeriods().stream().filter(repaymentPeriod -> changedRepaymentPeriod.equals(repaymentPeriod.getPrevious().orElse(null))).findFirst();
     ...
     */
    @Override
    public void addCapitalizedIncome(final ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate transactionDueDate, final Money transactionAmount) {
    ...
    private void addCapitalizedIncome(final ProgressiveLoanInterestScheduleModel scheduleModel, final EmiChangeOperation operation) {
        scheduleModel.repaymentPeriods().stream().filter(rp -> !operation.getSubmittedOnDate().isAfter(rp.getFromDate())).forEach(rp -> rp.setTotalCapitalizedIncomeAmount(rp.getTotalCapitalizedIncomeAmount().plus(operation.getAmount())));
        scheduleModel.changeOutstandingBalanceAndUpdateInterestPeriods(operation.getSubmittedOnDate(), scheduleModel.zero(), scheduleModel.zero(), operation.getAmount()).ifPresent(repaymentPeriod -> {
            calculateEMIValueAndRateFactors(getEffectiveRepaymentDueDate(scheduleModel, repaymentPeriod, operation.getSubmittedOnDate()), scheduleModel, operation);
        });
    }

    @Override
    public void changeInterestRate(final ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate newInterestSubmittedOnDate, final BigDecimal newInterestRate) {
        changeInterestRate(scheduleModel, EmiChangeOperation.changeInterestRate(newInterestSubmittedOnDate, newInterestRate));
    }

    @Override
    public void addRepaymentPeriods(final ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate submittedOnDate, final int numberOfRepaymentPeriodsToAdd, final List<ProcessedTransactionData> alreadyProcessedTransactions) {
        addRepaymentPeriods(scheduleModel, EmiChangeOperation.addRepaymentPeriods(submittedOnDate, scheduleModel.zero(), numberOfRepaymentPeriodsToAdd), alreadyProcessedTransactions);
    }

    private void addRepaymentPeriods(final ProgressiveLoanInterestScheduleModel scheduleModel, final EmiChangeOperation operation, final List<ProcessedTransactionData> alreadyProcessedTransactions) {
        final Optional<LoanReAgeParameterData> reAgeTransactionParameter = alreadyProcessedTransactions.stream().filter(ProcessedTransactionData::isReAge).findFirst().flatMap(ProcessedTransactionData::getReAgeParameterOptional);
        ...
        final int repaymentPeriodCount = reAgeTransactionParameter.map(param -> param.getNumberOfInstallments() - 1).orElse(scheduleModel.repaymentPeriods().size());
        final LocalDate interestRateChangeEffectiveDate = operation.getSubmittedOnDate().minusDays(1);
        final List<LocalDateInterval> periods2 = generateAdditionalRepaymentPeriodDueDates(scheduleModel, operation.getPeriodsToAdd(), repaymentPeriodCount, scheduleModel.resolveRepaymentPeriodLengthGeneratorFunction(seedDate));
        updateModel(scheduleModel, periods2, LocalDateInterval::startDate, LocalDateInterval::endDate);
        scheduleModel.changeOutstandingBalanceAndUpdateInterestPeriods(interestRateChangeEffectiveDate, scheduleModel.zero(), scheduleModel.zero(), scheduleModel.zero()).ifPresent(repaymentPeriod -> calculateEMIValueAndRateFactors(getEffectiveRepaymentDueDate(scheduleModel, repaymentPeriod, interestRateChangeEffectiveDate), scheduleModel, operation));
    }

    public <T> void updateModel(ProgressiveLoanInterestScheduleModel scheduleModel, List<T> updateExpectedRepaymentPeriods, Function<T, LocalDate> from, Function<T, LocalDate> to) {
        final ILoanConfigurationDetails loanProductRelatedDetail = scheduleModel.loanProductRelatedDetail();
    ...
    List<LocalDateInterval> generateAdditionalRepaymentPeriodDueDates(final ProgressiveLoanInterestScheduleModel scheduleModel, final int periods, final int existingRepayments, Function<Long, LocalDate> repaymentPeriodLengthResolver) {
        ...
        IntStream.range(existingRepayments, existingRepayments + periods).forEach(i -> expectedRepaymentPeriods.add(LocalDateInterval.create(repaymentPeriodLengthResolver.apply((long) i * repayEvery), repaymentPeriodLengthResolver.apply((long) (i + 1) * repayEvery))));
    ...
    @Override
    public void addBalanceCorrection(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate balanceCorrectionDate, Money balanceCorrectionAmount) {
        scheduleModel.changeOutstandingBalanceAndUpdateInterestPeriods(balanceCorrectionDate, scheduleModel.zero(), balanceCorrectionAmount, scheduleModel.zero()).ifPresent(repaymentPeriod -> {
    ...
    public void addOverdueBalanceCorrection(final ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate correctionDate, final Money overdueAmount) {
        scheduleModel.changeOutstandingBalanceAndUpdateInterestPeriods(correctionDate, scheduleModel.zero(), overdueAmount, scheduleModel.zero()).ifPresent(repaymentPeriod -> {
    ...
    @Override
    public void payInterest(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate repaymentPeriodFromDate, LocalDate repaymentPeriodDueDate, LocalDate transactionDate, Money interestAmount) {
        ...
        Optional<RepaymentPeriod> latestNotLastOpenRepaymentPeriodBeforeDate = getLatestNotLastOpenRepaymentPeriodBeforeDate(scheduleModel, transactionDate);
        if (latestNotLastOpenRepaymentPeriodBeforeDate.isPresent() && repaymentPeriod.equals(latestNotLastOpenRepaymentPeriodBeforeDate)) {
            calculateUnrecognizedInterestTillDateOnScheduleModelCopyAndDefer(scheduleModel, latestNotLastOpenRepaymentPeriodBeforeDate.get(), transactionDate);
        }
    ...
    @Override
    public void payPrincipal(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate repaymentPeriodFromDate, LocalDate repaymentPeriodDueDate, LocalDate transactionDate, Money principalAmount) {
        ...
        if (scheduleModel.getTotalPaidPrincipal().plus(principalAmount).isGreaterThan(scheduleModel.getTotalDuePrincipal().plus(scheduleModel.getTotalCreditedPrincipal()))) {
        ...
        LocalDate balanceCorrectionDate = DateUtils.isBefore(repaymentPeriodDueDate, transactionDate) ? repaymentPeriodDueDate : transactionDate;
        ...
        if (scheduleModel.isEMIRecalculationEnabled()) {
            repaymentPeriod.ifPresent(rp -> {
                ...
                if (multiplePeriodsAreUnpaid) {
                    ...
                    if (transactionDateIsBefore && rp.getTotalPaidAmount().isGreaterThan(rp.getEmiPlusCreditedAmountsPlusFutureUnrecognizedInterest())) {
                        rp.setEmi(rp.getTotalPaidAmount().minus(rp.getTotalCreditedAmount()));
                    } else if (transactionDateIsBefore && rp.getTotalPaidAmount().isEqualTo(rp.getOriginalEmi().add(rp.getTotalCreditedAmount()))) {
                        rp.setEmi(rp.getTotalPaidAmount().minus(rp.getTotalCreditedAmount()));
                    }
                }
                calculateLastUnpaidRepaymentPeriodEMI(scheduleModel, balanceCorrectionDate);
    ...
    private Optional<RepaymentPeriod> getLatestNotLastOpenRepaymentPeriodBeforeDate(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate transactionDate) {
        List<RepaymentPeriod> unpaidRepaymentPeriods =  //
        //
        //
        scheduleModel.repaymentPeriods().stream().filter(rp -> !rp.isFullyPaid()).toList(); //
        if (CollectionUtils.isEmpty(unpaidRepaymentPeriods) || unpaidRepaymentPeriods.getLast().equals(scheduleModel.repaymentPeriods().getLast())) {
            return Optional.empty();
        }
    ...
    private void addCreditedAmountsToInterestPeriod(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate transactionDate, Money creditedPrincipalAmount, Money creditedInterestAmount) {
        scheduleModel.repaymentPeriods().stream().filter(checkRepaymentPeriodIsInCreditRange(scheduleModel, transactionDate)).findFirst().flatMap(repaymentPeriod -> repaymentPeriod.getInterestPeriods().stream().filter(interestPeriod -> interestPeriod.getFromDate().equals(transactionDate)).reduce((v1, v2) -> v2)).ifPresent(interestPeriod -> {
    ...
    @Nonnull
    private static Predicate<RepaymentPeriod> checkRepaymentPeriodIsInCreditRange(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate transactionDate) {
        return repaymentPeriod -> scheduleModel.isLastRepaymentPeriod(repaymentPeriod) ? !transactionDate.isBefore(repaymentPeriod.getFromDate()) && !transactionDate.isAfter(repaymentPeriod.getDueDate()) : !transactionDate.isBefore(repaymentPeriod.getFromDate()) && transactionDate.isBefore(repaymentPeriod.getDueDate());
    }

    @Override
    public void creditPrincipal(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate transactionDate, Money creditedPrincipalAmount) {
        addCredit(scheduleModel, transactionDate, creditedPrincipalAmount, scheduleModel.zero());
    }

    @Override
    public void creditInterest(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate transactionDate, Money creditedInterestAmount) {
        addCredit(scheduleModel, transactionDate, scheduleModel.zero(), creditedInterestAmount);
    }

    private void addCredit(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate transactionDate, Money creditedPrincipalAmount, Money creditedInterestAmount) {
        scheduleModel.changeOutstandingBalanceAndUpdateInterestPeriods(transactionDate, scheduleModel.zero(), creditedPrincipalAmount, scheduleModel.zero()).ifPresent(repaymentPeriod -> {
    ...
    @Override
    @NotNull
    public PeriodDueDetails getDueAmounts(@NotNull ProgressiveLoanInterestScheduleModel scheduleModel, @NotNull LocalDate periodFromDate, @NotNull LocalDate periodDueDate, @NotNull LocalDate targetDate) {
        ProgressiveLoanInterestScheduleModel recalculatedScheduleModelTillDate = recalculateScheduleModelTillDate(scheduleModel, targetDate);
        RepaymentPeriod repaymentPeriod = recalculatedScheduleModelTillDate.findRepaymentPeriodByFromAndDueDate(periodFromDate, periodDueDate).orElseThrow();
        long notFullyRepaidRepaymentPeriodCount = recalculatedScheduleModelTillDate.repaymentPeriods().stream().filter(rp -> !rp.isFullyPaid()).count();
        boolean isVertical = scheduleModel.loanProductRelatedDetail().getLoanScheduleProcessingType() == LoanScheduleProcessingType.VERTICAL;
        adjustEmiIfRequired(repaymentPeriod, recalculatedScheduleModelTillDate, targetDate, notFullyRepaidRepaymentPeriodCount);
        Money duePrincipal = isVertical && notFullyRepaidRepaymentPeriodCount > 1 ? repaymentPeriod.getEmiPlusCreditedAmountsPlusFutureUnrecognizedInterest() : repaymentPeriod.getDuePrincipal();
        ...
        Money calculatedDuePrincipal = capDueAmount(duePrincipal, recalculatedScheduleModelTillDate.getTotalDuePrincipal(), recalculatedScheduleModelTillDate.getTotalPaidPrincipal(), repaymentPeriod.getPaidPrincipal());
        Money calculatedDueInterest = capDueAmount(dueInterest, recalculatedScheduleModelTillDate.getTotalDueInterest(), recalculatedScheduleModelTillDate.getTotalPaidInterest(), repaymentPeriod.getPaidInterest());
        return new PeriodDueDetails(repaymentPeriod.getEmi(), calculatedDuePrincipal, calculatedDueInterest);
    }

    private void adjustEmiIfRequired(RepaymentPeriod repaymentPeriod, ProgressiveLoanInterestScheduleModel recalculatedScheduleModelTillDate, LocalDate targetDate, long notFullyRepaidRepaymentPeriodCount) {
        ...
        if (notFullyRepaidRepaymentPeriodCount > 1L) {
            ...
            Money emiAdjustment = MathUtil.negativeToZero(repaymentPeriod.getOutstandingPrincipal().minus(recalculatedScheduleModelTillDate.getTotalOutstandingPrincipal()));
        ...
        if (repaymentPeriod.isFullyPaid() && notFullyRepaidRepaymentPeriodCount == 1L) {
            repaymentPeriod.setEmi(MathUtil.min(repaymentPeriod.getOriginalEmi(), recalculatedScheduleModelTillDate.getTotalDuePrincipal().minus(recalculatedScheduleModelTillDate.getTotalPaidPrincipal()).add(repaymentPeriod.getPaidPrincipal()).add(repaymentPeriod.getDueInterest()), false));
        }
    }
    ...
    @Override
    @NotNull
    public Money getPeriodInterestTillDate(@NotNull final ProgressiveLoanInterestScheduleModel scheduleModel, @NotNull final LocalDate periodFromDate, @NotNull final LocalDate periodDueDate, @NotNull final LocalDate targetDate, final boolean includeCreditedInterest, final boolean fixedInterestTillDate) {
        final ProgressiveLoanInterestScheduleModel recalculatedScheduleModelTillDate = recalculateScheduleModelTillDate(scheduleModel, targetDate);
        final MathContext mc = recalculatedScheduleModelTillDate.mc();
        final RepaymentPeriod repaymentPeriod = recalculatedScheduleModelTillDate.findRepaymentPeriodByFromAndDueDate(periodFromDate, periodDueDate).orElseThrow();
        Money calculatedDueInterest = repaymentPeriod.getCalculatedDueInterest();
        if (fixedInterestTillDate) {
            calculatedDueInterest = MathUtil.negativeToZero(calculatedDueInterest.minus(repaymentPeriod.getFixedInterest()).add(repaymentPeriod.calculateFixedInterestTillDate()), mc);
        }
    ...
    public OutstandingDetails getOutstandingAmountsTillDate(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate targetDate) {
        ...
        Money totalOutstandingPrincipal = MathUtil.negativeToZero(scheduleModelCopy.getTotalDuePrincipal().minus(scheduleModelCopy.getTotalPaidPrincipal()));
        Money totalOutstandingInterest = MathUtil.negativeToZero(scheduleModelCopy.getTotalDueInterest().minus(scheduleModelCopy.getTotalPaidInterest()));
    ...
    @NotNull
    private ProgressiveLoanInterestScheduleModel recalculateScheduleModelTillDate(@NotNull ProgressiveLoanInterestScheduleModel scheduleModel, @NotNull LocalDate targetDate) {
        MathContext mc = scheduleModel.mc();
        ProgressiveLoanInterestScheduleModel scheduleModelCopy = scheduleModel.deepCopy(mc);
                ...
                if (rp.getDueDate().isAfter(targetDate)) {
                    if (rp.equals(repaymentPeriod)) {
                        rp.findInterestPeriod(targetDate).ifPresent(ip -> {
                            ...
                            int nextIdx = index + 1;
                            boolean thereIsInterestPeriodFromDateOnTargetDate = ip.getRepaymentPeriod().getInterestPeriods().size() > nextIdx;
                            ...
                            ip.getRepaymentPeriod().getInterestPeriods().subList(nextIdx, ip.getRepaymentPeriod().getInterestPeriods().size()).clear();
                        });
                    } else if (rp.getPrevious().isPresent() && rp.getPrevious().get().equals(repaymentPeriod) && (rp.getInterestPeriods().getFirst().getCreditedInterest().isGreaterThanZero() || rp.getInterestPeriods().getFirst().getCreditedPrincipal().isGreaterThanZero())) {
    ...
    private void calculateEMIValueAndRateFactorsForFlatInterestMethod(final LocalDate calculateFromRepaymentPeriodDueDate, final ProgressiveLoanInterestScheduleModel scheduleModel, final EmiChangeOperation operation) {
     ...
     */
    private void calculateEMIValueAndRateFactors(final LocalDate calculateFromRepaymentPeriodDueDate, final ProgressiveLoanInterestScheduleModel scheduleModel, final EmiChangeOperation operation) {
        switch (scheduleModel.loanProductRelatedDetail().getInterestMethod()) {
            case FLAT -> calculateEMIValueAndRateFactorsForFlatInterestMethod(calculateFromRepaymentPeriodDueDate, scheduleModel, operation);
            case DECLINING_BALANCE -> calculateEMIValueAndRateFactorsForDecliningBalanceInterestMethod(calculateFromRepaymentPeriodDueDate, scheduleModel, operation);
            default -> throw new UnsupportedOperationException("Unsupported interest method: " + scheduleModel.loanProductRelatedDetail().getInterestMethod());
        }
    }

    private void calculateEMIValueAndRateFactorsForDecliningBalanceInterestMethod(final LocalDate calculateFromRepaymentPeriodDueDate, final ProgressiveLoanInterestScheduleModel scheduleModel, final EmiChangeOperation operation) {
        final List<RepaymentPeriod> relatedRepaymentPeriods = scheduleModel.getRelatedRepaymentPeriods(calculateFromRepaymentPeriodDueDate);
        final boolean onlyOnActualModelShouldApply = scheduleModel.isEmpty() || operation.getAction() == EmiChangeOperation.Action.INTEREST_RATE_CHANGE || operation.getAction() == EmiChangeOperation.Action.ADD_REPAYMENT_PERIODS || scheduleModel.isCopy();
        applyInterestMoratoriumIfRequired(scheduleModel);
        calculateRateFactorForPeriods(relatedRepaymentPeriods, scheduleModel);
        calculateOutstandingBalance(scheduleModel);
    ...
    @Override
    public void updateModelRepaymentPeriodsDuringReAge(final ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate reAgePeriodStartDate, final LocalDate reAgeFirstDueDate, final LocalDate targetDate, final LoanApplicationTerms loanApplicationTerms, final MathContext mc) {
        final Money futureCreditedPrincipals = scheduleModel.repaymentPeriods().stream().filter(rp -> !rp.getFromDate().isBefore(targetDate)).filter(rp -> rp.getDueDate().isAfter(targetDate)).map(RepaymentPeriod::getCreditedPrincipal).reduce(scheduleModel.zero(), Money::add);
        ...
        final ProgressiveLoanInterestScheduleModel temporaryReAgedScheduleModel = generateTemporaryScheduleModel(loanApplicationTerms, mc, reAgePeriodStartDate, reAgePeriodStartDate);
        attachTemporaryScheduleModelReAgedPeriodsToExistingModel(scheduleModel, temporaryReAgedScheduleModel, reAgeFirstDueDate, targetDate, paidBalancesFromTransactionDate);
    }

    @Override
    public boolean recalculateModelOverdueAmountsTillDate(final ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate targetDate, boolean prepayAttempt) {
        boolean hasChange = false;
        LocalDate recalculatedTargetDate = DateUtils.isAfter(targetDate, scheduleModel.getLastRepaymentPeriod().getDueDate()) ? scheduleModel.getLastRepaymentPeriod().getDueDate() : targetDate;
        final List<RepaymentPeriod> overdueInstallmentsSortedByInstallmentNumber = findPossiblyOverdueRepaymentPeriods(recalculatedTargetDate, scheduleModel);
        if (!overdueInstallmentsSortedByInstallmentNumber.isEmpty()) {
            ...
            for (RepaymentPeriod processingPeriod : overdueInstallmentsSortedByInstallmentNumber) {
                ...
                if (!overDuePrincipal.isZero()) {
                    final boolean currentChanges = adjustOverduePrincipal(recalculatedTargetDate, processingPeriod, overDuePrincipal, aggregatedOverDuePrincipal, scheduleModel, prepayAttempt);
            ...
            if (!currentPeriod.equals(lastPeriod) || !recalculatedTargetDate.isAfter(lastPeriod.getDueDate())) {
                final boolean currentChanges = adjustOverduePrincipal(recalculatedTargetDate, currentPeriod, overDuePrincipal, aggregatedOverDuePrincipal, scheduleModel, prepayAttempt);
                hasChange = hasChange || currentChanges;
            }
            if (aggregatedOverDuePrincipal.isGreaterThanZero() && (scheduleModel.lastOverdueBalanceChange() == null || scheduleModel.lastOverdueBalanceChange().isBefore(recalculatedTargetDate))) {
    ...
    public Money getOutstandingInterestTillDate(final ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate tillDate) {
        final ProgressiveLoanInterestScheduleModel recalculatedScheduleModelTillDate = recalculateScheduleModelTillDate(scheduleModel, tillDate);
        return recalculatedScheduleModelTillDate.repaymentPeriods().stream().filter(repaymentPeriod -> repaymentPeriod.getFromDate().isBefore(tillDate)).map(RepaymentPeriod::getOutstandingInterest).reduce(scheduleModel.zero(), Money::add);
    }

    @Override
    public void changeDueDate(final ProgressiveLoanInterestScheduleModel scheduleModel, final LoanApplicationTerms loanApplicationTerms, final LocalDate targetRepaymentPeriodDueDate, LocalDate newDueDate) {
        ...
        for (int i = targetRepaymentPeriodIndex + 1; i < scheduleModel.repaymentPeriods().size(); ++i) {
            ...
            if (i == lastRepaymentPeriodIndex && loanApplicationTerms.getFixedLength() != null) {
                nextRepaymentDate = calculateDateForFixedLength(scheduleModel, loanApplicationTerms, targetRepaymentPeriodDueDate, originalNewDueDate);
            }
    ...
    @Override
    public void updateModelRepaymentPeriodsDuringReAmortization(final ProgressiveLoanInterestScheduleModel model, final LocalDate transactionDate) {
        moveOutstandingAmountsFromPeriodsBeforeTransactionDate(model.repaymentPeriods(), transactionDate);
        final List<RepaymentPeriod> reAmortizedPeriods = model.repaymentPeriods().stream().filter(rp -> rp.getDueDate().isAfter(transactionDate)).toList();
    ...
    @Override
    public void updateModelRepaymentPeriodsDuringReAmortizationWithEqualInterestSplit(final ProgressiveLoanInterestScheduleModel model, final LocalDate transactionDate) {
        final MathContext mc = model.mc();
        final List<RepaymentPeriod> periodsBeforeTransactionDate = model.repaymentPeriods().stream().filter(rp -> !rp.getDueDate().isAfter(transactionDate) && !rp.isFullyPaid()).toList();
        ...
        final List<RepaymentPeriod> futurePeriods = model.repaymentPeriods().stream().filter(rp -> rp.getDueDate().isAfter(transactionDate)).toList();
    ...
    private void moveOutstandingAmountsFromPeriodsBeforeTransactionDateForEqualInterestSplit(final List<RepaymentPeriod> periodsBeforeTransactionDate, final LocalDate transactionDate) {
    ...
    private LocalDate calculateDateForFixedLength(final ProgressiveLoanInterestScheduleModel scheduleModel, final LoanApplicationTerms loanApplicationTerms, final LocalDate targetRepaymentPeriodDueDate, final LocalDate newDueDate) {
        ...
        return switch (loanApplicationTerms.getRepaymentPeriodFrequencyType()) {
            ...
            case INVALID, WHOLE_TERM -> throw new IllegalArgumentException("Unsupported period frequency type for fixed length: " + loanApplicationTerms.getRepaymentPeriodFrequencyType());
        };
    }

    private List<RepaymentPeriod> findPossiblyOverdueRepaymentPeriods(final LocalDate targetDate, final ProgressiveLoanInterestScheduleModel model) {
        return  //
        model.repaymentPeriods().stream().filter(repaymentPeriod -> !repaymentPeriod.isReAgedEarlyRepaymentHolder() && DateUtils.isAfter(targetDate, repaymentPeriod.getDueDate())).toList();
    }

    private boolean adjustOverduePrincipal(final LocalDate currentDate, final RepaymentPeriod currentInstallment, final Money overduePrincipal, final Money aggregatedOverDuePrincipal, final ProgressiveLoanInterestScheduleModel model, boolean prepayAttempt) {
    ...
    private boolean shouldRecalculateTillInstallmentDueDate(final ILoanConfigurationDetails loanProductRelatedDetails, final boolean isPrepayAttempt) {
        ...
        return switch (loanProductRelatedDetails.getRestFrequencyType()) {
            case DAILY -> isPrepayAttempt && loanProductRelatedDetails.getPreCloseInterestCalculationStrategy().calculateTillRestFrequencyEnabled();
            case SAME_AS_REPAYMENT_PERIOD -> loanProductRelatedDetails.getPreCloseInterestCalculationStrategy().calculateTillRestFrequencyEnabled();
    ...
    private void attachTemporaryScheduleModelReAgedPeriodsToExistingModel(final ProgressiveLoanInterestScheduleModel scheduleModel, final ProgressiveLoanInterestScheduleModel temporaryReAgedScheduleModel, final LocalDate reAgeFirstDueDate, final LocalDate targetDate, final OutstandingDetails paidBalancesFromTransactionDate) {
        ...
        if (!paidBalancesFromTransactionDate.getOutstandingInterest().isZero() || !paidBalancesFromTransactionDate.getOutstandingPrincipal().isZero()) {
            createRepaymentPeriodForEarlyRepaidAmountsDuringReAgeing(scheduleModel, paidBalancesFromTransactionDate.getOutstandingPrincipal(), paidBalancesFromTransactionDate.getOutstandingInterest(), false);
        }
        for (final RepaymentPeriod newPeriod : newPeriods) {
            final Money newPrincipal = newPeriod.getDuePrincipal();
            final Money newInterest = newPeriod.getDueInterest();
        ...
        final List<RepaymentPeriod> reAgedRepaymentPeriods = existingRepaymentPeriods.stream().filter(repaymentPeriod -> (repaymentPeriod.getFromDate().isEqual(targetDate) && repaymentPeriod.getDueDate().isEqual(reAgeFirstDueDate)) || !repaymentPeriod.getFromDate().isBefore(reAgeFirstDueDate)).toList();
        calculateOutstandingBalance(scheduleModel);
     ...
     */
    @NotNull
    private ProgressiveLoanInterestScheduleModel generateTemporaryScheduleModel(final LoanApplicationTerms loanApplicationTerms, final MathContext mc, final LocalDate periodStartDate, final LocalDate disbursementDate) {
        final List<LoanScheduleModelRepaymentPeriod> expectedRepaymentPeriods = scheduledDateGenerator.generateRepaymentPeriods(mc, periodStartDate, loanApplicationTerms, null);
        final ProgressiveLoanInterestScheduleModel temporaryReAgedScheduleModel = generatePeriodInterestScheduleModel(expectedRepaymentPeriods, loanApplicationTerms.toLoanConfigurationDetails(), loanApplicationTerms.getInstallmentAmountInMultiplesOf(), mc);
     ...
     */
    private static void moveOutstandingAmountsFromPeriodsBeforeTransactionDate(final List<RepaymentPeriod> existingRepaymentPeriods, final LocalDate transactionDate) {
        final List<RepaymentPeriod> periodsBeforeTransactionDate = existingRepaymentPeriods.stream().filter(rp -> !rp.getFromDate().isAfter(transactionDate) && !rp.isFullyPaid()).toList();
        periodsBeforeTransactionDate.forEach(rp -> {
            rp.getInterestPeriods().stream().filter(ip -> ip.getDueDate().isEqual(transactionDate)).forEach(ip -> {
                if (!ip.getBalanceCorrectionAmount().isZero()) {
                    ip.addBalanceCorrectionAmount(ip.getBalanceCorrectionAmount().negated());
    ...
    private void calculateLastUnpaidRepaymentPeriodEMI(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate tillDate) {
        ...
        Optional<RepaymentPeriod> findLastUnpaidRepaymentPeriod = scheduleModel.repaymentPeriods().stream().filter(rp -> !rp.isFullyPaid()).reduce((first, second) -> second);
        if (findLastUnpaidRepaymentPeriod.isEmpty() && scheduleModel.getTotalPaidPrincipal().isZero()) {
            findLastUnpaidRepaymentPeriod = scheduleModel.repaymentPeriods().stream().reduce((first, second) -> second).filter(rp -> !rp.getInterestPeriods().isEmpty()).filter(rp -> rp.getOutstandingLoanBalance().isGreaterThanZero());
        }
        findLastUnpaidRepaymentPeriod.ifPresent(repaymentPeriod -> {
            ...
            MathContext mc = scheduleModel.mc();
            Money totalDueInterest = scheduleModel.repaymentPeriods().stream().map(RepaymentPeriod::getDueInterest).reduce(scheduleModel.zero(), (m1, m2) -> m1.plus(m2, mc)); // 1.46
            Money totalEMI = scheduleModel.repaymentPeriods().stream().map(RepaymentPeriod::getEmiPlusCreditedAmountsPlusFutureUnrecognizedInterest).reduce(scheduleModel.zero(), (m1, m2) -> m1.plus(m2, mc)); // 101.48
            Money totalDisbursedAmount = scheduleModel.repaymentPeriods().stream().flatMap(rp -> rp.getInterestPeriods().stream().map(InterestPeriod::getDisbursementAmount)).reduce(scheduleModel.zero(), (m1, m2) -> m1.plus(m2, mc)); // 100
            Money totalCapitalizedIncome = scheduleModel.repaymentPeriods().stream().flatMap(rp -> rp.getInterestPeriods().stream().map(InterestPeriod::getCapitalizedIncomePrincipal)).reduce(scheduleModel.zero(), (m1, m2) -> m1.plus(m2, mc)); // 100
            Money diff = totalDisbursedAmount.plus(totalCapitalizedIncome, mc).plus(scheduleModel.getTotalCreditedPrincipal(), mc).plus(totalDueInterest, mc).minus(totalEMI, mc);
            Money adjustedEmi = repaymentPeriod.getEmi().add(diff, mc);
            if (repaymentPeriod.getFixedInterest().isGreaterThanZero() && adjustedEmi.isLessThan(repaymentPeriod.getPaidPrincipal().add(repaymentPeriod.getFixedInterest()))) {
                adjustedEmi = repaymentPeriod.getPaidPrincipal().add(repaymentPeriod.getFixedInterest());
            }
            repaymentPeriod.setEmi(adjustedEmi);
            if (repaymentPeriod.getEmi().isLessThan(repaymentPeriod.getTotalPaidAmount().minus(repaymentPeriod.getTotalCreditedAmount(), mc))) {
    ...
    private void calculateUnrecognizedInterestTillDateOnScheduleModelCopyAndDefer(ProgressiveLoanInterestScheduleModel scheduleModel, RepaymentPeriod repaymentPeriod, LocalDate tillDate) {
        MathContext mc = scheduleModel.mc();
        ProgressiveLoanInterestScheduleModel scheduleModelCopy = scheduleModel.deepCopy(mc);
        ...
        futureUnrecognizedInterestPeriod.ifPresent(period -> {
            ...
            //
            scheduleModel.repaymentPeriods().stream().filter(rp -> rp.getDueDate().isAfter(repaymentPeriod.getDueDate())).forEach(rp -> {
    ...
    private void checkAndAdjustEmiIfNeededOnRelatedRepaymentPeriods(final ProgressiveLoanInterestScheduleModel scheduleModel, final List<RepaymentPeriod> relatedRepaymentPeriods) {
        MathContext mc = scheduleModel.mc();
        ...
        do {
            ...
            newScheduleModel.repaymentPeriods().forEach(period -> {
                if (!period.getFromDate().isBefore(relatedPeriodsFirstFromDate) && !period.getDueDate().isBefore(relatedPeriodsFirstDueDate) && !adjustedEqualMonthlyInstallmentValue.isLessThan(period.getTotalPaidAmount()) && !period.isReAgedEarlyRepaymentHolder()) {
            ...
            final Iterator<RepaymentPeriod> relatedPeriodFromNewModelIterator = //
            //
            newScheduleModel.repaymentPeriods().stream().filter(period -> !period.getFromDate().isBefore(relatedPeriodsFirstFromDate) && !period.getDueDate().isBefore(relatedPeriodsFirstDueDate) && !period.isReAgedEarlyRepaymentHolder()).toList().iterator();//
     ...
     */
    private void calculateRateFactorForPeriods(final List<RepaymentPeriod> repaymentPeriods, final ProgressiveLoanInterestScheduleModel scheduleModel) {
    ...
    private BigDecimal getNumberOfDays(DaysInYearType daysInYearType, DaysInYearCustomStrategyType customStrategy, LocalDate interestPeriodFromDate, LocalDate repaymentPeriodFromDate, LocalDate repaymentPeriodDueDate) {
    ...
    BigDecimal calculateRateFactorPerPeriodForInterest(final ProgressiveLoanInterestScheduleModel scheduleModel, final RepaymentPeriod repaymentPeriod, final LocalDate interestPeriodFromDate, final LocalDate interestPeriodDueDate) {
        final MathContext mc = scheduleModel.mc();
        final ILoanConfigurationDetails loanProductRelatedDetail = scheduleModel.loanProductRelatedDetail();
        final BigDecimal interestRate = calcNominalInterestRatePercentage(scheduleModel.getInterestRate(interestPeriodFromDate), scheduleModel.mc());
        ...
        BigDecimal daysInYear = getNumberOfDays(daysInYearType, daysInYearCustomStrategy, interestPeriodFromDate, repaymentPeriod.getFromDate(), repaymentPeriod.getDueDate());
        final BigDecimal actualDaysInPeriod = BigDecimal.valueOf(DateUtils.getDifferenceInDays(interestPeriodFromDate, interestPeriodDueDate));
        final BigDecimal calculatedDaysInPeriod = BigDecimal.valueOf(DateUtils.getDifferenceInDays(repaymentPeriod.getFromDate(), repaymentPeriod.getDueDate()));
        final int numberOfYearsDifferenceInPeriod = interestPeriodDueDate.getYear() - interestPeriodFromDate.getYear();
        final boolean partialPeriodCalculationNeeded = daysInYearType == DaysInYearType.ACTUAL && numberOfYearsDifferenceInPeriod > 0 && (!DaysInYearCustomStrategyType.FEB_29_PERIOD_ONLY.equals(daysInYearCustomStrategy) || isPeriodContainsFeb29(repaymentPeriod.getFromDate(), repaymentPeriod.getDueDate()));
        final BigDecimal repaymentEvery = BigDecimal.valueOf(loanProductRelatedDetail.getRepayEvery());
        if (loanProductRelatedDetail.getInterestCalculationPeriodMethod() != null && loanProductRelatedDetail.getInterestCalculationPeriodMethod().isSameAsRepaymentPeriod()) {
            if (loanProductRelatedDetail.getRepaymentPeriodFrequencyType().isMonthly()) {
                return rateFactorByRepaymentPeriod(interestRate, BigDecimal.ONE, repaymentEvery, BigDecimal.valueOf(12), actualDaysInPeriod, calculatedDaysInPeriod, mc);
            }
            if (loanProductRelatedDetail.getRepaymentPeriodFrequencyType().isWeekly()) {
                return rateFactorByRepaymentPeriod(interestRate, BigDecimal.ONE, repaymentEvery, BigDecimal.valueOf(52), actualDaysInPeriod, calculatedDaysInPeriod, mc);
            }
        }
        ...
        if (partialPeriodCalculationNeeded) {
            final BigDecimal cumulatedPeriodFractions = calculatePeriodFractions(scheduleModel, interestPeriodFromDate, interestPeriodDueDate, mc);
            return rateFactorByRepaymentPartialPeriod(interestRate, BigDecimal.ONE, cumulatedPeriodFractions, BigDecimal.ONE, BigDecimal.ONE, mc);
        }
        if (daysInMonthType.equals(DaysInMonthType.ACTUAL)) {
            ...
            return calculateRateFactorPerPeriodBasedOnRepaymentFrequency(interestRate, repaymentFrequency, periodRatio, BigDecimal.valueOf(30), daysInYear, actualDaysInPeriod, calculatedDaysInPeriod, mc);
        }
        throw new UnsupportedOperationException("Unsupported combination: Days in year: " + daysInYearType + ", days in month: " + daysInMonthType);
    }

    private static BigDecimal calculatePeriodRatio(ProgressiveLoanInterestScheduleModel scheduleModel, RepaymentPeriod repaymentPeriod, ChronoUnit chronoUnit, MathContext mc) {
        ...
        int numberOfPeriodBetweenSeedDateAndActualRepaymentPeriod = switch (chronoUnit) {
            ...
            case MONTHS -> {
                ...
                int targetDateLastDay = ((LocalDate) TemporalAdjusters.lastDayOfMonth().adjustInto(repaymentPeriod.getFromDate())).getDayOfMonth();
    ...
    private static LocalDate calculateSeedDate(ProgressiveLoanInterestScheduleModel scheduleModel, RepaymentPeriod repaymentPeriod) {
        ...
        ChronoUnit chronoUnit = switch (scheduleModel.loanProductRelatedDetail().getRepaymentPeriodFrequencyType()) {
            ...
            default -> throw new UnsupportedOperationException("Unsupported repayment frequency: " + scheduleModel.loanProductRelatedDetail().getRepaymentPeriodFrequencyType());
        };
        do {
            calculatedDate = seedDate.plus(multiplicator, chronoUnit);
            multiplicator++;
        } while (calculatedDate.isBefore(repaymentPeriod.getDueDate()));
        return calculatedDate.equals(repaymentPeriod.getDueDate()) && calculatedDate.minus(scheduleModel.loanProductRelatedDetail().getRepayEvery(), chronoUnit).isEqual(repaymentPeriod.getFromDate()) ? seedDate : repaymentPeriod.getFromDate();
    }

    /**
     * Calculate Rate Factor for an exact Period
     */
    private BigDecimal calculateRateFactorPerPeriod(final ProgressiveLoanInterestScheduleModel scheduleModel, final RepaymentPeriod repaymentPeriod, final LocalDate interestPeriodFromDate, final LocalDate interestPeriodDueDate) {
        final MathContext mc = scheduleModel.mc();
        final ILoanConfigurationDetails loanProductRelatedDetail = scheduleModel.loanProductRelatedDetail();
        final BigDecimal interestRate = calcNominalInterestRatePercentage(scheduleModel.getInterestRate(interestPeriodFromDate), scheduleModel.mc());
        final DaysInYearType daysInYearType = DaysInYearType.fromInt(loanProductRelatedDetail.getDaysInYearType());
        ...
        BigDecimal daysInYear = getNumberOfDays(daysInYearType, daysInYearCustomStrategy, interestPeriodFromDate, repaymentPeriod.getFromDate(), repaymentPeriod.getDueDate());
        final BigDecimal actualDaysInPeriod = BigDecimal.valueOf(DateUtils.getDifferenceInDays(interestPeriodFromDate, interestPeriodDueDate));
        final BigDecimal calculatedDaysInRepaymentPeriod = BigDecimal.valueOf(DateUtils.getDifferenceInDays(repaymentPeriod.getFromDate(), repaymentPeriod.getDueDate()));
        final int numberOfYearsDifferenceInPeriod = interestPeriodDueDate.getYear() - interestPeriodFromDate.getYear();
        final boolean partialPeriodCalculationNeeded = daysInYearType == DaysInYearType.ACTUAL && numberOfYearsDifferenceInPeriod > 0 && (!DaysInYearCustomStrategyType.FEB_29_PERIOD_ONLY.equals(daysInYearCustomStrategy) || isPeriodContainsFeb29(repaymentPeriod.getFromDate(), repaymentPeriod.getDueDate()));
        final BigDecimal daysInMonth = daysInMonthType.isDaysInMonth_30() ? BigDecimal.valueOf(30) : calculatedDaysInRepaymentPeriod;
        if (loanProductRelatedDetail.getInterestCalculationPeriodMethod() != null && loanProductRelatedDetail.getInterestCalculationPeriodMethod().isSameAsRepaymentPeriod()) {
            if (loanProductRelatedDetail.getRepaymentPeriodFrequencyType().isMonthly()) {
                return rateFactorByRepaymentPeriod(interestRate, BigDecimal.ONE, repaymentEvery, BigDecimal.valueOf(12), actualDaysInPeriod, calculatedDaysInRepaymentPeriod, mc);
            }
            if (loanProductRelatedDetail.getRepaymentPeriodFrequencyType().isWeekly()) {
                return rateFactorByRepaymentPeriod(interestRate, BigDecimal.ONE, repaymentEvery, BigDecimal.valueOf(52), actualDaysInPeriod, calculatedDaysInRepaymentPeriod, mc);
            }
        }
        // TODO check: loanApplicationTerms.calculatePeriodsBetweenDates(startDate, endDate); // calculate period data
        // TODO review: (repayment frequency: days, weeks, years; validation day is month fix 30)
        ...
        return switch (daysInMonthType) {
            case ACTUAL -> rateFactorByRepaymentPeriod(interestRate, actualDaysInPeriod, BigDecimal.ONE, daysInYear, BigDecimal.ONE, BigDecimal.ONE, mc);
            case DAYS_30 -> calculateRateFactorPerPeriodBasedOnRepaymentFrequency(interestRate, repaymentFrequency, repaymentEvery, daysInMonth, daysInYear, actualDaysInPeriod, calculatedDaysInRepaymentPeriod, mc);
     ...
     * @return
     */
    public BigDecimal calculatePeriodFractions(ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate interestPeriodFromDate, final LocalDate interestPeriodDueDate, MathContext mc) {
     ...
     * @return
     */
    private BigDecimal calculateRateFactorPerPeriodBasedOnRepaymentFrequency(final BigDecimal interestRate, final PeriodFrequencyType repaymentFrequency, final BigDecimal repaymentEvery, final BigDecimal daysInMonth, final BigDecimal daysInYear, final BigDecimal actualDaysInPeriod, final BigDecimal calculatedDaysInPeriod, final MathContext mc) {
        return switch (repaymentFrequency) {
            case DAYS -> rateFactorByRepaymentEveryDay(interestRate, repaymentEvery, daysInYear, actualDaysInPeriod, calculatedDaysInPeriod, mc);
            case WEEKS -> rateFactorByRepaymentEveryWeek(interestRate, repaymentEvery, daysInYear, actualDaysInPeriod, calculatedDaysInPeriod, mc);
            case MONTHS -> rateFactorByRepaymentEveryMonth(interestRate, repaymentEvery, daysInMonth, daysInYear, actualDaysInPeriod, calculatedDaysInPeriod, mc);
            default -> throw new UnsupportedOperationException("Invalid repayment frequency"); // not supported yet
        };
    }

    private void calculateEMIOnActualModelWithFlatInterestMethod(List<RepaymentPeriod> repaymentPeriods, ProgressiveLoanInterestScheduleModel scheduleModel) {
        final MathContext mc = scheduleModel.mc();
        ...
        Money alreadyRepaidPrincipals = firstRepaymentPeriod.getPrevious().map(rp -> rp.calculateTotalDisbursedAndCapitalizedIncomeAmountTillGivenPeriod(null).minus(rp.getOutstandingLoanBalance())).orElse(Money.zero(currency));
        Money total = firstRepaymentPeriod.calculateTotalDisbursedAndCapitalizedIncomeAmountTillGivenPeriod(firstRepaymentPeriod.getLastInterestPeriod()).plus(sumOfInterest).minus(alreadyRepaidPrincipals);
        ...
        repaymentPeriods.forEach(rp -> {
            ...
            if (emi.plus(rp.getTotalCreditedAmount(), mc).plus(rp.getFutureUnrecognizedInterest(), mc).isGreaterThanOrEqualTo(rp.getTotalPaidAmount())) {
                rp.setEmi(emi);
                rp.setOriginalEmi(emi);
                rp.getInterestPeriods().forEach(InterestPeriod::updateOutstandingLoanBalance);
            } else {
                Money adjustment = rp.getTotalPaidAmount().minus(emi.plus(rp.getTotalCreditedAmount(), mc).plus(rp.getFutureUnrecognizedInterest(), mc));
        ...
        if (reallocationAmount.get().isGreaterThanZero()) {
            repaymentPeriods.reversed().forEach(rp -> {
                if (reallocationAmount.get().isGreaterThanZero() && !rp.isFullyPaid()) {
                    ...
                    Money alreadyPaidEmi = rp.getTotalPaidAmount().minus(rp.getTotalCreditedAmount(), mc).minus(rp.getFutureUnrecognizedInterest(), mc);
    ...
    private void applyPrincipalMoratoriumIfRequired(List<RepaymentPeriod> repaymentPeriods, ProgressiveLoanInterestScheduleModel scheduleModel) {
        if (repaymentPeriods.isEmpty()) {
            return;
        }
    ...
    private void calculateEMIOnActualModelWithDecliningBalanceInterestMethod(List<RepaymentPeriod> repaymentPeriods, ProgressiveLoanInterestScheduleModel scheduleModel) {
        final MathContext mc = scheduleModel.mc();
        ...
        final Money equalMonthlyInstallment = Money.of(outstandingBalance.getCurrencyData(), calculateEMIValue(rateFactorN, outstandingBalance.getAmount(), fnResult, mc), mc).add(calculateEMIValueForFixedInterest(repaymentPeriods, mc));
    ...
    private void calculateEMIOnNewModelAndMerge(List<RepaymentPeriod> repaymentPeriods, ProgressiveLoanInterestScheduleModel scheduleModel, final EmiChangeOperation operation) {
        if (repaymentPeriods.isEmpty()) {
            return;
        }
    ...
    private Money applyInstallmentAmountInMultiplesOf(final ProgressiveLoanInterestScheduleModel scheduleModel, final Money equalMonthlyInstallment) {
        return scheduleModel.installmentAmountInMultiplesOf() != null && scheduleModel.installmentAmountInMultiplesOf() > 0 ? safeRoundingForEMI(equalMonthlyInstallment, scheduleModel.installmentAmountInMultiplesOf()) : equalMonthlyInstallment;
    }
    ...
    public EmiAdjustment getEmiAdjustment(final List<RepaymentPeriod> repaymentPeriods) {
        for (int idx = repaymentPeriods.size() - 1; idx > 0; --idx) {
            ...
            if (!lastPeriod.isFullyPaid() && !penultimatePeriod.isFullyPaid()) {
                ...
                return new EmiAdjustment(penultimatePeriod.getEmi(), emiDifference, repaymentPeriods, getUncountablePeriods(repaymentPeriods, penultimatePeriod.getEmi()));
            }
        }
        return new EmiAdjustment(repaymentPeriods.getFirst().getEmi(), repaymentPeriods.getFirst().getEmi().copy(0.0), repaymentPeriods, 0);
    }

    private void calculateRateFactorForScheduleTillDateInclusive(ProgressiveLoanInterestScheduleModel scheduleModelCopy, LocalDate targetDate) {
        scheduleModelCopy.findRepaymentPeriod(targetDate).flatMap(rp -> rp.findInterestPeriod(targetDate)).ifPresent(ip -> ip.setDueDate(targetDate));
        calculateRateFactorForPeriods(scheduleModelCopy.repaymentPeriods(), scheduleModelCopy);
        scheduleModelCopy.repaymentPeriods().forEach(rp -> rp.getInterestPeriods().stream().filter(ip -> targetDate.isBefore(ip.getDueDate())).forEach(ip -> {
            ip.setRateFactor(BigDecimal.ZERO);
            ip.setRateFactorTillPeriodDueDate(BigDecimal.ZERO);
        }));
    }

    private Optional<RepaymentPeriod> getPeriodWithUnrecognizedInterest(RepaymentPeriod lastUnpaidRepaymentPeriod, ProgressiveLoanInterestScheduleModel scheduleModelCopy) {
        for (RepaymentPeriod period : scheduleModelCopy.repaymentPeriods().reversed()) {
            if (MathUtil.isGreaterThanZero(period.getUnrecognizedInterest()) && period.getDueDate().isAfter(lastUnpaidRepaymentPeriod.getDueDate())) {
    ...
    private BigDecimal calculateRateFactorPlus1NForEmi(final List<RepaymentPeriod> periods, final ProgressiveLoanInterestScheduleModel scheduleModel, MathContext mc) {
        return periods.stream().map(period -> getRateFactorPlus1ForEmi(period, scheduleModel)).reduce(BigDecimal.ONE, (BigDecimal acc, BigDecimal value) -> acc.multiply(value, mc));
    }

    private BigDecimal calculateFnResultForEmi(final List<RepaymentPeriod> periods, final ProgressiveLoanInterestScheduleModel scheduleModel, final MathContext mc) {
        return //
        //
        //
        periods.stream().skip(1).map(period -> getRateFactorPlus1ForEmi(period, scheduleModel)).reduce(BigDecimal.ONE, (previousFnValue, currentRateFactor) -> fnValue(previousFnValue, currentRateFactor, mc));//
    }
    ...
    private BigDecimal getRateFactorPlus1ForEmi(final RepaymentPeriod period, final ProgressiveLoanInterestScheduleModel scheduleModel) {
        return scheduleModel.isInterestPauseForEmiCalculationEnabled() && period.isInterestPaymentGrace() ? BigDecimal.ONE : period.getRateFactorPlus1();
    }

    /**
     * Calculate the EMI (Equal Monthly Installment) value
     */
    private BigDecimal calculateEMIValue(final BigDecimal rateFactorPlus1N, final BigDecimal outstandingBalanceForRest, final BigDecimal fnResult, MathContext mc) {
    ...
    private BigDecimal calculateEMIValueForFixedInterest(final List<RepaymentPeriod> repaymentPeriods, MathContext mc) {
        return repaymentPeriods.stream().map(RepaymentPeriod::getFixedInterest).map(Money::getAmount).reduce(ZERO, BigDecimal::add).divide(BigDecimal.valueOf(repaymentPeriods.isEmpty() ? 1 : repaymentPeriods.size()), mc);
    }
     ...
     */
    private BigDecimal rateFactorByRepaymentEveryDay(final BigDecimal interestRate, final BigDecimal repaymentEvery, final BigDecimal daysInYear, final BigDecimal actualDaysInPeriod, final BigDecimal calculatedDaysInPeriod, MathContext mc) {
        return rateFactorByRepaymentPeriod(interestRate, BigDecimal.ONE, repaymentEvery, daysInYear, actualDaysInPeriod, calculatedDaysInPeriod, mc);
    }
     ...
     * @return Rate Factor for period
     */
    private BigDecimal rateFactorByRepaymentEveryWeek(final BigDecimal interestRate, final BigDecimal repaymentEvery, final BigDecimal daysInYear, final BigDecimal actualDaysInPeriod, final BigDecimal calculatedDaysInPeriod, MathContext mc) {
        return rateFactorByRepaymentPeriod(interestRate, ONE_WEEK_IN_DAYS, repaymentEvery, daysInYear, actualDaysInPeriod, calculatedDaysInPeriod, mc);
    }
     ...
     * @return Rate Factor for period
     */
    BigDecimal rateFactorByRepaymentEveryMonth(final BigDecimal interestRate, final BigDecimal repaymentEvery, final BigDecimal daysInMonth, final BigDecimal daysInYear, final BigDecimal actualDaysInPeriod, final BigDecimal calculatedDaysInPeriod, final MathContext mc) {
        return rateFactorByRepaymentPeriod(interestRate, daysInMonth, repaymentEvery, daysInYear, actualDaysInPeriod, calculatedDaysInPeriod, mc);
    }
     ...
     * @return Rate Factor for period
     */
    private BigDecimal rateFactorByRepaymentPeriod(final BigDecimal interestRate, final BigDecimal repaymentPeriodMultiplierInDays, final BigDecimal repaymentEvery, final BigDecimal daysInYear, final BigDecimal actualDaysInPeriod, final BigDecimal calculatedDaysInPeriod, final MathContext mc) {
        if (MathUtil.isZero(calculatedDaysInPeriod)) {
            return BigDecimal.ZERO;
        }
        final BigDecimal interestFractionPerPeriod = //
        //
        repaymentPeriodMultiplierInDays.multiply(repaymentEvery, mc).divide(daysInYear, mc);//
        return //
        //
        //
     ...
     */
    private BigDecimal rateFactorByRepaymentPartialPeriod(final BigDecimal interestRate, final BigDecimal repaymentEvery, final BigDecimal cumulatedPeriodRatio, final BigDecimal actualDaysInPeriod, final BigDecimal calculatedDaysInPeriod, final MathContext mc) {
        if (MathUtil.isZero(calculatedDaysInPeriod)) {
            return BigDecimal.ZERO;
        }
    ...
    public Money getSumOfDueInterestsOnDate(ProgressiveLoanInterestScheduleModel scheduleModel, LocalDate subjectDate) {
        ...
        //
        //
        scheduleModel.repaymentPeriods().stream().map(repaymentPeriod -> getDueAmounts(scheduleModel, repaymentPeriod.getFromDate(), repaymentPeriod.getDueDate(), subjectDate).getDueInterest()).reduce(scheduleModel.zero(), Money::add); //
    }

    @Override
    public void applyInterestPause(final ProgressiveLoanInterestScheduleModel scheduleModel, final LocalDate fromDate, final LocalDate endDate) {
        scheduleModel.updateInterestPeriodsForInterestPause(fromDate, endDate).ifPresent(repaymentPeriod -> calculateRateFactorsForInterestPause(scheduleModel, repaymentPeriod.getFromDate()));
    }
    ...
    private long getUncountablePeriods(final List<RepaymentPeriod> relatedRepaymentPeriods, final Money originalEmi) {
        return  //
        //
        relatedRepaymentPeriods.stream().filter(repaymentPeriod -> originalEmi.isLessThan(repaymentPeriod.getTotalPaidAmount())).count(); //
    }

    private void accelerateRepaymentDueDateTo(ProgressiveLoanInterestScheduleModel interestSchedule, RepaymentPeriod repaymentPeriod, LocalDate transactionDate) {
    ...
    private void updateEMIForReAgeEqualAmortization(List<RepaymentPeriod> repaymentPeriods, Money principal, Money interest, Money feesPenaltiesOutstanding, EqualAmortizationValues feesPenaltiesEqualAmortizationValues, MonetaryCurrency currency) {
        EqualAmortizationValues interestEAV = calculateEqualAmortizationValues(interest, repaymentPeriods.size(), null, currency);
        EqualAmortizationValues principalEAV = calculateAdjustedEqualAmortizationValues(principal, principal.add(interest).add(feesPenaltiesOutstanding), interestEAV.value().add(feesPenaltiesEqualAmortizationValues.value()), repaymentPeriods.size(), null, currency);
    ...
    @Override
    public OutstandingDetails precalculateReAgeEqualAmortizationAmount(ProgressiveLoanInterestScheduleModel interestSchedule, LocalDate transactionDate, LoanReAgeParameterData reageParameter) {
        return getOutstandingAmountsTillDate(interestSchedule, reageParameter.getInterestHandlingType().equals(LoanReAgeInterestHandlingType.EQUAL_AMORTIZATION_PAYABLE_INTEREST) ? transactionDate : interestSchedule.getMaturityDate());
    }

    @Override
    public void reAgeEqualAmortization(ProgressiveLoanInterestScheduleModel interestSchedule, LocalDate transactionDate, LoanReAgeParameterData reageParameter, Money feesPenaltiesOutstanding, EqualAmortizationValues feesPenaltiesEqualAmortizationValues) {
        ...
        Money futureCreditedPrincipals = interestSchedule.repaymentPeriods().stream().filter(rp -> !rp.getFromDate().isBefore(transactionDate)).filter(rp -> rp.getDueDate().isAfter(transactionDate)).map(RepaymentPeriod::getCreditedPrincipal).reduce(zero, Money::add);
        ...
        interestSchedule.repaymentPeriods().forEach(rp -> {
            rp.getInterestPeriods().getLast().addCreditedInterestAmount(MathUtil.min(rp.getOutstandingInterest(), rp.getCreditedInterest(), false).negated());
            rp.setEmi(rp.getTotalPaidAmount());
            rp.moveOutstandingDueToReAging();
            rp.setInterestMovedDownward(true);
        });
        if (!originalMaturityDate.isBefore(transactionDate)) {
            createRepaymentPeriodForEarlyRepaidAmountsDuringReAgeing(interestSchedule, paidBalancesFromTransactionDate.getOutstandingPrincipal(), paidBalancesFromTransactionDate.getOutstandingInterest(), true);
        }
        ...
        updateEMIForReAgeEqualAmortization(reAgedRepaymentPeriods, reAgeingAmounts.getOutstandingPrincipal(), reAgeingAmounts.getOutstandingInterest(), feesPenaltiesOutstanding, feesPenaltiesEqualAmortizationValues, zero.getCurrency());
    ...
    private void updateModelForReageEqualAmortization(ProgressiveLoanInterestScheduleModel interestSchedule, LoanReAgeParameterData reageParameter, List<RepaymentPeriod> reAgedRepaymentPeriods) {
        ...
        for (int i = 0; i < numberOfInstallmentsToAdd; i++) {
            RepaymentPeriod repaymentPeriod = RepaymentPeriod.create(previous, previous.getDueDate(), toDate, interestSchedule.zero(), interestSchedule.mc(), previous.getLoanProductRelatedDetail());
    ...
    private void createRepaymentPeriodForEarlyRepaidAmountsDuringReAgeing(final ProgressiveLoanInterestScheduleModel interestSchedule, final Money totalPaidPrincipal, final Money totalPaidInterest, final boolean isReAged) {
    ...
    private OutstandingDetails calculatePaidBalancesAfterDate(ProgressiveLoanInterestScheduleModel interestSchedule, LocalDate transactionDate) {
        Money principal = interestSchedule.repaymentPeriods().stream().filter(rp -> !rp.getDueDate().isBefore(transactionDate)).map(RepaymentPeriod::getPaidPrincipal).reduce(interestSchedule.zero(), Money::add);
        Money interest = interestSchedule.repaymentPeriods().stream().filter(rp -> !rp.getDueDate().isBefore(transactionDate)).map(RepaymentPeriod::getPaidInterest).reduce(interestSchedule.zero(), Money::add);
        return new OutstandingDetails(principal, interest);
    }

    @Override
    public EqualAmortizationValues calculateEqualAmortizationValues(Money totalOutstanding, Integer numberOfInstallments, Integer installmentAmountInMultiplesOf, MonetaryCurrency currency) {
    ...
    @Override
    public EqualAmortizationValues calculateAdjustedEqualAmortizationValues(Money outstanding, Money total, Money sumOfOtherEqualAmortizationValues, Integer numberOfInstallments, Integer installmentAmountInMultiplesOf, MonetaryCurrency currency) {
        EqualAmortizationValues calculatedEMI = calculateEqualAmortizationValues(total, numberOfInstallments, installmentAmountInMultiplesOf, currency);
    ...
    @java.lang.SuppressWarnings("all")
        public ProgressiveEMICalculator(final ScheduledDateGenerator scheduledDateGenerator) {
