...
/** Composition-root hosted search results for the Equinox bridge smoke. */
final class HostedSearchReadService implements SearchReadService {

    static final long HOSTED_ID = 1L;

    @Override
    public List<SearchData> retriveMatchingData(final SearchConditions searchConditions) {
        return List.of(new SearchData(HOSTED_ID, "hosted", "hosted", "hosted", "client", null, null, null, null,
                new EnumOptionData(HOSTED_ID, "hosted", "hosted"), null));
    }

    @Override
    public AdHocSearchQueryData retrieveAdHocQueryTemplate() {
        return AdHocSearchQueryData.template(List.of(), List.of());
    }
