...
Optional Boot embed (`fineract.osgi.enabled=true`, default off): `ServerApplication` starts in-process Equinox and registers Wave-1 through Wave-5 Spring beans via `org.apache.fineract.infrastructure.osgi.SpringOsgiPortBridge` (Wave-1 catalogs, Wave-2 content/cashier/originator/mix/investor, Wave-3 `GLClosureReadPlatformService`, `SavingsDropdownReadPlatformService`, `LoanProductLookupReadPort`, `BuyDownFeeReadPlatformService`, `WorkingCapitalLoanPeriodPaymentRateChangeReadService`, Wave-4 `ConfigJobParameterService`, `AccessTokenGenerationService`, `BusinessDateReadPlatformService`, `CodeReadPlatformService`, `ProvisioningCategoryReadPlatformService`, Wave-5 `CurrencyWritePlatformService`, `PasswordValidationPolicyReadPlatformService`, `AdHocReadPlatformService`, `TemplateMergeService`, `UserNotificationService`). Set `fineract.osgi.catalog-dir` to a staged `osgi/` directory (`./gradlew osgiStageBundles`) to install empty catalog activators first; Spring ports still win on ranking. Spring is not staged. Missing catalog is a no-op.
