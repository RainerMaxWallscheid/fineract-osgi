...
Fails when leftover ArchUnit freeze-store lines exceed `osgi/archunit-freeze-budget.txt`, or when the budget is stale after a shrink. Remaining lines are leftover JPA / entity residuals (ADR-022 B5). Freeze shrink started at 1166 lines (GSIM / `ClientTransactionDTO` / journal `SavingsEnumerations` leftover enum retargets). Do not peel `fineract-core` or the composition root to force a shrink.
