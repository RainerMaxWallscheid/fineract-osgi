...
| `fineract-bulkimport` | `bulkimport` | `BulkImportOsgiServiceRegistrar` / DS `bulkimport-*.xml` |
| `fineract-instancemode` | `instancemode` | `InstanceModeOsgiServiceRegistrar` / DS `instancemode.xml` |
| `fineract-jobs` | `jobs` | `JobsOsgiServiceRegistrar` |
| `fineract-s3` | `s3` | `S3OsgiServiceRegistrar` / DS `s3.xml` |
| `fineract-openapi` | `openapi` | `OpenApiOsgiServiceRegistrar` / DS `openapi.xml` |
| `fineract-springbatch` | `springbatch` | `SpringBatchOsgiServiceRegistrar` |
| `fineract-event` | `event` | `EventOsgiServiceRegistrar` / DS `event-*.xml` |
| `fineract-interoperation` | `interoperation` | `InteroperationOsgiServiceRegistrar` / DS `interoperation.xml` |
...
| Artifact | Bundle-SymbolicName | Notes |
|----------|---------------------|-------|
| `fineract-useradministration-api` | `org.apache.fineract.useradministration.api` | Read/write ports, password policy DTO, `MakerCheckerPermissionPort`, `ReportPermissionPort` |
...
Equinox registers `CreditBureauReadPlatformService` only (`retrieveCreditBureau` → empty). Did not register loan-product mapping reads. Mapping entities store loan product id (not leftover `LoanProduct`).
...
| `fineract-bulkimport-impl` | `org.apache.fineract.bulkimport.impl` | REST/JPA + `BulkImportOsgiServiceRegistrar` / DS `OSGI-INF/bulkimport-*.xml` |
| `fineract-bulkimport-test` | `org.apache.fineract.bulkimport.test` | Fragment-Host → bulkimport.impl |

Equinox registers `BulkImportWorkbookService` (`importWorkbook` / `getImport` → null) and `BulkImportWorkbookPopulatorService` (`getTemplate` → null). Jersey `FormDataContentDisposition` and JAX-RS `Response` on those ports are Object-typed. Did not register POI `WorkbookPopulator` / `ImportHandler`.
...
| `fineract-instancemode-impl` | `org.apache.fineract.instancemode.impl` | Test-profile REST + `InstanceModeOsgiServiceRegistrar` / DS `OSGI-INF/instancemode.xml` |
| `fineract-instancemode-test` | `org.apache.fineract.instancemode.test` | Fragment-Host → instancemode.impl |

Equinox registers `InstanceModePort` (`changeMode` no-op). Swagger request DTO and `FineractInstanceModeApiFilter` stay off the catalog port.
...
| Artifact | Bundle-SymbolicName | Notes |
|----------|---------------------|-------|
| `fineract-s3-api` | `org.apache.fineract.s3.api` | `S3ClientCustomizer` SPI (AWS builder Object-typed) |
| `fineract-s3-impl` | `org.apache.fineract.s3.impl` | Amazon/Localstack config + `S3OsgiServiceRegistrar` / DS `OSGI-INF/s3.xml` |
| `fineract-s3-test` | `org.apache.fineract.s3.test` | Fragment-Host → s3.impl |

Equinox registers `S3ClientCustomizer` (`customize` no-op). AWS `S3Client` / `S3ClientBuilder` stay unpublished.

### Provider peel: fineract-openapi (complete)

| Artifact | Bundle-SymbolicName | Notes |
|----------|---------------------|-------|
| `fineract-openapi-api` | `org.apache.fineract.openapi.api` | `OpenApiPort` + swagger-gradle-plugin reader/filter utilities |
| `fineract-openapi-impl` | `org.apache.fineract.openapi.impl` | `OpenApiOsgiServiceRegistrar` / DS `OSGI-INF/openapi.xml` |
| `fineract-openapi-test` | `org.apache.fineract.openapi.test` | Fragment-Host → openapi.impl |

Equinox registers `OpenApiPort` (`isRemovingUnreferencedDefinitions` → false). Swagger reader and `FineractOpenApiSpecFilter` stay off the catalog port.
...
| `fineract-interoperation-impl` | `org.apache.fineract.interoperation.impl` | REST/JPA + `InteroperationOsgiServiceRegistrar` / DS `OSGI-INF/interoperation.xml` |
| `fineract-interoperation-test` | `org.apache.fineract.interoperation.test` | Fragment-Host → interoperation.impl |

Equinox registers `InteropService` (account/quote/transfer/KYC → null). Leftover savings catalog enums on account/transaction DTOs are Object-typed. Did not register leftover unpublished `DepositAccountType` / `SavingsAccountTransactionType` types themselves.

### Provider peel: fineract-event (complete)

| Artifact | Bundle-SymbolicName | Notes |
|----------|---------------------|-------|
| `fineract-event-api` | `org.apache.fineract.event.api` | `PortfolioNotificationEventPort`, `SmsCampaignTriggerEventPort` |
| `fineract-event-impl` | `org.apache.fineract.event.impl` | Serializers/producers + `EventOsgiServiceRegistrar` / DS `OSGI-INF/event-*.xml` |
| `fineract-event-test` | `org.apache.fineract.event.test` | Fragment-Host → event.impl |

Equinox registers `PortfolioNotificationEventPort` (`onNotifications` no-op) and `SmsCampaignTriggerEventPort` (`on*` no-op). Did not register leftover `ExternalEventProducer` (core only).
...
Fails on a new domain `*-impl` dependency on a foreign `*-impl`, any `*-api` dependency on a `*-impl`, or a stale `osgi/foreign-impl-allowlist.txt` row. Composition roots are not scanned. No leftover-JPA foreign `-impl` edges remain. Progressive leftover loan writes (Loan graphs, schedule processor, leftover REST/write residual) live on leftover loan-impl. S3 datatable report export implements `DatatableReportExportService` on s3-impl (not dataqueries-impl). SMS, jobs, command, configuration, accounting, campaigns, dataqueries, interoperation, report-mailing-job, savings, and bulkimport consume security/business-date types from core or `*-api` (not those `*-impl` jars). Bulkimport consumes codes, document, fund, charge, address, calendar, and collateral-management types from core or `*-api` (not those `*-impl` jars). Campaigns consumes configuration and calendar types from core or `*-api` (not those `*-impl` jars). Charge office-access uses `OfficeProductRestrictionService` (not entityaccess-impl). Charge client-charge validation lives on charge-impl (not clients-impl). Dataqueries consumes savings product types from core or `*-api` (not savings-impl). Progressive leftover transfer validation lives on leftover loan-impl (not transfer-impl). Currency write checks loan-product currency use via `LoanProductExistencePort` (not loan-impl). Collectionsheet bulk-repayment payment details use `PaymentDetailWritePlatformService` (not paymentdetail-impl). Progressive-loan deposit bulk-mandatory payment details use `PaymentDetailWritePlatformService` (not paymentdetail-impl). Leftover collection-sheet write uses collectionsheet-api deserializers/validator (not collectionsheet-impl). Leftover ClientsApiResourceSwagger uses clients-api `ClientAddressRequest` (not address-impl). Leftover entity-datatable checks read uses `EntityDatatableChecksReadService` on dataqueries-api (not dataqueries-impl). Leftover account-transfer write uses accounttransfer-api validators (not accounttransfer-impl). Command audit loan-product name and loan enumerations use `LoanProductLookupReadPort` (not loan-impl). Dataqueries entity-datatable loan-product lookup/existence use `LoanProductLookupReadPort` / `LoanProductExistencePort` (not loan-impl). Entity-access loan-product existence uses `LoanProductExistencePort` (not loan-impl). Entity-access savings-product existence uses `SavingsProductExistencePort` (not savings-impl). Entity-access role existence uses `RoleReadPlatformService` (not useradministration-impl). Credit-bureau loan-product mapping stores loan product id and checks existence via `LoanProductExistencePort` (not loan-impl). Configuration maker-checker permission lookup uses `MakerCheckerPermissionPort` (not useradministration-impl). Jobs and bulkimport consume AppUser/Role types from core or `useradministration-api` (not useradministration-impl). Dataqueries report READ-permission catalog uses `ReportPermissionPort` (not useradministration-impl). Charge client-transaction journals use `ClientTransactionJournalPort` (not accounting-impl). Meeting calendar-instance lookup uses `CalendarInstanceLookupPort` (not calendar-impl). Charge share-product write/read helpers live on charge-impl (not shares-impl). Progressive-loan share-account charge read uses shares-api `ShareAccountChargeReadPlatformService` (not charge-impl). Progressive-loan share-account purchased-shares/dividend/command readers live in progressive-loan-impl (not shares-impl). Leftover group/center/role readers and validators live on leftover loan-impl (not group-impl). Leftover client write lives on leftover loan-impl; progressive client read uses clients-api `ClientDataValidator` and progressive `ClientMapper` (not clients-impl). Collateral loan existence/pending-approval checks use `LoanExistencePort` (not loan-impl). Organisation provisioning criteria stores loan product id and looks up products via `LoanProductExistencePort` / `LoanProductLookupReadPort` (not loan-impl). Accounting loan-loss provisioning stores provision category id and checks category/criteria existence via `ProvisioningExistencePort` (not organisation-impl). Note loan/loan-transaction attach uses `LoanExistencePort` (not loan-impl). Note savings/savings-transaction attach uses `SavingsAccountExistencePort` (not savings-impl). Branch cashier allocate/settle journals use `CashierJournalPort` (not accounting-impl). Calendar loan attach/reschedule uses `LoanExistencePort` (not loan-impl). Collateral-management loan/product existence and transaction collateral refs use `LoanExistencePort` / `LoanProductExistencePort` (not loan-impl). GCM push send uses `GcmPushNotification` (not sms-impl). Hooks message-gateway SMS uses `TwoFactorSmsDeliveryPort` (not sms-impl / campaigns-impl). Interoperation loan disburse/repay looks up non-closed loan id via `LoanExistencePort` (not loan-impl). Investor external-transfer loan lookup uses `LoanDataForExternalTransferPort` (not cob-impl). Event stayed-locked loan serializers use cob-api types (not cob-impl). Campaigns FCM push send uses `NotificationSenderService` on gcm-api (not gcm-impl). Event document serializers use document-api `DocumentData` / `DocumentCreatedBusinessEvent` (not document-impl). Jobs progressive-model checker uses `ProgressiveLoanModelRebuildPort` (not progressive-loan-impl). Jobs working-capital COB filter uses `WorkingCapitalLoanExistencePort` (not working-capital-loan-impl). Jobs COB filters use cob-api `AccountLockService` / `RetrieveLoanIdService` / `LoanCOBEnabledCondition` / `COBConstant` (not cob-impl). Hooks UGD template stores template id and loads TemplateData via `TemplateDomainService` (not template-impl). Loan-origination originator attach/lookup uses `LoanExistencePort` / `LoanReadPlatformServiceCommon` (not loan-impl). Loan-origination working-capital originator attach/lookup uses `WorkingCapitalLoanExistencePort` (not working-capital-loan-impl). Interoperation savings-transaction notes use `NoteReadPlatformService` / `NoteWritePlatformService` (not note-impl). Full OSGi lookup is complete for Equinox-safe hosted PILOT_PORTs (ADR-022 B4): Boot consumers resolve them via a `@Primary` Service Registry façade when `fineract.osgi.enabled=true`.
...
| `foreign-impl-allowlist.txt` | Empty — no leftover-JPA foreign `-impl` edges remain |
...
Copies every `fineract-*-api`, `fineract-*-impl`, and `fineract-core` jar into `osgi/bundles/` and writes `osgi/config/config.ini` (template + absolute `osgi.bundles` `reference:file:` URLs). Start levels: Felix SCR + OSGi DS API `@1`, core `@2`, api `@3`, impl `@4`. Relative bundle paths resolve against `osgi.install.area` (`osgi/equinox`), not the working directory. Equinox-safe empty catalog ports and the command dispatcher graph register via Declarative Services (`Service-Component`). No Bundle-Activator remains.
...
Starts the same catalog, then registers every composition-root hosted PILOT_PORT (in-memory, not JPA / Spring). Exit 0 requires Felix SCR ACTIVE, every fineract bundle ACTIVE, and the selected services to be the hosted ports. Empty catalog ports stay lowest-ranked. Empty stubs stay in the impl bundle class space; the system classpath does not treat those Classes as assignable. Every staged fineract impl publishes an Equinox-safe catalog port.

Optional Boot embed (`fineract.osgi.enabled=true`, default off): `ServerApplication` starts in-process Equinox and registers every Equinox-safe hosted Spring bean via `org.apache.fineract.infrastructure.osgi.SpringOsgiPortBridge`. Boot callers look services up with `OsgiServiceLookup` (empty when Equinox is off or the type is unpublished). When Equinox is on, `ChargeDefinitionPort`, `FloatingRatePort`, `TaxCatalogPort`, `ContentStoreService`, `ContentStreamPort`, `CashierTxnValidationPort`, `LoanOriginatorReadPlatformService`, `MixTaxonomyReadService`, `DelayedSettlementAttributeService`, `GLClosureReadPlatformService`, `SavingsDropdownReadPlatformService`, `LoanProductLookupReadPort`, `BuyDownFeeReadPlatformService`, `WorkingCapitalLoanPeriodPaymentRateChangeReadService`, `ConfigJobParameterService`, `AccessTokenGenerationService`, `BusinessDateReadPlatformService`, `CodeReadPlatformService`, `ProvisioningCategoryReadPlatformService`, `CurrencyWritePlatformService`, `PasswordValidationPolicyReadPlatformService`, `AdHocReadPlatformService`, `TemplateMergeService`, `UserNotificationService`, `ScorecardReadPlatformService`, `FundReadPlatformService`, `AccountNumberFormatReadPlatformService`, `ReadLikelihoodService`, `TransferWritePlatformService`, `PaymentTypeReadService`, `SearchReadService`, `CollectionSheetWritePlatformService`, `StandingInstructionWritePlatformService`, `ShareProductDropdownReadPlatformService`, `GroupLevelReadPlatformService`, `ClientIdentifierWritePlatformService`, `RepaymentWithPostDatedChecksWritePlatformService`, `ProductCommandsService`, `CacheWritePlatformService`, `FineractEntityAccessReadService`, `CalendarDropdownReadPlatformService`, `MeetingAttendanceDropdownReadService`, `FieldConfigurationReadPlatformService`, `CreditBureauReadPlatformService`, `CollateralWritePlatformService`, `CollateralManagementReadService`, `NoteReadPlatformService`, `HookReadPlatformService`, `SmsWritePlatformService`, `ReportMailingJobConfigurationReadPlatformService`, `SmsCampaignDropdownReadPlatformService`, `NotificationConfigurationReadService`, `ReportWritePlatformService`, `ExternalServicesReadPlatformService`, `StuckJobExecutorService`, `PropertyService`, `PaymentDetailWritePlatformService`, `CommandDispatcher`, `PortfolioNotificationEventPort`, `SmsCampaignTriggerEventPort`, `InteropService`, `BulkImportWorkbookService`, `BulkImportWorkbookPopulatorService`, `InstanceModePort`, `OpenApiPort`, and `S3ClientCustomizer` are `@Primary` lookup façades so Boot injection resolves the Service Registry; the bridge publishes the Spring-owned adapter, not those façades. The façades are `OsgiBackedPortFactory` lazy no-op proxies and are not published back into Equinox. Set `fineract.osgi.catalog-dir` to a staged `osgi/` directory (`./gradlew osgiStageBundles`) to install empty catalog activators first; Spring ports still win on ranking. Spring is not staged. Missing catalog is a no-op.

```bash
./gradlew :fineract-provider:test --tests EquinoxBootEmbedIT
```

Slice test: `fineract.osgi.enabled=true` starts in-process Equinox, injects the ChargeDefinitionPort `@Primary` façade, and ranks the Spring adapter over the empty catalog when `fineract.osgi.catalog-dir` is staged.
