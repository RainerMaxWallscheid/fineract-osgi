...
Optional Boot embed (`fineract.osgi.enabled=true`, default off): `ServerApplication` starts in-process Equinox and registers Wave-1/Wave-2 Spring beans via `org.apache.fineract.infrastructure.osgi.SpringOsgiPortBridge` (`ChargeDefinitionPort`, `FloatingRatePort`, `TaxCatalogPort`, `ContentStoreService`, `CashierTxnValidationPort`, `LoanOriginatorReadPlatformService`, `MixTaxonomyReadService`, `DelayedSettlementAttributeService`). Set `fineract.osgi.catalog-dir` to a staged `osgi/` directory (`./gradlew osgiStageBundles`) to install empty catalog activators first; Spring ports still win on ranking. Spring is not staged. Missing catalog is a no-op.
