...
catalog ports from the composition-root classpath. Does not stage Spring.
Writes ``osgi/logs/spring-bridge-smoke.txt``.
...
def main() -> int:
    ...
    compile_cp = os.pathsep.join((str(equinox), str(charge_api), str(rates_api), str(tax_api), str(core)))
