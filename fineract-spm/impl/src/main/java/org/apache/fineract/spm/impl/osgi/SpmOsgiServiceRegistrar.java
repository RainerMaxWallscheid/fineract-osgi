...
/** Spring ↔ OSGi bridge for SPM ports. */
@Component
public class SpmOsgiServiceRegistrar implements InitializingBean, DisposableBean {

    private static final Logger LOG = LoggerFactory.getLogger(SpmOsgiServiceRegistrar.class);

    private final ObjectProvider<ScorecardReadPlatformService> scorecardRead;
    private final List<Object> registrations = new ArrayList<>();

    public SpmOsgiServiceRegistrar(final ObjectProvider<ScorecardReadPlatformService> scorecardRead) {
        this.scorecardRead = scorecardRead;
    }
    ...
    public void afterPropertiesSet() {
        try {
            ...
            final Object bundle = getBundle.invoke(null, SpmOsgiServiceRegistrar.class);
            if (bundle == null) {
                return;
            }
            ...
            register(context, ScorecardReadPlatformService.class, scorecardRead.getIfAvailable());
            LOG.info("Registered {} spm OSGi service(s)", registrations.size());
        } catch (final ClassNotFoundException ex) {
            LOG.debug("OSGi framework classes not present; Spring-only spm wiring");
        } catch (final ReflectiveOperationException ex) {
            LOG.warn("Failed to register spm OSGi services: {}", ex.toString());
        }
    }
