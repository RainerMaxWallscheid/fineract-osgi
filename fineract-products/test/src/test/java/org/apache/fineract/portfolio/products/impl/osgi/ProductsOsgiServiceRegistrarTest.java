...
class ProductsOsgiServiceRegistrarTest {

    @Test
    @SuppressWarnings("unchecked")
    void afterPropertiesSet_withoutOsgi_doesNotThrow() {
        ObjectProvider<ShareProductReadPlatformService> r = mock(ObjectProvider.class);
        ObjectProvider<ProductCommandsService> c = mock(ObjectProvider.class);
        when(r.getIfAvailable()).thenReturn(null);
        when(c.getIfAvailable()).thenReturn(null);
        var reg = new ProductsOsgiServiceRegistrar(r, c);
