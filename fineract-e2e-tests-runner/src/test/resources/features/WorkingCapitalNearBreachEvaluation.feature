...
Feature: Working Capital Near Breach Evaluation
  ...
  Scenario: Verify near breach not triggered when payment brings outstanding below threshold before evaluation date
    ...
    And Customer makes repayment on "20260215" with 700.0 transaction amount on Working Capital loan
  ...
  Scenario: Verify near breach is immutable - stays true after subsequent payment
    ...
    And Customer makes repayment on "20260315" with 900.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260401"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach not triggered when outstanding equals threshold exactly - strict greater than
    ...
    And Customer makes repayment on "20260115" with 450.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260401"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach evaluated independently per breach period
    ...
    When Admin sets the business date to "20260205"
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Customer makes repayment on "20260205" with 300.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260301"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach with non-zero grace days shifts breach period and eval dates
    ...
    When Admin sets the business date to "20260313"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach with PERCENTAGE breach amount and non-zero discount
    ...
    And Admin creates a working capital loan using created product with the following data:
      | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20260101 | 20260101          | 9000            | 100000       | 18                | 500      |
    And Admin successfully approves the working capital loan on "20260101" with "9000" amount and "500" discount amount and expected disbursement date on "20260101"
    When Admin successfully disburse the Working Capital loan on "20260101" with "9000" EUR transaction amount and "500" discount amount
    And Admin runs inline COB job for Working Capital Loan by loanId
  ...
  Scenario: Verify near breach stays null when eval date falls outside period due to February short month
    ...
    And Admin creates a working capital loan using created product with the following data:
      | submittedOnDate  | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20260201 | 20260201         | 9000            | 100000       | 18                | 0        |
    And Admin successfully approves the working capital loan on "20260201" with "9000" amount and expected disbursement date on "20260201"
    When Admin successfully disburse the Working Capital loan on "20260201" with "9000" EUR transaction amount
    And Admin runs inline COB job for Working Capital Loan by loanId
    When Admin sets the business date to "20260401"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
  ...
  Scenario: Verify near breach not triggered with multiple partial payments bringing outstanding below threshold
    ...
    And Customer makes repayment on "20260110" with 200.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260125"
    And Customer makes repayment on "20260125" with 150.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260215"
    And Customer makes repayment on "20260215" with 200.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260401"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach false and breach false when full payment made before first eval date
    ...
    When Admin sets the business date to "20260115"
    And Customer makes repayment on "20260115" with 900.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260401"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach evaluated correctly across 4 consecutive breach periods with mixed results
    ...
    When Admin sets the business date to "20260205"
    And Customer makes repayment on "20260205" with 200.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260301"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
    ...
    And Customer makes repayment on "20260401" with 300.0 transaction amount on Working Capital loan
  ...
  Scenario: Verify near breach eval#1 OK, eval#2 fails with cumulative stepped threshold
    ...
    And Customer makes repayment on "20260105" with 10.0 transaction amount on Working Capital loan
  ...
  Scenario: Verify near breach not triggered when full minimum is paid front-loaded - cumulative requirement satisfied at every eval
    ...
    When Admin sets the business date to "20260102"
    And Customer makes repayment on "20260102" with 90.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260110"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach false when cumulative payments meet stepped requirements across multi-eval period
    ...
    And Customer makes repayment on "20260102" with 30.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260105"
    And Customer makes repayment on "20260105" with 30.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260110"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify that near breach is detected by cumulative paid falling below stepped requirement across two consecutive breach periods - UC1
    ...
    And Admin creates a working capital loan using created product with the following data:
      | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20260101 | 20260101          | 800             | 100000       | 18                | 0        |
    ...
    When Admin sets the business date to "20260102"
    And Customer makes repayment on "20260102" with 200.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260105"
    And Customer makes repayment on "20260105" with 50.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260108"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
    ...
    When Admin sets the business date to "20260114"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify that grace days shift breach period start and near breach is evaluated at shifted eval dates - UC4
    ...
    When Admin sets the business date to "20260105"
    And Customer makes repayment on "20260105" with 100.0 transaction amount on Working Capital loan
  ...
  Scenario: Verify that near breach stays null between evals and is detected when cumulative paid falls short of stepped requirement at a later eval - UC5
    ...
    And Admin creates a working capital loan using created product with the following data:
      | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20260101 | 20260101          | 10000           | 100000       | 18                | 0        |
    And Admin successfully approves the working capital loan on "20260101" with "10000" amount and expected disbursement date on "20260101"
    When Admin successfully disburse the Working Capital loan on "20260101" with "10000" EUR transaction amount
    And Admin runs inline COB job for Working Capital Loan by loanId
    ...
    When Admin sets the business date to "20260102"
    And Customer makes repayment on "20260102" with 400.0 transaction amount on Working Capital loan
  ...
  Scenario: Verify credit balance refund does not mutate already evaluated near breach schedule - UC6
    ...
    When Admin sets the business date to "20260106"
    And Customer makes repayment on "20260106" with 9500.0 transaction amount on Working Capital loan
    And Customer makes credit balance refund on "20260106" with 500.0 transaction amount on Working Capital loan
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach evaluation window shifts when breachGraceDays is set on the product
    ...
    When Admin sets the business date to "20260308"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach RESCHEDULE action - UC2: threshold lowered so period that would not have triggered now triggers
    ...
    When Admin sets the business date to "20260102"
    And Customer makes repayment on "20260102" with 28.0 transaction amount on Working Capital loan
  ...
  Scenario: Verify near breach RESCHEDULE action - UC7: repayment after action keeps nearBreach=false at new eval date
    ...
    And Customer makes repayment on "20260104" with 50.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260108"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify near breach RESCHEDULE action - UC9: goodwill credit after action keeps nearBreach=false at eval date
    ...
    Then Near breach action history has the following data:
      ...
      | RESCHEDULE | 50        | 6         | DAYS          |
    When Admin sets the business date to "20260104"
    And Customer makes "GOODWILL_CREDIT" transaction on "20260104" with 50.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260108"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | minPaymentAmount | outstandingAmount | nearBreach | breach |
