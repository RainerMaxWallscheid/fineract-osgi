...
Feature: Working Capital Breach Evaluation
  ...
  Scenario: Verify that full payment covering minPayment results in breach false after period end
    ...
    And Customer makes repayment on "20260115" with 500.0 transaction amount on Working Capital loan
  ...
  Scenario: Verify that excess payment in a later period does not retroactively clear previous period breach
    ...
    And Customer makes repayment on "20260210" with 1500.0 transaction amount on Working Capital loan
  ...
  Scenario: Verify that multiple partial payments summing to minPayment result in breach false
    ...
    And Customer makes repayment on "20260110" with 150.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260120"
    And Customer makes repayment on "20260120" with 150.0 transaction amount on Working Capital loan
  ...
  Scenario: Verify that payment on exact period end date applies to that period
    ...
    And Customer makes repayment on "20260131" with 500.0 transaction amount on Working Capital loan
    When Admin sets the business date to "20260201"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify that breach evaluation matches CSV example with 90 DAY frequency and partial payment
    ...
    And Admin creates a working capital loan using created product with the following data:
      | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20190101 | 20190101          | 9000            | 100000       | 18                | 1000     |
    ...
    When Admin successfully disburse the Working Capital loan on "20190101" with "9000" EUR transaction amount and "1000" discount amount
    And Admin runs inline COB job for Working Capital Loan by loanId
    ...
    And Customer makes repayment on "20190105" with 250.0 transaction amount on Working Capital loan
    ...
    When Admin sets the business date to "20190405"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify that paidAmount and outstandingAmount update immediately on payment
    ...
    When Admin sets the business date to "20260110"
    And Customer makes repayment on "20260110" with 400.0 transaction amount on Working Capital loan
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
    ...
    When Admin sets the business date to "20260120"
    And Customer makes repayment on "20260120" with 600.0 transaction amount on Working Capital loan
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify that payment exceeding minPayment does not carry over to next period
    ...
    When Admin sets the business date to "20260115"
    And Customer makes repayment on "20260115" with 800.0 transaction amount on Working Capital loan
