...
Feature: Working Capital Breach Schedule
  ...
  Scenario: Verify working capital loan breach schedule - custom breach config with grace days, 10 periods
    ...
    When Admin sets the business date to "20260309"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify working capital loan breach schedule - disburse on month-end (31st) with MONTHS=1
    ...
    And Admin creates a working capital loan using created product with the following data:
      | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20260131 | 20260131          | 9000            | 100000       | 18                | 0        |
    And Admin successfully approves the working capital loan on "20260131" with "9000" amount and expected disbursement date on "20260131"
    When Admin successfully disburse the Working Capital loan on "20260131" with "9000" EUR transaction amount
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify working capital loan breach schedule - leap-year Feb 29 with YEARS=1
    ...
    And Admin creates a working capital loan using created product with the following data:
      | submittedOnDate  | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20240229 | 20240229         | 9000            | 100000       | 18                | 0        |
    And Admin successfully approves the working capital loan on "20240229" with "9000" amount and expected disbursement date on "20240229"
    When Admin successfully disburse the Working Capital loan on "20240229" with "9000" EUR transaction amount
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify working capital loan breach schedule - rejected loan has no schedule
    ...
    And Admin creates a working capital loan using created product with the following data:
      | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20260101 | 20260101          | 9000            | 100000       | 18                | 0        |
    And Admin rejects the working capital loan on "20260101"
    When Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has no data
  ...
  Scenario: Verify that delinquencyStartDate is anchored on the loan creation date when delinquencyStartType is LOAN_CREATION
    ...
    And Admin creates a working capital loan with the grace days product and the following data:
      | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20260101 | 20260108          | 9000            | 100000             | 18                |          |
    And Admin successfully approves the working capital loan on "20260101" with "9000" amount and expected disbursement date on "20260108"
    When Admin sets the business date to "20260108"
    And Admin successfully disburse the Working Capital loan on "20260108" with "9000" EUR transaction amount
    ...
    When Admin sets the business date to "20260207"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working capital loan account has the correct data:
      | breachStartDate | delinquencyStartDate |
      | null            | 2026-01-04           |