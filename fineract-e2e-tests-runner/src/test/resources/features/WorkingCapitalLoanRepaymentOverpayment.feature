...
# --- repay closed loan to make it overpaid --- #
    And Customer makes repayment on "20260121" with 199.0 transaction amount on Working Capital loan
    And Working capital loan account has the correct data:
      | product.name | submittedOnDate | expectedDisbursementDate | status   | principal | approvedPrincipal | totalPaymentVolume | periodPaymentRate | discount | totalPaidPrincipal | overpaymentAmount |
...
#--- make CBR to make loan closed as per finalize test scenario ---#
    And Customer makes credit balance refund on "20260121" with 199.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "CLOSED_OBLIGATIONS_MET"
...
# --- repay active loan with amount to make it overpaid --- #
    When Admin sets the business date to "20260120"
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Customer makes repayment on "20260120" with 9200.0 transaction amount on Working Capital loan
    And Working capital loan account has the correct data:
      | product.name | submittedOnDate | expectedDisbursementDate | status | principal | approvedPrincipal | totalPaymentVolume | periodPaymentRate | discount | totalPaidPrincipal | overpaymentAmount |
...
# --- repay on overpaid loan should be allowed --- #
    ...
    Then Working Capital loan status will be "OVERPAID"
#--- make CBR to make loan closed as per finalize test scenario ---#
    And Customer makes credit balance refund on "20260121" with 399.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "CLOSED_OBLIGATIONS_MET"
...
#   --- Mixed: repayment + goodwill credit in same period ---
    ...
    And Customer makes repayment on "20260112" with 8900.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "OVERPAID"
    And Working capital loan account has the correct data:
      | principal | approvedPrincipal | totalPaymentVolume | periodPaymentRate | discount | totalPaidPrincipal | realizedIncome | unrealizedIncome | overpaymentAmount |
    ...
    And Working Capital Loan has transactions:
      ...
      | 20260101 | Disbursement    | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
      | 20260110 | Goodwill Credit | 170.0             | 170.0            | 0.0               | 0.0                   | false    |
      | 20260112 | Repayment       | 8900.0            | 8830.0           | 0.0               | 0.0                   | false    |
#--- make CBR to make loan closed as per finalize test scenario ---#
    And Customer makes credit balance refund on "20260112" with 70.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "CLOSED_OBLIGATIONS_MET"
...
# --- repay loan to make it closed --- #
    When Admin sets the business date to "20260112"
    And Customer makes repayment on "20260112" with 9035.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "CLOSED_OBLIGATIONS_MET"
    And Working capital loan account has the correct data:
      | principal | approvedPrincipal | totalPaymentVolume | periodPaymentRate | discount | totalPaidPrincipal | realizedIncome | unrealizedIncome | overpaymentAmount |
    ...
    And Working Capital Loan has transactions:
      | transactionDate | type            | transactionAmount | principalPortion | feeChargesPortion | penaltyChargesPortion | reversed |
      | 20260101 | Disbursement    | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
      | 20260112 | Repayment       | 9035.0            | 9000.0           | 0.0               | 35.0                  | false    |
    When Admin sets the business date to "20260115"
    And Customer makes repayment on "20260115" with 120.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "OVERPAID"
    And Working capital loan account has the correct data:
      | principal | approvedPrincipal | totalPaymentVolume | periodPaymentRate | discount | totalPaidPrincipal | realizedIncome | unrealizedIncome | overpaymentAmount |
    ...
    And Working Capital Loan has transactions:
      ...
      | 20260101 | Disbursement    | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
      | 20260112 | Repayment       | 9035.0            | 9000.0           | 0.0               | 35.0                  | false    |
      | 20260115 | Repayment       | 120.0             | 0.0              | 0.0               | 0.0                   | false    |
#--- make CBR to make loan closed as per finalize test scenario ---#
    And Customer makes credit balance refund on "20260115" with 120.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "CLOSED_OBLIGATIONS_MET"
...
# --- add discount after disbursement on the same disbursement date --- #
    ...
    When Admin sets the business date to "20260115"
    And Customer makes repayment on "20260115" with 9020.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "OVERPAID"
    And Working capital loan account has the correct data:
      | principal | approvedPrincipal | totalPaymentVolume | periodPaymentRate | discount | totalPaidPrincipal | realizedIncome | unrealizedIncome | overpaymentAmount |
    ...
    And Working Capital Loan has transactions:
      ...
      | 20260101 | Disbursement    | 9000.0            | 9000.0           | 0.0               | 0.0                   | false    |
      | 20260101 | Discount Fee    | 12.0              | 12.0             | 0.0               | 0.0                   | false    |
      | 20260115 | Repayment       | 9020.0            | 9012.0           | 0.0               | 0.0                  | false    |
#--- make backdated discount fee adjustment to make loan closed as per finalize test scenario ---#
    And Customer makes credit balance refund on "20260115" with 8.0 transaction amount on Working Capital loan
    Then Working Capital loan status will be "CLOSED_OBLIGATIONS_MET"
