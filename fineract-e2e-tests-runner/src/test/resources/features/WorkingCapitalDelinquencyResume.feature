...
Feature: Working Capital Delinquency Resume
  ...
  Scenario: Verify working capital loan delinquency resume - UC1: resume shortens active pause and delinquency range schedule
    ...
    And Admin initiate a Working Capital loan delinquency pause with startDate "20260101" and endDate "20260116"
    ...
    And Admin initiate a Working Capital loan delinquency resume with startDate "20260110"
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-01 | 2026-01-16 |
      | RESUME | 2026-01-10 |            |
  ...
  Scenario: Verify working capital loan delinquency resume - UC2: resume on the day before planned pause end shortens schedule by one day
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan delinquency resume with startDate "20260115"
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-01 | 2026-01-16 |
      | RESUME | 2026-01-15 |            |
  ...
  Scenario: Verify working capital loan delinquency resume - UC3: resume by external ID
    ...
    When Admin successfully disburse the Working Capital loan on "20260101" with "9000" EUR transaction amount
    When Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan delinquency pause by external ID with startDate "20260101" and endDate "20260116"
    When Admin sets the business date to "20260110"
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan delinquency resume by external ID with startDate "20260110"
  ...
  Scenario: Verify working capital loan delinquency resume - UC4: resume without an active pause results an error (Negative)
    ...
    When Admin sets the business date to "20260110"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Initiating a Working Capital loan delinquency resume with startDate "20260110" results an error with the following data:
  ...
  Scenario: Verify working capital loan delinquency resume - UC5: backdated resume results an error (Negative)
    ...
    When Admin sets the business date to "20260110"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Initiating a Working Capital loan delinquency resume with startDate "20260109" results an error with the following data:
      | httpCode | errorMessage                                                                  |
  ...
  Scenario: Verify working capital loan delinquency resume - UC6: resume on pause start date results an error (Negative)
    ...
    When Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan delinquency pause with startDate "20260101" and endDate "20260116"
    Then Initiating a Working Capital loan delinquency resume with startDate "20260101" results an error with the following data:
      | httpCode | errorMessage                                          |
  ...
  Scenario: Verify working capital loan delinquency resume - UC8: resume recalculates delinquency immediately
    ...
    And Admin initiate a Working Capital loan delinquency pause with startDate "20260215" and endDate "20260315"
    Then Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-01-30 | 270.0          | 0.0        | 270.0             | false                 | 270.0            | 16             |
      | 2            | 2026-01-31 | 2026-03-30 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    When Admin sets the business date to "20260220"
    And Admin initiate a Working Capital loan delinquency resume with startDate "20260220"
    Then Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
  ...
  Scenario: Verify resume keeps the PAUSE action dates unchanged and only shortens the delinquency period extension
    ...
    And Admin initiate a Working Capital loan delinquency pause with startDate "20260128" and endDate "20260220"
    When Admin sets the business date to "20260129"
    And Admin initiate a Working Capital loan delinquency resume with startDate "20260129"
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
  ...
  Scenario: Verify that COB-generated period after resume must honour the shortened (resumed) pause
    ...
    Then Admin closes the Working Capital loan with a full repayment on "20260202"
  ...
  Scenario: Verify new pause is allowed after resumed pause when outside effective pause window
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan delinquency pause with startDate "20260205" and endDate "20260210"
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-28 | 2026-02-20 |
      | RESUME | 2026-01-29 |            |
    ...
    Then Admin closes the Working Capital loan with a full repayment on "20260205"
  ...
  # both pause dates are INCLUSIVE, so startDate == endDate is a valid 1-day pause
    ...
    When Admin successfully disburse the Working Capital loan on "20260101" with "9000" EUR transaction amount
    When Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan delinquency pause with startDate "20260110" and endDate "20260110"
    Then Working Capital loan delinquency action has the following data:
      | action | startDate  | endDate    |
  ...
  # while paused it must stay non-delinquent (null) - i.e. delinquency does not grow across the paused period's start/due boundary.
    ...
    And Admin initiate a Working Capital loan delinquency pause with startDate "20260130" and endDate "20260210"
    Then Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
    ...
    Then Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-02-11 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    Then Admin closes the Working Capital loan with a full repayment on "20260131"
  ...
  # On 05-Feb (the shifted toDate) the period is still open (null); delinquency only resumes on 06-Feb (= 1).
    ...
    When Admin runs inline COB job for Working Capital Loan by loanId
    When Admin sets the business date to "20260110"
    And Admin initiate a Working Capital loan delinquency pause with startDate "20260110" and endDate "20260220"
    When Admin sets the business date to "20260115"
    And Admin initiate a Working Capital loan delinquency resume with startDate "20260115"
    Then Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
    ...
    Then Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-02-05 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    When Admin sets the business date to "20260206"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan delinquency range schedule has the following data:
      | periodNumber | fromDate   | toDate     | expectedAmount | paidAmount | outstandingAmount | minPaymentCriteriaMet | delinquentAmount | delinquentDays |
      | 1            | 2026-01-01 | 2026-02-05 | 270.0          | 0.0        | 270.0             | false                 | 270.0            | 1              |
      | 2            | 2026-02-06 | 2026-03-07 | 270.0          | 0.0        | 270.0             | null                  | null             | null           |
    Then Admin closes the Working Capital loan with a full repayment on "20260206"
