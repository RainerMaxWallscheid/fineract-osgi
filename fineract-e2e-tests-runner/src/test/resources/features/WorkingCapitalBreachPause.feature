...
Feature: Working Capital Breach Pause
  ...
  Scenario: Verify working capital loan breach pause - pause in current period extends breach schedule and does not affect delinquency schedule
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach pause with startDate "20260115" and endDate "20260125"
  ...
  Scenario: Verify working capital loan breach pause - backdated pause re-triggers evaluation of an already evaluated period
    ...
    When Admin initiate a Working Capital loan breach pause with startDate "20260220" and endDate "20260302"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
    ...
    Then Working Capital loan breach schedule has the following data:
      ...
      | 2            | 2026-03-12 | 2026-05-11 | 61           | 110.70           | 110.70            | null       | null   |
    Then Admin closes the Working Capital loan with a full repayment on "20260312"
  ...
  Scenario: Verify working capital loan breach pause - backdated pause keeps breach flag when extended period still ends in the past
    ...
    Then Working Capital loan breach schedule has the following data:
      ...
      | 1            | 2026-01-01 | 2026-02-28 | 59           | 110.70           | 110.70            | null       | true   |
      | 2            | 2026-03-01 | 2026-04-30 | 61           | 110.70           | 110.70            | null       | null   |
    When Admin initiate a Working Capital loan breach pause with startDate "20260220" and endDate "20260225"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
  ...
  Scenario: Verify working capital loan breach pause - multiple non-overlapping pauses are cumulative
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach pause with startDate "20260115" and endDate "20260125"
    And Admin initiate a Working Capital loan breach pause with startDate "20260201" and endDate "20260206"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-15 | 2026-01-25 |
  ...
  Scenario: Verify working capital loan breach pause - overlapping pauses are rejected
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach pause with startDate "20260115" and endDate "20260125"
    Then Initiating a Working Capital loan breach pause with startDate "20260120" and endDate "20260130" results an error with the following data:
      | httpCode | message                                                  |
      | 400      | Failed data validation due to: overlapping.pause.periods |
    And Initiating a Working Capital loan breach pause with startDate "20260110" and endDate "20260130" results an error with the following data:
      | httpCode | message                                                  |
      | 400      | Failed data validation due to: overlapping.pause.periods |
  ...
  Scenario: Verify working capital loan breach pause - breach pause and delinquency pause are independent
    ...
    And Admin initiate a Working Capital loan breach pause with startDate "20260110" and endDate "20260130"
  ...
  Scenario: Verify working capital loan breach pause - next period is generated from the extended period and recorded pauses apply to it
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach pause with startDate "20260115" and endDate "20260125"
    And Admin initiate a Working Capital loan breach pause with startDate "20260320" and endDate "20260325"
    When Admin sets the business date to "20260312"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify working capital loan breach pause - future pause beyond the schedule end is preserved when a later backdated pause extends the period over its window
    ...
    When Admin sets the business date to "20260115"
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach pause with startDate "20260305" and endDate "20260308"
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
      | 1            | 2026-01-01 | 2026-02-28 | 59           | 110.70           | 110.70            | null       | null   |
    When Admin initiate a Working Capital loan breach pause with startDate "20260201" and endDate "20260210"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
  ...
  Scenario: Verify working capital loan breach pause - pause created before the first COB run is applied to the initial period
    ...
    And Admin successfully approves the working capital loan on "20260101" with "9000" amount and expected disbursement date on "20260101"
    When Admin successfully disburse the Working Capital loan on "20260101" with "9000" EUR transaction amount
    And Admin initiate a Working Capital loan breach pause with startDate "20260105" and endDate "20260115"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
  ...
  Scenario: Verify working capital loan breach pause - validation errors
    ...
    When Admin successfully disburse the Working Capital loan on "20260101" with "9000" EUR transaction amount
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Initiating a Working Capital loan breach pause with startDate "20260125" and endDate "20260115" results an error with the following data:
      | httpCode | message                                                      |
      | 400      | Failed data validation due to: must.be.on.or.after.startDate |
    And Initiating a Working Capital loan breach pause with startDate "20251225" and endDate "20260105" results an error with the following data:
      | httpCode | message                                                                                  |
      | 400      | The parameter `startDate` must be greater than or equal to the provided date: 2026-01-01 |
    And Initiating a Working Capital loan breach action "invalid" with startDate "20260115" and endDate "20260125" results an error with the following data:
      | httpCode | message                                                             |
  ...
  Scenario: Verify working capital loan breach pause - pause is rejected for a loan without breach configuration
    ...
    When Admin successfully disburse the Working Capital loan on "20260101" with "9000" EUR transaction amount
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Initiating a Working Capital loan breach pause with startDate "20260115" and endDate "20260125" results an error with the following data:
      | httpCode | message                                                |
  ...
  Scenario: Verify working capital loan breach pause - pause start date is validated against the grace-shifted breach schedule start
    ...
    And Admin creates a working capital loan using created product with the following data:
      | submittedOnDate | expectedDisbursementDate | principalAmount | totalPaymentVolume | periodPaymentRate | discount |
      | 20260101 | 20260101          | 9000            | 100000             | 18                | 1000     |
    And Admin successfully approves the working capital loan on "20260101" with "9000" amount and expected disbursement date on "20260101"
    When Admin successfully disburse the Working Capital loan on "20260101" with "9000" EUR transaction amount and "1000" discount amount
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
      | 1            | 2026-01-04 | 2026-01-10 | 7            | 900.00           | 900.00            | null       | null   |
    And Initiating a Working Capital loan breach pause with startDate "20260101" and endDate "20260110" results an error with the following data:
      | httpCode | message                                                                                  |
      | 400      | The parameter `startDate` must be greater than or equal to the provided date: 2026-01-04 |
    When Admin initiate a Working Capital loan breach pause with startDate "20260104" and endDate "20260108"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
  ...
  Scenario: Verify working capital loan breach pause - backdated payment resets breach flag of an already breached period
    ...
    Then Working Capital loan breach schedule has the following data:
      ...
      | 1            | 2026-01-01 | 2026-02-28 | 59           | 110.70           | 110.70            | null       | true   |
      | 2            | 2026-03-01 | 2026-04-30 | 61           | 110.70           | 110.70            | null       | null   |
    When Admin initiate a Working Capital loan breach pause with startDate "20260320" and endDate "20260330"
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
      | 1            | 2026-01-01 | 2026-02-28 | 59           | 110.70           | 110.70            | null       | true   |
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Customer makes repayment on "20260215" with 150.0 transaction amount on Working Capital loan
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
      | 1            | 2026-01-01 | 2026-02-28 | 59           | 110.70           | 0.00              | null       | false  |
      | 2            | 2026-03-01 | 2026-05-11 | 72           | 110.70           | 110.70            | null       | null   |
    Then Admin closes the Working Capital loan with a full repayment on "20260414"
  ...
  Scenario: Verify working capital loan breach pause - touching pauses are rejected and contiguous pauses must not share a day
    ...
    Then Initiating a Working Capital loan breach pause with startDate "20260125" and endDate "20260205" results an error with the following data:
      | httpCode | message                                                  |
      | 400      | Failed data validation due to: overlapping.pause.periods |
    # a contiguous pause must start the day after the previous one ends, so the boundary day is not double-counted
    When Admin initiate a Working Capital loan breach pause with startDate "20260126" and endDate "20260205"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-15 | 2026-01-25 |
  ...
  Scenario: Verify working capital loan breach pause - a pre-existing payment is not re-bucketed when a later pause shifts the period boundary across its date
    ...
    And Customer makes repayment on "20260305" with 50.0 transaction amount on Working Capital loan
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
      | 1            | 2026-01-01 | 2026-02-28 | 59           | 110.70           | 110.70            | null       | true   |
    ...
    When Admin initiate a Working Capital loan breach pause with startDate "20260115" and endDate "20260131"
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
  ...
  Scenario: Verify working capital loan breach pause - pause can be applied and retrieved by loan external id
    ...
    When Admin sets the business date to "20260115"
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach pause by external ID with startDate "20260115" and endDate "20260125"
  ...
  Scenario: Verify working capital loan breach pause - a single day pause with the same start and end date extends the schedule by one day
    ...
    When Admin sets the business date to "20260115"
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach pause with startDate "20260120" and endDate "20260120"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
  ...
  Scenario: Verify working capital loan breach pause - a single day pause on the schedule start date extends the schedule by one day
    ...
    When Admin initiate a Working Capital loan breach pause with startDate "20260101" and endDate "20260101"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
  ...
  Scenario: Verify working capital loan breach pause - resume shortens the active pause and recalculates the breach schedule
    ...
    And Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
      | 1            | 2026-01-01 | 2026-02-28 | 59           | 110.70           | 110.70            | null       | null   |
    And Admin initiate a Working Capital loan breach pause with startDate "20260101" and endDate "20260116"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach resume with startDate "20260110"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-01 | 2026-01-16 |
      | RESUME | 2026-01-10 |            |
    ...
    Then Admin closes the Working Capital loan with a full repayment on "20260110"
  ...
  Scenario: Verify working capital loan breach pause - a new pause can start after the effective resumed pause ends
    ...
    When Admin initiate a Working Capital loan breach pause with startDate "20260111" and endDate "20260116"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-01 | 2026-01-16 |
      | RESUME | 2026-01-10 |            |
  ...
  Scenario: Verify working capital loan breach pause - resume is rejected when there is no active pause on the resume date
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Initiating a Working Capital loan breach resume with startDate "20260120" results an error with the following data:
      | httpCode | message                                                       |
      | 400      | Failed data validation due to: resume.not.during.active.pause |
    Then Admin closes the Working Capital loan with a full repayment on "20260120"
  ...
  Scenario: Verify working capital loan breach pause - resume is rejected when the resume date is not the current business date
    ...
    When Admin sets the business date to "20260110"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Initiating a Working Capital loan breach resume with startDate "20260111" results an error with the following data:
      | httpCode | message                                                      |
  ...
  Scenario: Verify working capital loan breach pause - resume with an end date is rejected
    ...
    When Admin sets the business date to "20260110"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Initiating a Working Capital loan breach action "resume" with startDate "20260110" and endDate "20260120" results an error with the following data:
      | httpCode | message                                                        |
  ...
  Scenario: Verify working capital loan breach pause - resume is rejected for a pause that was already resumed
    ...
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach resume with startDate "20260110"
    Then Initiating a Working Capital loan breach resume with startDate "20260110" results an error with the following data:
      | httpCode | message                                                       |
      | 400      | Failed data validation due to: resume.not.during.active.pause |
    Then Admin closes the Working Capital loan with a full repayment on "20260110"
  ...
  Scenario: Verify working capital loan breach pause - resume shortens the pause so the period breaches earlier
    ...
    Then Admin closes the Working Capital loan with a full repayment on "20260311"
  ...
  Scenario: Verify working capital loan breach pause - resume on the day before planned pause end shortens schedule by one day
    ...
    When Admin sets the business date to "20260115"
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach resume with startDate "20260115"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-01 | 2026-01-16 |
      | RESUME | 2026-01-15 |            |
  ...
  Scenario: Verify working capital loan breach pause - resume by external ID
    ...
    And Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
      | 1            | 2026-01-01 | 2026-02-28 | 59           | 110.70           | 110.70            | null       | null   |
    And Admin initiate a Working Capital loan breach pause by external ID with startDate "20260101" and endDate "20260116"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-01 | 2026-01-16 |
    And Working Capital loan breach schedule has the following data:
    ...
    When Admin sets the business date to "20260110"
    And Admin runs inline COB job for Working Capital Loan by loanId
    And Admin initiate a Working Capital loan breach resume by external ID with startDate "20260110"
    Then Working Capital loan breach action by external ID has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-01 | 2026-01-16 |
      | RESUME | 2026-01-10 |            |
  ...
  Scenario: Verify working capital loan breach pause - backdated resume is rejected
    ...
    When Admin sets the business date to "20260110"
    And Admin runs inline COB job for Working Capital Loan by loanId
    Then Initiating a Working Capital loan breach resume with startDate "20260109" results an error with the following data:
      | httpCode | message                                                      |
      | 400      | Failed data validation due to: must.be.current.business.date |
    Then Admin closes the Working Capital loan with a full repayment on "20260110"
  ...
  Scenario: Verify working capital loan breach pause - resume keeps the PAUSE action dates unchanged and only shortens the breach schedule
    ...
    And Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
      | 1            | 2026-01-01 | 2026-02-28 | 59           | 110.70           | 110.70            | null       | null   |
    And Admin initiate a Working Capital loan breach pause with startDate "20260128" and endDate "20260220"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
    ...
    And Admin initiate a Working Capital loan breach resume with startDate "20260129"
    Then Working Capital loan breach action has the following data:
      | action | startDate  | endDate    |
      | PAUSE  | 2026-01-28 | 2026-02-20 |
      | RESUME | 2026-01-29 |            |
    ...
    Then Admin closes the Working Capital loan with a full repayment on "20260129"
  ...
  Scenario: Verify working capital loan breach pause - COB-generated period after resume honours the shortened pause
    ...
    Then Admin closes the Working Capital loan with a full repayment on "20260303"
  ...
  Scenario: Verify working capital loan breach pause - resume after a reschedule keeps the rescheduled minimum payment and shortens the schedule
    ...
    When Admin initiate a Working Capital loan breach pause with startDate "20260105" and endDate "20260116"
    Then Working Capital loan breach schedule has the following data:
      | periodNumber | fromDate   | toDate     | numberOfDays | minPaymentAmount | outstandingAmount | nearBreach | breach |
