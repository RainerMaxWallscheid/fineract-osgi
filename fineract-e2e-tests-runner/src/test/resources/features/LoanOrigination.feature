...
Feature: Loan Origination
  ...
  Scenario: Verify loan originator registration, attachment to loan, and persistence through approval and disbursal
    ...
    When Admin creates a new default Loan with date: "20250101"
  ...
  Scenario: Verify loan originator inline attachment with auto-creation during loan application
    ...
    When Admin sets the business date to "20250101"
    When Admin creates a client with random data
    When Admin creates a new Loan with originator inline submitted on date: "20250101"
    Then Loan details with association "originators" has the originator attached
    And Admin successfully approves the loan on "20250101" with "1000" amount and expected disbursement date on "20250101"
    When Admin successfully disburse the loan on "20250101" with "1000" EUR transaction amount
    Then Loan details with association "originators" has the originator attached
  ...
  Scenario: Verify loan originator details in external business event after loan approval
    ...
    When Admin creates a new default Loan with date: "20250101"
    When Admin attaches the originator to the loan
    When Admin approves the loan on "20250101" with "1000" amount and expected disbursement date on "20250101"
  ...
  Scenario: Verify loan originator detachment from loan before approval
    ...
    And Admin successfully rejects the loan on "20250101"
  ...
  Scenario: Verify loan originator persistence through full loan lifecycle with repayments
    ...
    When Admin creates a fully customized loan with the following data:
      ...
      | LP2_ADV_CUSTOM_PMT_ALLOC_PROGRESSIVE_LOAN_SCHEDULE_HORIZONTAL | 20250101   | 1000           | 12                     | DECLINING_BALANCE | DAILY                      | EQUAL_INSTALLMENTS | 3                 | MONTHS                | 1              | MONTHS                 | 3                  | 0                       | 0                      | 0                    | ADVANCED_PAYMENT_ALLOCATION |
    When Admin attaches the originator to the loan
    Then Loan details with association "originators" has the originator with all fields attached
    And Admin successfully approves the loan on "20250101" with "1000" amount and expected disbursement date on "20250101"
    When Admin successfully disburse the loan on "20250101" with "1000" EUR transaction amount
    ...
    And Customer makes "AUTOPAY" repayment on "20250201" with 340 EUR transaction amount
    Then Loan details with association "originators" has the originator with all fields attached
    When Admin sets the business date to "20250301"
    And Customer makes "AUTOPAY" repayment on "20250301" with 340 EUR transaction amount
    Then Loan details with association "originators" has the originator with all fields attached
  ...
  Scenario: Verify loan originator persistence through loan charge-off
    ...
    When Admin successfully disburse the loan on "20250101" with "1000" EUR transaction amount
    Then Loan details with association "originators" has the originator attached
    And Admin does charge-off the loan on "20250101"
    Then Loan details with association "originators" has the originator attached
    When Loan Pay-off is made on "20250101"
    Then Loan is closed with zero outstanding balance and it's all installments have obligations met
  ...
  Scenario: Verify that originator details are present in LoanAdjustTransactionBusinessEvent after repayment reversal
    ...
    And Admin successfully approves the loan on "20250101" with "1000" amount and expected disbursement date on "20250101"
    When Admin successfully disburse the loan on "20250101" with "1000" EUR transaction amount
    And Customer makes "AUTOPAY" repayment on "20250101" with 500 EUR transaction amount
    When Customer makes a repayment undo on "20250101" without event check
  ...
  Scenario: Verify that originator details are present in LoanAccrualTransactionCreatedBusinessEvent after COB runs
    ...
    And Admin successfully approves the loan on "20250101" with "1000" amount and expected disbursement date on "20250101"
    When Admin successfully disburse the loan on "20250101" with "1000" EUR transaction amount
    When Admin adds "LOAN_SNOOZE_FEE" due date charge with "20250101" due date and 10 EUR transaction amount
    When Admin sets the business date to "20250102"
    When Admin runs inline COB job for Loan
    Then LoanAccrualTransactionCreatedBusinessEvent is created with originator details on "20250101"
  ...
  Scenario: Verify that originator details are present in LoanAdjustTransactionBusinessEvent after charge waiver reversal
    ...
    When Customer reverses the waiver transaction on "20250101"
    Then LoanAdjustTransactionBusinessEvent is created with originator details in "transactionToAdjust"
    When Loan Pay-off is made on "20250101"
    Then Loan is closed with zero outstanding balance and it's all installments have obligations met
  ...
  Scenario: Verify that originator details are present in LoanAdjustTransactionBusinessEvent new transaction detail after repayment adjustment
    ...
    When Admin successfully disburse the loan on "20250101" with "1000" EUR transaction amount
    And Customer makes "AUTOPAY" repayment on "20250101" with 500 EUR transaction amount
    When Customer adjusts the repayment on "20250101" to 300 EUR without event check
  ...
  Scenario: Verify that LoanAccountDelinquencyRangeDataV1 and LoanRepaymentDueDataV1 has the correct data for Originators - UC1: no originator attached
    ...
    When Admin creates a fully customized loan with the following data:
      | LoanProduct                                                                                       | submitted on date | with Principal | ANNUAL interest rate % | interest type | interest calculation period | amortization type  | loanTermFrequency | loanTermFrequencyType | repaymentEvery | repaymentFrequencyType | numberOfRepayments | graceOnPrincipalPayment | graceOnInterestPayment | interest free period | Payment strategy            |
      | LP2_DOWNPAYMENT_ADV_PMT_ALLOC_PROGRESSIVE_LOAN_SCHEDULE_HORIZONTAL_INSTALLMENT_LEVEL_DELINQUENCY | 20250101   | 1000           | 0                      | FLAT          | SAME_AS_REPAYMENT_PERIOD    | EQUAL_INSTALLMENTS | 3                | MONTHS                  | 1             | MONTHS                   | 3                  | 0                       | 0                      | 0                    | ADVANCED_PAYMENT_ALLOCATION |
    And Admin successfully approves the loan on "20250101" with "1000" amount and expected disbursement date on "20250101"
    When Admin successfully disburse the loan on "20250101" with "1000" EUR transaction amount
    When Admin sets the business date to "20250201"
    When Admin runs inline COB job for Loan
  ...
  Scenario: Verify that originator details are present in LoanRepaymentDueBusinessEvent after inline COB runs
    ...
    When Admin creates a new Loan with date: "20230101" and with 1 day loan term and repayment
    When Admin attaches the originator to the loan
    ...
    When Loan Pay-off is made on "20230102"
    Then Loan is closed with zero outstanding balance and it's all installments have obligations met
  ...
  Scenario: Verify that originator details are present in LoanDelinquencyRangeChangeEvent when loan becomes delinquent
    ...
    When Loan Pay-off is made on "20230107"
    Then Loan is closed with zero outstanding balance and it's all installments have obligations met
