...
public class AccountingRuleWritePlatformServiceJpaRepositoryImpl implements AccountingRuleWritePlatformService {
     ...
     * @param dve
     */
    private void handleAccountingRuleIntegrityIssues(final JsonCommand command, final Throwable realCause, final NonTransientDataAccessException dve) {
        ...
        throw ErrorHandler.getMappable(dve, "error.msg.accounting.rule.unknown.data.integrity.issue", "Unknown data integrity issue with resource Accounting Rule: " + realCause.getMessage());
    }
    ...
    public CommandProcessingResult createAccountingRule(final JsonCommand command) {
        try {
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withOfficeId(officeId).withEntityId(accountingRule.getId()).build();
    ...
    private AccountingRule assembleAccountingRuleAndTags(final Office office, final JsonCommand command) {
        ...
        final AccountingRule accountingRule = AccountingRule.fromJson(office, debitAccount, creditAccount, command, allowMultipleCreditEntries, allowMultipleDebitEntries);
    ...
    public CommandProcessingResult updateAccountingRule(final Long accountingRuleId, final JsonCommand command) {
        try {
            ...
            if (creditTags != null && creditTags.length > 0 && command.parameterExists(AccountingRuleJsonInputParams.CREDIT_ACCOUNT_TAGS.getValue())) {
                final Set<String> creditTagsToAdd = determineCreditTagToAddAndRemoveOldTags(creditTags, JournalEntryType.CREDIT, accountingRule);
            ...
            if (debitTags != null && debitTags.length > 0 && command.parameterExists(AccountingRuleJsonInputParams.DEBIT_ACCOUNT_TAGS.getValue())) {
                final Set<String> debitTagsToAdd = determineCreditTagToAddAndRemoveOldTags(debitTags, JournalEntryType.DEBIT, accountingRule);
            ...
            //
            //
            new CommandProcessingResultBuilder().withCommandId(command.commandId()).withEntityId(accountingRule.getId()).with(changesOnly).build();
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            final Throwable throwable = dve.getMostSpecificCause();
            handleAccountingRuleIntegrityIssues(command, throwable, dve);
            return CommandProcessingResult.empty();
    ...
    private Set<String> determineCreditTagToAddAndRemoveOldTags(final String[] creditOrDebitTags, final JournalEntryType type, final AccountingRule accountingRule) {
    ...
    public CommandProcessingResult deleteAccountingRule(final Long accountingRuleId) {
        ...
        return  //
        //
        new CommandProcessingResultBuilder().withEntityId(accountingRule.getId()).build();
    }

    private List<AccountingTagRule> saveDebitOrCreditTags(final Set<String> creditOrDebitTagArray, final JournalEntryType transactionType, final List<AccountingTagRule> accountingTagRules) {
