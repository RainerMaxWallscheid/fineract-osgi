...
public interface ProductToGLAccountMappingRepository
    ...
    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.productId =:productId and mapping.productType =:productType and mapping.financialAccountType=:financialAccountType and mapping.paymentTypeId is NULL and mapping.chargeId is NULL and mapping.chargeOffReasonId is NULL and mapping.writeOffReasonId is NULL and mapping.capitalizedIncomeClassificationId is NULL and mapping.buydownFeeClassificationId is NULL")
     ...
     ***/
    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.productId =:productId and mapping.productType =:productType and mapping.financialAccountType=1 and mapping.paymentTypeId is not NULL")
    ...
    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.productId =:productId and mapping.productType =:productType and mapping.writeOffReasonId is not NULL")
    List<ProductToGLAccountMapping> findAllWriteOffReasonsMappings(@Param("productId") Long productId,
            @Param("productType") int productType);

    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.chargeOffReasonId =:chargeOffReasonId AND mapping.productId =:productId AND mapping.productType =:productType")
    ...
    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.productId =:productId AND mapping.productType =:productType AND mapping.chargeId is NULL AND mapping.paymentTypeId IS NULL AND mapping.chargeOffReasonId IS NULL AND mapping.writeOffReasonId IS NULL AND mapping.capitalizedIncomeClassificationId is NULL AND mapping.buydownFeeClassificationId is NULL")
    ...
    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.productId =:productId and mapping.productType =:productType and mapping.paymentTypeId is not NULL")
    List<ProductToGLAccountMapping> findAllPaymentTypeMappings(@Param("productId") Long productId,
            @Param("productType") Integer productType);

    /**
     * All product↔GL rows that reference a catalog charge. Penalty vs fee filtering is done in the
     * service layer via {@code ChargeDefinitionPort} (no JPA association to charge-impl).
     */
    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.productId =:productId AND mapping.productType =:productType AND mapping.chargeId is not null")
    List<ProductToGLAccountMapping> findAllChargeIdMappings(@Param("productId") Long productId, @Param("productType") Integer productType);

    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.productId =:productId and mapping.productType =:productType and mapping.capitalizedIncomeClassificationId is not NULL")
    List<ProductToGLAccountMapping> findAllCapitalizedIncomeClassificationsMappings(@Param("productId") Long productId,
            @Param("productType") int productType);

    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.productId =:productId and mapping.productType =:productType and mapping.buydownFeeClassificationId is not NULL")
    List<ProductToGLAccountMapping> findAllBuyDownFeeClassificationsMappings(@Param("productId") Long productId,
            @Param("productType") int productType);

    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.buydownFeeClassificationId = :classificationId AND mapping.productId = :productId AND mapping.productType = :productType")
    ...
    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.capitalizedIncomeClassificationId = :classificationId AND mapping.productId = :productId AND mapping.productType = :productType")
    ProductToGLAccountMapping findCapitalizedIncomeClassificationMapping(@Param("productId") Long productId,
            @Param("productType") Integer productType, @Param("classificationId") Long classificationId);

    @Query("select mapping from ProductToGLAccountMapping mapping where mapping.writeOffReasonId = :writeOffReasonId AND mapping.productId = :productId AND mapping.productType = :productType")
