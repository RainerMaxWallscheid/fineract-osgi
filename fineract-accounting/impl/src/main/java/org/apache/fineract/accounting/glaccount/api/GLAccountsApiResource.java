...
public class GLAccountsApiResource {
    ...
    @Operation(tags = {"General Ledger Account"}, summary = "Retrieve GL Accounts Template", description = """
    ...
    @GET
    @Produces({MediaType.APPLICATION_JSON})
    @Operation(tags = {"General Ledger Account"}, summary = "List General Ledger Account", description = """
        ARGUMENTS
    ...
    @Produces({MediaType.APPLICATION_JSON})
    @Operation(tags = {"General Ledger Account"}, summary = "Create a General Ledger Account", operationId = "createGLAccount", description = """
    ...
    public CommandProcessingResult createGLAccount(@Parameter(hidden = true) GLAccountCommand glAccountCommand) {
        final CommandWrapper commandRequest = new CommandWrapperBuilder().createGLAccount().withJson(apiJsonSerializerService.serialize(glAccountCommand)).build();
    ...
    @Consumes({MediaType.APPLICATION_JSON})
    @Produces({MediaType.APPLICATION_JSON})
    @Operation(tags = {"General Ledger Account"}, summary = "Update a GL Account", operationId = "updateGLAccount", description = "Updates a GL Account")
    ...
    public CommandProcessingResult updateGLAccount(@PathParam("glAccountId") @Parameter(description = "glAccountId") final Long glAccountId, @Parameter(hidden = true) GLAccountCommand accountCommand) {
        final CommandWrapper commandRequest = new CommandWrapperBuilder().updateGLAccount(glAccountId).withJson(apiJsonSerializerService.serialize(accountCommand)).build();
        return this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
    }
    ...
    @Path("{glAccountId}")
    @Produces({MediaType.APPLICATION_JSON})
    @Operation(tags = {"General Ledger Account"}, summary = "Delete a GL Account", operationId = "deleteGLAccount", description = "Deletes a GL Account")
    ...
    public CommandProcessingResult deleteGLAccount(@PathParam("glAccountId") @Parameter(description = "glAccountId") final Long glAccountId) {
    ...
    @RequestBody(description = "Upload GL accounts template", content = {@Content(mediaType = MediaType.MULTIPART_FORM_DATA, schema = @Schema(implementation = UploadRequest.class))})
    public Long postGlAccountsTemplate(@FormDataParam("file") InputStream uploadedInputStream, @FormDataParam("file") FormDataContentDisposition fileDetail, @FormDataParam("locale") final String locale, @FormDataParam("dateFormat") final String dateFormat) {
        return bulkImportWorkbookService.importWorkbook(GlobalEntityType.CHART_OF_ACCOUNTS.toString(), uploadedInputStream, fileDetail, locale, dateFormat);
    }
    ...
    private GLAccountData handleTemplate(final GLAccountData glAccountData) {
        ...
        return new GLAccountData().setId(glAccountData.getId()).setName(glAccountData.getName()).setParentId(glAccountData.getParentId()).setGlCode(glAccountData.getGlCode()).setDisabled(glAccountData.getDisabled()).setManualEntriesAllowed(glAccountData.getManualEntriesAllowed()).setType(glAccountData.getType()).setUsage(glAccountData.getUsage()).setDescription(glAccountData.getDescription()).setNameDecorated(glAccountData.getNameDecorated()).setTagId(glAccountData.getTagId()).setOrganizationRunningBalance(glAccountData.getOrganizationRunningBalance()).setAccountTypeOptions(accountTypeOptions).setUsageOptions(usageOptions).setAssetHeaderAccountOptions(assetHeaderAccountOptions).setLiabilityHeaderAccountOptions(liabilityHeaderAccountOptions).setEquityHeaderAccountOptions(equityHeaderAccountOptions).setIncomeHeaderAccountOptions(incomeHeaderAccountOptions).setExpenseHeaderAccountOptions(expenseHeaderAccountOptions).setAllowedAssetsTagOptions(allowedAssetsTagOptions).setAllowedLiabilitiesTagOptions(allowedLiabilitiesTagOptions).setAllowedEquityTagOptions(allowedEquityTagOptions).setAllowedIncomeTagOptions(allowedIncomeTagOptions).setAllowedExpensesTagOptions(allowedExpensesTagOptions);
    }
