...
public class AppUserReadPlatformServiceImpl implements AppUserReadPlatformService {
    ...
    public AppUserData retrieveUser(final Long userId) {
        ...
        AppUserData retUser = AppUserData.instance(user.getId(), user.getUsername(), user.getEmail(), user.getOffice().getId(), user.getOffice().getName(), user.getFirstname(), user.getLastname(), availableRoles, selectedUserRoles, linkedStaff, user.getPasswordNeverExpires());
        return retUser;
    }
    ...
    private static final class AppUserMapper implements RowMapper<AppUserData> {
        ...
        public String schema() {
            return " u.id as id, u.username as username, u.firstname as firstname, u.lastname as lastname, u.email as email, u.password_never_expires as passwordNeverExpires, " + " u.office_id as officeId, o.name as officeName, u.staff_id as staffId from m_appuser u " + " join m_office o on o.id = u.office_id where o.hierarchy like ? and u.is_deleted=false order by u.username";
        }
    }
    ...
    private static final class AppUserLookupMapper implements RowMapper<AppUserData> {
        ...
        public String schema() {
            return " u.id as id, u.username as username from m_appuser u " + " join m_office o on o.id = u.office_id where o.hierarchy like ? and u.is_deleted=false order by u.username";
        }
    }
