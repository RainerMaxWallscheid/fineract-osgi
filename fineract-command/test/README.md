...
Production **impl** has no unit-test source tree; tests run here:
...
| **OSGi container (later)** | Fragment attaches to `org.apache.fineract.command.impl` and shares the host classloader — no need to export impl packages to other domain modules |

Satellites (jdbc/async/disruptor) may `testImplementation` this project for the sample fixtures only; they do not depend on production `Fragment-Host` wiring at compile time for domain modules.
